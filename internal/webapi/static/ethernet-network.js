...
async function ethernetSafeApplyPanel(){
  ...
  const allUplinks=uplinkData.items||[],ethernet=allUplinks.filter(item=>item.type==='ETHERNET'),interfaces=interfaceData.items||[],free=ethernetFreeInterfaces(interfaces),enabledUplinks=allUplinks.filter(item=>item.enabled).sort((left,right)=>left.priority-right.priority);
  ...
  const configuredDNSFor=item=>Array.isArray(item.configured_dns)?item.configured_dns:[];
  const reorder=async(item,delta)=>{const ids=enabledUplinks.map(candidate=>candidate.id),index=ids.indexOf(item.id),next=index+delta;if(index<0||next<0||next>=ids.length)return;[ids[index],ids[next]]=[ids[next],ids[index]];await api('/api/v1/uplinks/priorities',{method:'PUT',body:JSON.stringify({ids})});notice('Общий приоритет физических выходов обновлён')};
  const setEnabled=async item=>{if(item.enabled){const impact=await api(`/api/v1/uplinks/${encodeURIComponent(item.id)}/impact?operation=DISABLE`),warning=impact.internet_may_be_blocked?'Этот выход активен и других готовых выходов сейчас нет: Internet будет закрыт до появления рабочего пути.\n\n':'',affected=`Будут сброшены результаты путей: VPN ${impact.affected_vpn_paths}, прямой ${impact.affected_direct_paths}.`;if(!confirm(`${warning}${affected}\n\nОтключить «${item.name}»?`))return}await api(`/api/v1/uplinks/${encodeURIComponent(item.id)}/${item.enabled?'disable':'enable'}`,{method:'POST',body:JSON.stringify({expected_desired_generation:item.desired_generation})});notice(item.enabled?'Ethernet-выход отключён':'Ethernet-выход включён; ожидается наблюдение адреса и маршрута');await renderNetwork()};
  const deleteUplink=async item=>{const impact=await api(`/api/v1/uplinks/${encodeURIComponent(item.id)}/impact?operation=DELETE`);if(!impact.allowed){notice(impact.blocked_reason==='DISABLE_FIRST'?'Сначала отключите Ethernet-выход':'Активный Ethernet-выход сейчас защищён от удаления');return}const message=`Будут удалены конфигурация выхода и результаты путей: VPN ${impact.affected_vpn_paths}, прямой ${impact.affected_direct_paths}. Сама физическая карта останется доступна для нового назначения.\n\nУдалить «${item.name}»?`;if(!confirm(message))return;const result=await api(`/api/v1/uplinks/${encodeURIComponent(item.id)}`,{method:'DELETE',headers:{'X-Confirm-Destructive':'delete-disabled-ethernet-uplink'},body:JSON.stringify({expected_desired_generation:item.desired_generation})});rememberEthernetApply(result,`${item.name}: удаление`);notice('Удаление применено временно; подтвердите его, иначе конфигурация восстановится')};
  const rows=ethernet.map(item=>({...item,address:item.address_mode==='STATIC'?`${item.configured_ipv4_cidr} → ${item.configured_gateway}`:`Автоматически (DHCP)${item.ipv4_cidr?' · сейчас '+item.ipv4_cidr:''}`,readiness:uplinkReasonLabel(item.readiness_reason),actions:actionGroup(
    actionButton('↑',()=>reorder(item,-1),false,!item.enabled||enabledUplinks[0]?.id===item.id),
    actionButton('↓',()=>reorder(item,1),false,!item.enabled||enabledUplinks.at(-1)?.id===item.id),
    actionButton(item.enabled?'Отключить':'Включить',()=>setEnabled(item)),
    actionButton('IP, DNS и MTU',()=>{const form=openEthernetDialog(`Настройки «${item.name}»`,ethernetNetworkFields({...item,ipv4_cidr:item.configured_ipv4_cidr,gateway:item.configured_gateway,dns:configuredDNSFor(item)}),'Применить на 60 секунд',async form=>{if(!confirm('Временно применить новые IP-настройки?'))return;const result=await api(`/api/v1/uplinks/${encodeURIComponent(item.id)}/network`,{method:'PUT',body:JSON.stringify({expected_desired_generation:item.desired_generation,...ethernetPayload(form)})});rememberEthernetApply(result,`${item.name}: IP-настройки`)});bindEthernetMode(form)}),
    ...
    actionButton('Удалить',()=>deleteUplink(item),true,item.enabled)
  )}));
  section.append(table([{label:'Выход',key:'name'},{label:'Карта',render:item=>item.interface_name||item.network_interface_id},{label:'Адрес',key:'address'},{label:'Состояние',render:item=>badge(item.state)},{label:'Что происходит',key:'readiness'},{label:'Observed / desired',render:item=>`${item.observed_generation}/${item.desired_generation}`},{label:'Действия',render:item=>item.actions}],rows));
  return section;
}
