...
func TestPortableBackupEncryptsSecretsAuthenticatesChunksAndContainsVerifiedManifest(t *testing.T) {
	...
	if err := json.Unmarshal(contents["manifest.json"], &manifest); err != nil || !manifest.SecretsIncluded || manifest.SnapshotID != artifact.SnapshotID || manifest.SchemaVersion != 20 || len(manifest.Files) != len(contents)-1 {
