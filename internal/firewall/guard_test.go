...
chain prerouting { type filter hook prerouting priority mangle; iifname @hilink_interfaces ct state { established, related } meta mark set ct mark; meta mark set iifname map @active_direct_marks }
chain forward_mss { type filter hook forward priority mangle; policy accept; tcp flags syn tcp option maxseg size set rt mtu comment "gateway-vpn clamp LAN TCP MSS to verified TUN route"; tcp flags syn tcp option maxseg size set rt mtu comment "gateway-vpn clamp allowed WireGuard TCP MSS to verified TUN route"; tcp flags syn tcp option maxseg size set rt mtu comment "gateway-vpn clamp LAN TCP MSS to verified direct route"; tcp flags syn tcp option maxseg size set rt mtu comment "gateway-vpn clamp allowed WireGuard TCP MSS to verified direct route"; }
...
chain output { type filter hook output priority filter; policy drop; oifname @hilink_interfaces meta mark != 0x00000000 ct mark set meta mark; oifname . meta mark . ip daddr @wireguard_endpoint_v4 udp dport 51821 accept; oifname . meta mark . ip daddr . udp dport @management_fabric_endpoints accept; }
