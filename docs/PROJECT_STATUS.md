...
| Репозиторий | `EXPANDED_SUCCESSOR_WORKTREE / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Новый schema-v17/uplink increment ещё проходит локальный audit, не подписан, не тегирован и не опубликован |
...
| SQLite | `V20_ETHERNET_POWER_MIGRATION_LOCAL_PASS / UPDATE_AUDIT_PENDING` | Migration 19 расширяет durable safe-apply typed manifest/candidate metadata; migration 20 сериализует active `SYSTEM_POWER`. Полный migration suite, backup/recovery/update fixtures и foreign-key checks проходят. Signed 13→20 update/recovery gate ещё не запускался |
| Safe network apply | `ETHERNET_TYPED_TRANSACTION_LOCAL_PASS / REAL_NETWORKD_NOT_RUN` | LAN и Ethernet `CREATE/REPLACE_INTERFACE/UPDATE_ADDRESS` используют один root broker, stable NIC ID, optimistic generation, persistent networkd snapshot/apply/rollback+reload, 60-секундное подтверждение и reboot recovery. Unit/domain tests проходят; реальный carrier/DHCP/static/gateway observer и routing/firewall convergence ещё не подключены |
| API / Web UI | `ETHERNET_SAFE_APPLY_AND_MANUAL_POWER_LOCAL_PASS / WG_INGRESS_UX_PENDING` | Вкладка физических выходов создаёт Ethernet, переназначает карту и меняет DHCP/static только через safe apply. В «Система и безопасность» добавлено питание: capability status, password re-auth, typed phrase, отменяемый countdown, audit и reboot/shutdown/verified RTC actions. WG-ingress clients, thematic log tabs и полный contextual help ещё не завершены |
