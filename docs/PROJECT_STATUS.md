...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V30_SOURCE / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_GATEWAY_PRIVILEGED_NETNS_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V4_SOURCE / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / GATEWAY_VPS_AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_PRIVILEGED_PACKET_3X_PASS / E2E_RELAY_ROLE_BACKUP_ROUNDTRIP_PASS / WG_ADMIN_ROTATION_ROLLBACK_PASS / LOCAL_RESOURCE_GATEWAY_PACKET_PASS / TOPOLOGY_PROFILE_SAFE_APPLY_PRIVILEGED_PASS / INITIAL_LAN_ROLE_IMPORT_LOCAL_PASS / GROUPED_RESPONSIVE_WEBUI_BROWSER_PASS / FIREWALL_SCHEMA_V6 / WG_INGRESS_LOCAL_PASS / WATCHDOG_19_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source Gateway schema 30 и VPS Agent schema 4 реализуют Gateway-terminated `wg-admin`, allowlisted VPS UDP relay, external end-to-end administrators и единую post-install safe-apply transaction четырёх topology profiles. Initial observation однократно импортирует выбранные установщиком физические LAN-порты по stable identity; profile transaction координирует роли, LAN/networkd, DHCP/DNS, firewall, policy routing и `wg-ingress`, а timeout/reboot возвращают полный LKG contour. Privileged Ubuntu gate доказал topology apply/commit/rollback, tamper rejection и peer-scoped one-arm anti-spoof; Gateway WebUI повторно прошёл desktop/320 px проверку новой вкладки сети. Следующий крупный блок — VPS Hub logs/update/diagnostics, затем Windows deploy delivery и exact release audit. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `95%`; общая production-готовность расширенного проекта — ориентировочно `77%`. Gateway/VPS privileged Management Fabric, local-resource packet path, root/encrypted backup boundary, обе backup-роли, end-to-end relay и единая topology-profile transaction теперь имеют локальное evidence. В программном scope остаются VPS Hub logs/update/diagnostics, Windows deploy delivery и финальная exact release/security сборка. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| API / Web UI | `GROUPED_GATEWAY_AND_VPS_LOCAL_PASS / TOPOLOGY_DESKTOP_AND_320PX_BROWSER_PASS` | Обе роли прошли все прежние вкладки при 320×720. Новая сетевая панель показывает четыре profile, stable physical inventory и role matrix, требует Preview перед Apply и объясняет external prerequisites. Повторный browser gate подтвердил desktop и 320×720 без page overflow/clipped buttons, локальную прокрутку широких matrix/table, пустую console и возврат к заголовку при смене вкладки. Hub Logs/Diagnostics как функции ещё не реализованы |
...
| Автоматические тесты | `TOPOLOGY_AND_ONE_ARM_PRIVILEGED_PASS / GATEWAY_AND_VPS_FABRIC_3X_PASS / E2E_RELAY_PRIVILEGED_3X_PASS / FULL_GO_TEST_PASS / SCHEMA25_EXACT_HOST_EVIDENCE` | Полный `go test ./... -count=1 -p 1`, `go vet ./...`, все Node/shell syntax и diff gates проходят. Новый Ubuntu 24 gate доказал topology apply/commit/rollback/tamper rejection и peer-scoped one-arm anti-spoof; предыдущие relay, Gateway/VPS fabric, firewall recovery, LAN/SSH, WireGuard ingress и systemd/GRUB gates остаются зелёными. Exact signed host artifact всё ещё относится к schema 25 |
...
**Итог аудита:** `28 PASS_LOCAL`, `4 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: обе backup-роли, information architecture, privileged Gateway/VPS Management Fabric, end-to-end administrator relay и единая topology transaction закрыты локально; полный VPS Hub lifecycle и Windows delivery требуют реализации/evidence. Затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Следующий инкремент — VPS Hub logs/update/diagnostics и закрытие оставшихся Agent lifecycle-функций; затем Windows deploy delivery. До exact release повторить privileged Gateway↔VPS systemd/browser/coexistence/topology gates. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP-логи, privileged many-to-many Management Fabric, проверенный inner `wg-admin` relay с role backup round-trip и единую post-install смену topology. Полный Hub lifecycle и Windows delivery ещё не завершены. Поэтому exact `a7a783b` нельзя объявлять окончательным `INSTALL_READY` для всего расширенного проекта, хотя schema-25 recovery gate закрыт. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.
