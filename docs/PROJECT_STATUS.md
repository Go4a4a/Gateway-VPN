...
**Текущий этап:** Три unique exact signed candidate прошли build/verify. Первые два DHCP apply fail-closed откатились и выявили state-root/internal privilege-drop defects. Третий собран после systemd-owned dnsmasq + `dnsmasq-base`; focused negative acceptance обнаружил, что combined `ss -lntu` не видит TCP/UDP одновременно на Ubuntu iproute2. Gate исправлен на отдельные TCP/UDP queries, реально отклоняет wildcard TCP/53 до transaction marker, package/shell tests PASS. Нужны commit, четвёртая unique signed apply/reboot и затем реализация принятого PLAN §9.8 self-health supervisor до 24-часового endurance
