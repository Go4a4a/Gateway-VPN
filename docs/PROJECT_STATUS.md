...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V25 / MANAGEMENT_FABRIC_SPEC_ADDED / VPS_AGENT_HUB_SPEC_ADDED / AMNEZIA_COEXISTENCE_REQUIRED / ROLE_BACKUP_SPEC_ADDED / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_ACCESS_SPEC_ADDED / GROUPED_WEBUI_SPEC_ADDED / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / WATCHDOG_17_IMPLEMENTED_19_SPECIFIED / WEBUI_UNINSTALL_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation, ручное capability-gated power management, интерактивный SSH/OpenSSH шаг, полный `wg-ingress`, тематические WebUI/SFTP-логи, доступные contextual help и текущий 17-component watchdog contour реализованы и локально проверены. Exact schema-25 source `0d28010fc5e99238d0636d627d245445dc854083` добавляет re-authenticated WebUI uninstall и прошёл source/Linux build, preserve/reinstall/purge gates. Его фактический new-PID1 retry сохранил durable marker, но выявил узкий regression: безусловный `networkctl reload` пытался активировать неработающий `systemd-networkd` и завершился timeout; source исправлен условным reload только уже active daemon и прошёл полный test/vet/cross-build/Bash gate, новый exact systemd candidate ещё не собран. Новое обязательное расширение от 2026-08-30 утверждено архитектурно: many-to-many `1..N Gateway ↔ 1..N VPS`, отдельный лёгкий VPS Agent/Hub, безопасный invitation pairing без VPS password в Gateway, Amnezia/foreign VPN coexistence, per-role encrypted WebUI backup/restore, Windows deploy wizard после core API, optional end-to-end `wg-admin`, local-resource aliases/ACL, post-install topology switching и grouped WebUI. Для него ещё нет schema/runtime/VPS Agent/firewall/UI implementation или test evidence. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, но относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `82%`; общая production-готовность расширенного проекта — ориентировочно `66%`. Снижение не означает потерю выполненного кода: утверждены новые VPS Agent/Hub, pairing, coexistence, VPS backup и Windows delivery gates поверх уже добавленного Management Fabric/local-resource scope. До этого расширения по-прежнему нужно завершить exact Ubuntu 24 uninstall recovery gate. Затем обязательны новые multi-site/multi-VPS/Amnezia netns/systemd/security tests и физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
|---|---|---|
| Архитектурный план | `DONE / GENERIC_UPLINK_AMENDED_2026-08-28` | Закреплены generic HILINK/ETHERNET uplinks, interface roles/replacement, whitelist-only evidence, bounded modem recovery, `wg-ingress`, тематические логи/SFTP и contextual help |
| Репозиторий | `EXPANDED_SUCCESSOR_WORKTREE / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Новый schema-v25/firewall-v4 successor проходит локальный audit, не подписан, не тегирован и не опубликован |
...
| Firewall / routing | `SCHEMA_V4_WG_INGRESS_NETNS_PASS / HARDWARE_PENDING` | Schema `4` сохраняет counters и atomic mutually-exclusive TUN/direct gates, а также принимает входящий WireGuard UDP только на явно выбранных listener interfaces. Privileged netns подтвердил startup policy, flush recovery, multi-LAN SSH/uplink isolation и реальный `wg-ingress` handshake/revoke lifecycle; физический capture остаётся обязательным |
...
| WireGuard management | `SINGLE_VPS_LOCAL_RUNTIME_PASS / MANY_TO_MANY_SPEC_ONLY / HOST_NOT_RUN` | Реализованный controller идемпотентно создаёт один `10.80.0.0/24 dev wg-mgmt protocol 186`; signed predecessor доказал endpoint/fwmark/address/handshake. Несколько VPS links, per-link keys/interfaces/subnets, pairing/rotation и несколько Gateway peers на VPS ещё не реализованы |
| Management Fabric / VPS Hub | `APPROVED_ARCHITECTURE / NOT_IMPLEMENTED` | Зафиксированы `1..N Gateway ↔ 1..N VPS`, simultaneously active links, invitation pairing без VPS credentials в Gateway, один lightweight Go Agent/embedded WebUI/CLI/SQLite, ownership-scoped watchdog, double ACL, portable admin profiles и recommended Gateway-terminated `wg-admin`. Нужны migration/runtime/VPS API/WebUI/install/security gates |
| Удалённые локальные ресурсы | `REQUIREMENT_AND_ARCHITECTURE_ADDED / NOT_IMPLEMENTED` | Зафиксированы default-off `GATEWAY_ONLY`, `KEENETIC_WAN`, routed WAN, router WireGuard и dedicated-LAN profiles, external prerequisite state, unique alias pools для одинаковых Home LAN и typed route/NAT generation. Текущий runtime по-прежнему не forwards Home LAN через `wg-mgmt` |
...
| Self-health / watchdog | `V2_17_COMPONENT_LOCAL_PASS / SUCCESSOR_19_SPEC_ONLY / HARDWARE_PENDING` | Текущие 17 компонентов локально проходят. Новые `management_fabric_routes` и `wireguard_admin` должны отдельно сверять links/resource routes/alias/ACL и optional inner admin interface/relay; оба специфицированы, но migration/observer/recovery/UI tests отсутствуют |
| SQLite | `V25_SYSTEM_UNINSTALL_MIGRATION_LOCAL_PASS / UPDATE_AUDIT_PENDING` | Migration 25 добавляет partial unique invariant для одной активной `SYSTEM_UNINSTALL`; migrations 23–24 сохраняют ingress/watchdog contour без private keys в SQLite. Clean schema 25, migration/idempotency, backup/recovery/update fixtures и foreign-key checks проходят. Exact signed 16→25 update/recovery gate ещё не запускался |
| Safe network apply | `ETHERNET_LIFECYCLE_RUNTIME_LOCAL_PASS / FULL_TOPOLOGY_SWITCH_SPEC_ONLY / REAL_NETWORKD_NOT_RUN` | LAN/Ethernet CRUD/replacement уже используют durable broker/snapshot/rollback. Единый post-install switch всех topology profiles с coordinated wg-ingress/API/firewall/policy state и подтверждением только через новый management path теперь обязателен, но ещё не реализован и не проверен |
| API / Web UI | `CURRENT_WG_INGRESS_LOG_HELP_BROWSER_PASS / GROUPED_NAV_AND_REMOTE_ACCESS_SPEC_ONLY` | Текущие WireGuard-клиенты, тематические logs и contextual help проходят browser smoke. Новый sidebar contract из шести групп и отдельные `VPS и каналы` / `Администраторы` / `Локальные ресурсы` / `Матрица доступа` ещё не реализованы; до этого нельзя считать UI нового scope завершённым |
| Logging / audit | `THEMATIC_EXPORT_LOCAL_PASS / HOST_NOT_RUN` | Journald namespace остаётся authoritative. Allowlisted категории, второй redaction pass, bounded current/archive exports, rotation/retention/disk budget, root-owned `gateway-vpn-log-readers` и read-only `/var/log/gateway-vpn/current` для штатного SFTP покрыты tests/installer gates; bare-metal OpenSSH/SFTP ещё не проверен |
...
| Backup / restore | `GATEWAY_EXACT_SIGNED_POWER_CUT_PASS / VPS_ROLE_SPEC_ONLY / HOST_NOT_RUN` | Gateway `.gvpn` exact signed gate доказал corruption rejection, staged no-op, success и exit-137 rollback. Для VPS утверждён отдельный encrypted `.gvpn-vps`, WebUI preview/apply, same-VPS/new-identity modes и durable rollback, но Agent/format/API/systemd tests ещё не реализованы; bare-metal power cut обеих ролей отсутствует |
| Signed update | `EXACT_13_TO_16_POINTER_PASS / EXACT_16_TO_24_HOST_CONTRACT_PASS / HARDWARE_POWER_CUT_PENDING` | Pointer candidate `9301e05` прошёл controlled health rejection, successful `13 → 16`, finalization/reboot и interruption recovery. Exact `gc632868` доказал public-baseline `16 → 24`, controlled failure rollback и повторный boot rollback после независимых SIGKILL upgrade/recovery; bare-metal power cut остаётся внешним gate |
| Packaging | `PRODUCTION_GVKEY_READY / REPRODUCIBLE_C632868 / HOST_UPGRADE_PASS / PUBLIC_5723940_IMMUTABLE` | Две offline сборки exact `c632868` совпали побайтно: 85 files; Gateway `1c071b9a…`, VPS `24b0802e…`, bootstrap `a4b2fba8…`, deploy `33ee9710…`, channel `87aced35…`; disposable signer `8231e4d3…`. Production key не открывался. Public Release неизменён |
| Uninstall | `WEBUI_DURABLE_GUARDIAN_LOCAL_PASS / NEW_PID1_REGRESSION_FIXED_SOURCE / EXACT_RETEST_PENDING / HOST_NOT_RUN` | Password re-auth, CSRF, точная фраза, impact acknowledgements, preserve/purge, typed broker, exclusive fsync marker, signed root-only tooling, boot guardian и fsync terminal receipt реализованы. CLI/WebUI применяют `PATH_BLOCKED`, purge не создаёт скрытую DB-копию и packages остаются. Фактический new-PID1 retry сохранил marker/tooling, но остановился на попытке D-Bus activation неактивного networkd; conditional reload исправлен и source tests проходят, новый exact systemd retry ещё обязателен |
...
| Автоматические тесты | `SCHEMA_V25_FULL_LINUX_NETNS_SYSTEMD_BROWSER_HOST_UPGRADE_PASS` | Полные serial `go test ./... -count=1` и `go vet ./...`, Linux cross-build, JavaScript/shell syntax, diff check, Ubuntu WireGuard JSON contour, SSH multi-LAN/uplink/opt-out, firewall recovery, реальный `wg-ingress` kernel handshake/lifecycle, offline systemd verify, browser preview и exact host-upgrade matrix проходят. Exact schema-25 uninstall systemd gate и hardware gates ещё нужны |
...
| 10 | `IN_PROGRESS_LOCAL` | Прежние README/NETWORKING/OPERATIONS/H1/H2 входили в проверенный signed tree; новый Management Fabric/topology/local-resource contract уже добавлен в PLAN/NETWORKING/OPERATIONS, но ещё не реализован, не дополнен фактическими commands/evidence и не собран в новый signed tree |
...
| 36 | `IN_PROGRESS_LOCAL` | Durable Ethernet/LAN safe-apply, LKG snapshot, timeout/reboot recovery и new-address confirmation существуют; единая transaction всех topology profiles с wg-ingress/API/firewall coordination ещё не реализована |
| 37 | `IN_PROGRESS_LOCAL` | Один Gateway↔VPS contour, endpoint uplink failover и admin peer уже существуют. Multiple simultaneous VPS links, per-link keys/subnets/interfaces и multiple Gateway peers на VPS пока только специфицированы |
| 38 | `IN_PROGRESS_LOCAL` | Текущий firewall имеет management allowlist и запрет uplink exposure, но versioned double ACL, pairing/rotation generations и explicit inter-site/admin deny для many-to-many ещё не реализованы |
| 39 | `IN_PROGRESS_LOCAL` | Gateway management и `wg-ingress ROUTER_ROUTED` primitives существуют; Keenetic WAN/routed/dedicated-LAN resource profiles, external prerequisite health и unified publication API отсутствуют |
| 40 | `IN_PROGRESS_LOCAL` | Общая subnet/AllowedIPs conflict validation реализована для текущих roles, но per-`site × resource × VPS link` alias allocator, typed prefix translation и multi-site/multi-VPS overlap netns gate отсутствуют |
| 41 | `IN_PROGRESS_LOCAL` | Текущий WebUI имеет предметные страницы, thematic logs и accessible help; новая шестиуровневая grouped navigation и четыре remote-access subpages ещё не реализованы |
| 42 | `IN_PROGRESS_LOCAL` | Текущий routed VPS hub защищает WebUI/SSH прикладным TLS/auth, но Gateway-terminated `wg-admin`, UDP relay allocation, inner peer ACL и VPS-spoof negative packet gate отсутствуют |
| 43 | `IN_PROGRESS_LOCAL` | VPS installer/CLI primitives существуют, но единый lightweight Agent с embedded restricted Hub/SQLite и ownership-scoped watchdog ещё не реализован |
| 44 | `IN_PROGRESS_LOCAL` | Текущая VPS роль владеет отдельной nft table и не делает global flush, но scope-only base-chain semantics и install/update/watchdog/uninstall coexistence gate с Amnezia/Docker/UFW отсутствуют |
| 45 | `IN_PROGRESS_LOCAL` | Linux SSH orchestrator и cross-platform Go command существуют; Windows OpenSSH adapter, portable signed `.exe`, wizard и clean Windows VM readiness gate отсутствуют |
| 46 | `IN_PROGRESS_LOCAL` | Gateway encrypted WebUI backup/restore и power-cut gate проходят; VPS `.gvpn-vps`, identity-aware modes, role rejection и VPS restore transaction пока только специфицированы |

**Итог аудита:** `20 PASS_LOCAL`, `12 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: текущему schema-25 successor ещё нужен завершающий recovery gate, а новый Management Fabric/VPS Agent/coexistence/backup/Windows scope требует отдельной реализации и Linux/Windows evidence; затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Сначала завершить уже начатый exact schema-25 uninstall recovery gate: зафиксировать conditional-networkd source, собрать новый disposable exact candidate и повторить preserve/SIGKILL/new-PID1 с очищенным `/run`, terminal receipt и preserved schema-25 data без ручной замены hash-bound tooling. Затем начать schema 26 Management Fabric foundation без host mutation: migrations/repositories, collision/ACL validation, pairing state machine и read-only route/firewall plan fixtures. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, single-VPS management interface, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP-логи и понятный выбор каждого install-компонента. Новый обязательный Management Fabric/local-resource scope пока является архитектурой, поэтому текущий exact commit нельзя объявлять окончательным `INSTALL_READY` для всего проекта даже после закрытия schema-25 recovery gate. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.

После реализации Management Fabric отдельный путь до production release включает реальные multi-Gateway/multi-VPS/Keenetic/HiLink проверки, найденные исправления, 24-часовой developer и несокращаемый 72-часовой release endurance. Прежний ориентир `4–8 дней` относился к старому single-VPS scope и больше не используется как обещание. Новый срок имеет смысл фиксировать после schema 26 foundation и первого двух-VPS netns gate; без фактического доступа к Linux/VPS/оборудованию можно передать install-ready candidate, но нельзя честно поставить production status `DONE`.
...
9. Many-to-many Management Fabric, VPS Hub, local-resource alias/ACL и grouped WebUI добавлены в требования 2026-08-30, но текущая schema 25 и release artifacts их не содержат. До новых migrations/runtime/netns/systemd/browser gates нельзя использовать прежний single-VPS PASS как доказательство этого расширения.
