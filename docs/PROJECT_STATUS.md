...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V24 / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / THEMATIC_LOGGING_LOCAL_PASS / WATCHDOG_17_COMPONENT_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact source `1b90ffcb99b25f79954cbc1b4bde7bcc0140175d` и disposable-signed `0.1.0-successor.1b90ffc` сохраняются как доказательство прежнего universal-installer scope, но больше не являются кандидатом текущего расширенного контракта. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation, ручное capability-gated power management, интерактивный SSH/OpenSSH шаг, полный `wg-ingress`, тематические WebUI/SFTP-логи, доступные contextual help и 17-component watchdog contour реализованы и локально проверены. Exact source `0858294aa308a0b4a8d3fb641bd31eaba1ac5cf9` дважды воспроизводимо собрался как `0.1.0-successor.g0858294`; clean Ubuntu 24.04 с SSH/WG ingress ON прошёл fresh install, полную readiness, повторный идемпотентный installer и десять последовательных systemd validator runs. Для перехода immutable public baseline с прежнего root lifecycle на schema-v24 candidate реализована отдельная signed host-contract transaction с cold snapshot, exact DB history verification, boot recovery и сохранением исходного first-install marker; локальные source/systemd gates проходят. Следующий обязательный gate — exact signed schema `16 → 24` success/injected-failure/new-PID1 recovery matrix. Firmware/USB recovery, RTC wake-from-S5 и физические gates остаются впереди. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** прежний scope остаётся воспроизводимо завершённым примерно на `99%`, но это больше не процент текущего проекта. После успешного exact fresh/idempotency gate текущая программная реализация оценивается примерно в `95%`, общая production-готовность — примерно в `77%`. Незавершённый программный объём сосредоточен в exact new-PID1, SSH/WG OFF, update/rollback и WebUI-uninstall audit, а также в исправлениях, которые эти проверки могут выявить. Затем обязательны физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Репозиторий | `EXPANDED_SUCCESSOR_WORKTREE / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Новый schema-v24/firewall-v4 successor проходит локальный audit, не подписан, не тегирован и не опубликован |
...
| SQLite | `V24_WG_INGRESS_LOGGING_MIGRATION_LOCAL_PASS / UPDATE_AUDIT_PENDING` | Migrations 23–24 добавляют ingress server/peers/runtime/policies и 17-й watchdog component без хранения private keys в SQLite. Clean schema 24, migration/idempotency, backup/recovery/update fixtures и foreign-key checks проходят. Signed 13→24 update/recovery gate ещё не запускался |
...
| Signed update | `EXACT_13_TO_16_POINTER_PASS / HOST_CONTRACT_UPGRADE_LOCAL_PASS / EXACT_16_TO_24_PENDING` | Pointer candidate `9301e05` прошёл controlled health rejection, successful `13 → 16`, finalization/reboot и interruption recovery. Отдельная host-contract transaction для несовместимого lifecycle реализована и проходит local/Linux source+unit gates; exact public-baseline `16 → 24` success/failure/new-PID1 matrix ещё не запускалась |
| Packaging | `PRODUCTION_GVKEY_READY / REPRODUCIBLE_0858294 / FRESH_IDEMPOTENCY_PASS / PUBLIC_5723940_IMMUTABLE` | Две offline сборки exact `0858294` совпали побайтно: Gateway `70e679f8…`, VPS `e7ea3f3b…`, bootstrap `ff88263c…`, deploy `0976dcae…`, channel `baa378c6…`; disposable signer `8231e4d3…`. Clean Ubuntu 24.04 SSH/WG ON fresh install, readiness и идемпотентный repeat — PASS. Production key не открывался. Предыдущий public immutable Release по-прежнему имеет проверенные `10/10` assets |
| Uninstall | `DOCKER_SYSTEMD_PRESERVE_REINSTALL_PURGE_PASS / RECOVERY_V20_FIX_PASS / WEBUI_TRIGGER_PENDING / HOST_NOT_RUN` | Gateway exact `.27`: default preserve, reboot, reinstall с сохранённой SQLite, explicit purge с root-only DB backup и reboot — PASS. Fresh v20 recovery после исправления завершил marker transaction и восстановил LAN/SSH состояния. Для WebUI принят отдельный re-authenticated root-owned uninstall job с выбором preserve/purge; реализация ещё не начата. Bare-metal cleanup ещё не выполнялся |
...
| Автоматические тесты | `SCHEMA_V24_FULL_LINUX_NETNS_SYSTEMD_BROWSER_PASS` | Полные serial `go test ./... -count=1` и `go vet ./...`, JavaScript/shell syntax, diff check, Ubuntu WireGuard JSON contour, SSH multi-LAN/uplink/opt-out, firewall recovery, реальный `wg-ingress` kernel handshake/lifecycle, systemd/GRUB verify и browser preview проходят. Новый exact signed update/hardware gate ещё нужен |
