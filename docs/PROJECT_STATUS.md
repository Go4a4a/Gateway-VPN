...
| Репозиторий | `PRODUCTION_SIGNED_TESTING_3BB466B_DRAFT_VERIFIED / FULL_CI_RUN_33863273880_PASS / PUBLIC_BASE_IMMUTABLE` | Exact testing tag `v0.1.1-testing.3bb466b` и скрытый prerelease draft созданы; `14/14` remote assets совпадают по имени, размеру и SHA-256. Draft не опубликован. Public stable/latest `v0.1.0-successor.5723940` и Release `378316577` неизменны |
...
| Packaging | `PRODUCTION_GVKEY_SIGNED_TESTING_3BB466B / REMOTE_14_OF_14_SHA256_PASS / NATIVE_WINDOWS_TRUST_SMOKE_PASS / PUBLIC_5723940_IMMUTABLE` | Production signer `fceb4a54…eda60c` подписал exact `0.1.1-testing.3bb466b`: Gateway `57`, VPS `35`, channel `5` artifacts. GitHub draft остаётся hidden prerelease; remote download `14/14` совпал по имени, размеру и SHA-256. Текущий Windows EXE принимает exact signed identity и отвергает wrong hash/signer, tampered signature/PE; stable release не изменён |
