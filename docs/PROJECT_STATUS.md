...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / GATEWAY_REMOTE_SIGNED_SOURCE_LOCAL_PASS / GATEWAY_COMPLETE_RESTORE_POINT_SYSTEMD_REBOOT_PASS / VPS_UPDATE_SYSTEMD_SIGKILL_ROLLBACK_PASS / BOTH_ROLES_SHARED_LIFECYCLE_LOCK_PASS / VPS_UNINSTALL_LIFECYCLE_SYSTEMD_PASS / SCHEMA_V32_SOURCE / AUTOMATIC_UPDATE_SCHEDULER_CORE_LOCAL_PASS / MANAGEMENT_FABRIC_RUNTIME_OBSERVER_PRIVILEGED_PASS / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_GATEWAY_PRIVILEGED_NETNS_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V4_SOURCE / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / VPS_OPERATIONS_DIAGNOSTICS_LOCAL_PASS / VPS_OPERATIONS_ROOT_BOUNDARY_PRIVILEGED_PASS / GATEWAY_VPS_AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_PRIVILEGED_PACKET_3X_PASS / E2E_RELAY_ROLE_BACKUP_ROUNDTRIP_PASS / WG_ADMIN_ROTATION_ROLLBACK_PASS / LOCAL_RESOURCE_GATEWAY_PACKET_PASS / TOPOLOGY_PROFILE_SAFE_APPLY_PRIVILEGED_PASS / INITIAL_LAN_ROLE_IMPORT_LOCAL_PASS / GROUPED_RESPONSIVE_WEBUI_BROWSER_PASS / FIREWALL_SCHEMA_V6 / WG_INGRESS_LOCAL_PASS / WATCHDOG_19_PLUS_NEW_WORKERS_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. Текущий schema-32 source добавляет durable signed-update scheduler и generation-bound runtime observation Management Fabric поверх manual Stable/Testing/exact HTTPS staging, complete restore points и общего lifecycle lock. Полный Linux race/vet, responsive WebUI и disposable privileged Ubuntu 24 nftables/WireGuard/systemd gates проходят. До перехода к Windows deploy нужно закрыть оставшиеся требования automatic update control plane: bounded maximum apply delay и service-route ladder для check/download. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `98%`; относительно текущего расширенного обязательного scope программная часть ориентировочно `96%`. Готовность к первой установке на физический стенд — примерно `92%`, полная production-готовность — примерно `81%`. Durable scheduler core, signed Gateway/VPS update foundations, complete Gateway restore-point rollback и live Management Fabric observations имеют local/privileged evidence. В программном scope остаются automatic-update service-route/max-delay contract, Windows deploy delivery и финальная exact release/security сборка. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Репозиторий | `EXPANDED_SCHEMA32_WORKTREE / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Exact local `a7a783b` schema-v25 baseline сохранён; текущий schema-32 блок ещё не является production-signed/tagged release |
...
| SQLite | `GATEWAY_V32_AND_VPS_AGENT_V4_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Gateway schema 32 добавляет singleton durable scheduler state/lease поверх schema-31 update policy; internal lease fields не выдаются API. Contiguous VPS Agent schema 4 хранит public relay allocation и external administrator trust mode без inner private key. v1→current migration, integrity/FK/lifecycle и оба encrypted backup/restore round-trip проходят. Exact production-signed host evidence пока относится к Gateway schema 25 |
...
| Signed update | `AUTOMATIC_SCHEDULER_CORE_LOCAL_PASS / GATEWAY_COMPLETE_RESTORE_SYSTEMD_PASS / VPS_TRANSACTION_SYSTEMD_SIGKILL_ROLLBACK_PASS / BOTH_ROLES_SHARED_LIFECYCLE_LOCK_PASS / SERVICE_ROUTE_MAX_DELAY_PENDING / HARDWARE_POWER_CUT_PENDING` | Schema-32 worker имеет durable lease/deadlines/jitter, separate check/download/apply, UTC window, manual/automatic ownership, maintenance suppression, fresh FULL+management readiness и ambiguous-intent no-redispatch. Полный Linux race нашёл и после immutable interval/join исправления подтвердил lease-renewal lifecycle. До полного §17.3.1 остаются service-route ladder и bounded maximum apply delay; real bare-metal power-cut также pending |
...
| 47 | `PARTIAL_EXTERNAL` | Signed GitHub Stable/Testing manual check/download, exact HTTPS/upload, two-stage pointer transaction, complete restore-point retention/UI и privileged manual rollback/reboot recovery прошли. Schema-32 scheduler core и multi-VPS runtime observation реализованы и прошли Linux race/privileged gate; service-route/max-delay остаются локальным gap, exact production-signed tree и bare-metal power-cut — внешним gap |

**Итог аудита:** `29 PASS_LOCAL`, `3 IN_PROGRESS_LOCAL`, `13 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: обе backup-роли, information architecture, privileged Gateway/VPS Management Fabric, end-to-end administrator relay, единая topology transaction, Hub operations/diagnostics и signed VPS lifecycle закрыты локально; automatic remote Gateway update требует service-route/max-delay завершения, а Windows delivery — реализации/evidence. Затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Следующий инкремент — завершить automatic update control plane поверх уже доказанного schema-32 scheduler core: добавить bounded maximum apply delay без принудительного unsafe Apply и service-route ladder для signed check/download через разрешённые VPN/uplink/direct service paths. После focused regressions перейти к Windows deploy delivery и exact release/security audit. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP/Hub-логи, bounded diagnostics, privileged many-to-many Management Fabric с live observations, проверенный inner `wg-admin` relay с role backup round-trip, единую post-install смену topology, signed VPS update и Gateway automatic scheduler core с complete restore-point recovery. Service-route/max-delay automatic update contract, Windows delivery и новый exact production-signed audit ещё не завершены. Поэтому exact `a7a783b` нельзя объявлять окончательным `INSTALL_READY` для всего расширенного проекта, хотя schema-25 recovery gate закрыт. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.
