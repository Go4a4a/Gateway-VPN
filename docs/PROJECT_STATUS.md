...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V17_FOUNDATION / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact source `1b90ffcb99b25f79954cbc1b4bde7bcc0140175d` и disposable-signed `0.1.0-successor.1b90ffc` сохраняются как доказательство прежнего universal-installer scope, но больше не являются кандидатом текущего расширенного контракта. После согласования generic `uplink × access method`, Ethernet uplinks, NIC roles/replacement, direct-only whitelist indicators, bounded modem recovery, `wg-ingress`, тематических логов/SFTP и полного contextual help начата successor schema v17 и canonical uplink repository. Новый tag/Release запрещён до реализации и проверки всего расширения.

**Оценка прогресса:** прежний scope остаётся воспроизводимо завершённым примерно на `99%`, но это больше не процент текущего проекта. После расширения знаменателя текущая программная реализация оценивается примерно в `80%`, общая production-готовность — примерно в `65%`. Это не потеря уже сделанного: installer, update/rollback, fail-closed, подписки, базовая multi-modem policy, API/auth/logging/watchdog и release pipeline переиспользуются. Незавершённый объём теперь включает generic uplink runtime, Ethernet/role safe apply, whitelist-only classifier, реальный bounded USB recovery, полный `wg-ingress`, новые WebUI/exports/help и повторный сквозной audit. После этого всё равно остаются physical Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture и обязательные 24/72-часовые endurance.
...
| SQLite | `V17_GENERIC_UPLINK_MIGRATION_LOCAL_PASS / UPDATE_AUDIT_PENDING` | Migration 17 сохраняет IDs HiLink/path/node/target/runtime, создаёт generic uplinks, interface roles, WG ingress, recovery и log-export schema; upgrade fixture и foreign-key check проходят. Полный signed 13→17 update/recovery gate ещё не запускался |
...
| API / Web UI | `LEGACY_SCOPE_PASS / GENERIC_UPLINK_WG_INGRESS_UX_PENDING` | Прежние 98 routes и direct/VPN read model проходят tests; generic uplink/NIC roles, whitelist indicators, WG-ingress clients, thematic log tabs и contextual help ещё не подключены |
...
| Автоматические тесты | `SCHEMA_V17_FULL_GO_TEST_VET_PASS / SYSTEMD_NETNS_SUCCESSOR_AUDIT_PENDING` | Полные `go test ./...` и `go vet ./...`, migration preservation/FK, compatibility projection и canonical uplink create/interface replacement проходят; прежний exact CI остаётся evidence только прежнего scope. Новый systemd/netns gate будет запущен после подключения runtime |
...
| 29 | `IN_PROGRESS_LOCAL` | Schema v17 атомарно переносит HiLink и прежние path IDs в generic uplinks; bounded compatibility triggers проецируют ещё не перенесённые HiLink/path/runtime writes в той же transaction. Canonical Ethernet create и generation-safe NIC replacement проходят tests; selector/runtime/API ещё подлежат прямому переносу |
