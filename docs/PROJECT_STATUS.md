...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V26 / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_DATA_PAIRING_RENDERER_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / AMNEZIA_COEXISTENCE_REQUIRED / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_ACCESS_FOUNDATION / GROUPED_WEBUI_SPEC_ADDED / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / WATCHDOG_17_IMPLEMENTED_19_SPECIFIED / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source schema 26 реализует durable Management Fabric data model, bounded invitation pairing и read-only route/alias/ACL renderer. Теперь также реализованы отдельный непривилегированный VPS Agent с restricted TLS WebUI/SQLite/auth и полный role-separated `.gvpn-vps` backup/preview/restore: same-VPS quarantine, import-as-new с новой identity и `wg-mgmt` без stale peers, root journal, handled rollback и boot recovery после частичного swap. Signed VPS installer локально интегрирует Agent account/state/config/TLS/admin/systemd и Admin-only TCP/9443 firewall. Для расширения всё ещё отсутствуют privileged many-to-many WireGuard/firewall apply, полный VPS pairing/resources/watchdog API, Amnezia coexistence lifecycle и grouped Gateway WebUI. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `86%`; общая production-готовность расширенного проекта — ориентировочно `69%`. Schema 26 data/pairing/renderer и обе backup-роли закрыты локальным кодом и тестами; VPS Agent backup-capable installation собрана, но полный many-to-many host runtime/Hub API/watchdog и coexistence ещё впереди. Далее обязательны multi-site/multi-VPS/Amnezia netns/systemd/security tests и физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Management Fabric / VPS Hub | `VPS_AGENT_BACKUP_LOCAL_PASS / FULL_FABRIC_RUNTIME_PENDING` | Реализованы role-specific Agent DB/schema/immutable identity, restricted TLS WebUI/auth, `.gvpn-vps`, safe restore и signed installer wiring. Management data/pairing/read-only renderer также проходят; Gateway/links/admin/resources/ACL/watchdog tabs пока placeholders, privileged many-to-many apply и coexistence отсутствуют |
...
| Автоматические тесты | `SCHEMA_V26_VPS_BACKUP_FULL_GO_PASS / VPS_SYSTEMD_NFT_SOURCE_PASS / SCHEMA25_EXACT_HOST_EVIDENCE` | Полные Windows и offline Linux `go test ./... -count=1`, vet и Linux builds проходят. Ubuntu 24 systemd graph и реальная загрузка owned VPS nft table проходят; exact signed install/reboot/preserve/import restore и many-to-many privileged runtime для нового source ещё не выполнялись |
...
| 43 | `IN_PROGRESS_LOCAL` | Lightweight Go Agent, separate SQLite/auth, TLS-only restricted Hub, Agent system user/state boundary и backup page реализованы; Gateway/links/admin/resources/ACL/update/diagnostics APIs и ownership-scoped watchdog ещё не завершены |
...
Следующий инкремент — VPS Agent pairing/links/admin/resources/ACL API и ownership-scoped watchdog, затем privileged many-to-many `gvm<N>`/VPS peer apply с versioned double ACL. До расширения root mutation добавить negative Amnezia/Docker/UFW coexistence fixtures. Production `.gvkey` не открывать, tag/Release не создавать.
