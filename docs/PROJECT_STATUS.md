...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V25 / MANAGEMENT_FABRIC_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_ACCESS_SPEC_ADDED / GROUPED_WEBUI_SPEC_ADDED / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / WATCHDOG_17_IMPLEMENTED_19_SPECIFIED / WEBUI_UNINSTALL_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation, ручное capability-gated power management, интерактивный SSH/OpenSSH шаг, полный `wg-ingress`, тематические WebUI/SFTP-логи, доступные contextual help и текущий 17-component watchdog contour реализованы и локально проверены. Exact schema-25 source `0d28010fc5e99238d0636d627d245445dc854083` добавляет re-authenticated WebUI uninstall и прошёл source/Linux build, preserve/reinstall/purge gates. Его фактический new-PID1 retry сохранил durable marker, но выявил узкий regression: безусловный `networkctl reload` пытался активировать неработающий `systemd-networkd` и завершился timeout; source исправлен условным reload только уже active daemon и прошёл полный test/vet/cross-build/Bash gate, новый exact systemd candidate ещё не собран. Новое обязательное расширение от 2026-08-30 пока зафиксировано только архитектурно: many-to-many `1..N Gateway ↔ 1..N VPS`, несколько одновременно active management links, restricted VPS Hub, optional end-to-end `wg-admin` through VPS UDP relay, explicit Gateway/Keenetic/local-resource publication, per-link alias prefixes для одинаковых Home LAN, post-install topology switching и grouped WebUI. Для него ещё нет schema/runtime/firewall/UI implementation или test evidence. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, но относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `86%`; общая production-готовность расширенного проекта — ориентировочно `70%`. Снижение не означает потерю выполненного кода: добавлен существенный новый объём Management Fabric/VPS Hub/local-resource routing/ACL/alias/WebUI и его Linux gates. До этого расширения по-прежнему нужно завершить exact Ubuntu 24 uninstall recovery gate. Затем обязательны новые multi-site/multi-VPS netns/systemd/security tests и физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Management Fabric / VPS Hub | `REQUIREMENT_AND_ARCHITECTURE_ADDED / NOT_IMPLEMENTED` | Зафиксированы `1..N Gateway ↔ 1..N VPS`, simultaneously active links, separate uplink selection, one-time pairing, double ACL, restricted VPS Hub, portable admin profiles и recommended Gateway-terminated `wg-admin` through VPS UDP relay. Нужны новые migration/runtime/VPS API/WebUI/install/security gates |
...
**Итог аудита:** `20 PASS_LOCAL`, `8 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: текущему schema-25 successor ещё нужен завершающий recovery gate, а новый Management Fabric scope требует отдельной реализации и Linux evidence; затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.
