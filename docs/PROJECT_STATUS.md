...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V26 / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_DATA_PAIRING_RENDERER_LOCAL_PASS / VPS_AGENT_HUB_SPEC_ADDED / AMNEZIA_COEXISTENCE_REQUIRED / ROLE_BACKUP_SPEC_ADDED / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_ACCESS_FOUNDATION / GROUPED_WEBUI_SPEC_ADDED / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / WATCHDOG_17_IMPLEMENTED_19_SPECIFIED / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation, ручное capability-gated power management, интерактивный SSH/OpenSSH шаг, полный `wg-ingress`, тематические WebUI/SFTP-логи, доступные contextual help и текущий 17-component watchdog contour реализованы и локально проверены. Exact schema-25 candidate `0.1.0-successor.ga7a783b` из commit `a7a783bbc7c48f9aad84afc766b05f74f55cad72` воспроизводимо собран и закрыл regression: WebUI `PRESERVE_DATA`, SIGKILL guardian после durable tooling, новый PID 1 с пустым `/run` и unavailable networkd, terminal receipt, точное сохранение DB/WAL/SHM/secrets, отсутствие owned host projection и reinstall прошли. В текущем successor source schema 26 уже реализует durable Management Fabric data model, collision validation, одноразовый invitation pairing и read-only owned route/alias/ACL renderer без host mutation; полный offline suite и vet проходят. Для расширения всё ещё отсутствуют VPS Agent runtime/API/Hub, privileged many-to-many WireGuard/firewall apply, grouped WebUI и VPS `.gvpn-vps` implementation/evidence. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, но относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `84%`; общая production-готовность расширенного проекта — ориентировочно `68%`. Снижение относительно прежнего узкого scope не означает потерю выполненного кода: утверждены новые VPS Agent/Hub, pairing, coexistence, VPS backup и Windows delivery gates поверх уже добавленного Management Fabric/local-resource scope. Schema 26, typed repositories, pairing и read-only renderer теперь закрыты локально; privileged runtime/VPS role ещё впереди. Exact Ubuntu 24 uninstall recovery gate закрыт локально. Далее обязательны новые multi-site/multi-VPS/Amnezia netns/systemd/security tests и физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Management Fabric / VPS Hub | `SCHEMA26_REPOSITORY_PAIRING_RENDERER_LOCAL_PASS / VPS_AGENT_NOT_IMPLEMENTED` | Реализованы `1..N Gateway ↔ 1..N VPS` data model, prefix/address/identity/ACL validation, digest-only invitation с expiry/attempt budget/exactly-once consume и deterministic read-only renderer protocol 186/routes/alias/double ACL. Host mutation отсутствует; нужны VPS Agent DB/API/WebUI/install/coexistence/security gates |
...
| SQLite | `V26_MANAGEMENT_FABRIC_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Migration 26 добавляет durable Management Fabric, монотонные counters и schema checks для digest-only token/secret refs/no-default-route. Fresh/idempotent migration, integrity/FK и backup/update tests проходят; exact signed host evidence пока относится к schema 25, physical power cut остаётся внешним gate |
...
| Packaging | `PRODUCTION_GVKEY_READY / REPRODUCIBLE_A7A783B / HOST_UPGRADE_PASS / PUBLIC_5723940_IMMUTABLE` | Две network-disabled offline сборки exact `a7a783b` совпали побайтно: 87 files/31 dirs; Gateway `af632ddd…`, VPS `c15eb94b…`, bootstrap `d140e467…`, deploy `b558ab6b…`, channel `26399bf5…`; disposable signer `8231e4d3…`. Production key не открывался. Public Release неизменён |
...
| Автоматические тесты | `SCHEMA_V26_FULL_GO_PASS / SCHEMA25_PRIVILEGED_HOST_EVIDENCE` | Полные offline `go test ./... -count=1` и `go vet ./...` проходят с schema 26 и Management Fabric/pairing/renderer tests. Предыдущие Linux cross-build/netns/systemd/browser/host-upgrade/uninstall gates относятся к exact schema-25 candidate; новый privileged schema-26 runtime gate ещё не выполнялся. Hardware gates нужны |
...
**Итог аудита:** `20 PASS_LOCAL`, `12 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: exact schema-25 recovery gate закрыт, но новый Management Fabric/VPS Agent/coexistence/backup/Windows scope требует отдельной реализации и Linux/Windows evidence; затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Начать schema 26 Management Fabric foundation без host mutation: migrations/repositories для `1..N Gateway ↔ 1..N VPS`, conflict-free address/interface/table/mark allocation, versioned double-ACL validation, bounded invitation pairing state machine и read-only route/firewall plan fixtures. Затем добавить VPS Agent storage/API skeleton и Amnezia/Docker/UFW coexistence fixture до первой host mutation. Production `.gvkey` не открывать, tag/Release не создавать.
