...
| SQLite | `V18_GENERIC_RUNTIME_MIGRATION_LOCAL_PASS / UPDATE_AUDIT_PENDING` | Migration 17 создаёт generic uplinks/interface roles/WG ingress/recovery/log-export schema; migration 18 делает generic path/runtime foreign keys authoritative и добавляет `events.uplink_id`. Upgrade fixtures и foreign-key check проходят. Полный signed 13→18 update/recovery gate ещё не запускался |
...
| API / Web UI | `GENERIC_UPLINK_INVENTORY_AND_WHITELIST_PASS / WG_INGRESS_UX_PENDING` | Authenticated `/uplinks` и `/network/interfaces` выдают HiLink/Ethernet, NIC roles и общий path read model без identity hashes/full MAC/secrets; новая вкладка «Физические выходы» и generic subscription projection подключены. Safe Ethernet mutations, WG-ingress clients, thematic log tabs и полный contextual help ещё не завершены |
...
| Автоматические тесты | `SCHEMA_V18_FULL_GO_TEST_VET_PASS / SYSTEMD_NETNS_SUCCESSOR_AUDIT_PENDING` | Полные `go test ./...`, `go vet ./...`, JavaScript syntax и diff checks текущего exact worktree проходят. Прежний exact CI остаётся evidence только прежнего scope. Новый systemd/netns gate будет запущен после завершения remaining vertical slices |
