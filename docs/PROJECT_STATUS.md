...
**Текущий этап:** после кандидата `0.1.0-endurance.618d617` реализован новый незакоммиченный инкремент traffic accounting: отдельные user/service nftables counters, firewall schema generation `2`, SQLite schema `13`, runtime/API/UI и безопасная смена firewall schema при update/rollback. Windows, native Linux, реальный kernel nftables reader, netns guard и отдельный systemd dependency probe прошли. Старый exact candidate больше не является текущим install-ready артефактом; следующий рубеж — clean commit, новый воспроизводимый signed candidate и fresh Ubuntu 24.04 install/reboot/update/rollback acceptance со сменой firewall schema.

**Оценка прогресса:** около `96%` программно-релизной готовности и около `87%` полной production-готовности. Процент снизился не из-за потери реализованных функций: принятый traffic-accounting инкремент расширил release scope и сделал предыдущий exact signed candidate устаревшим. Оставшиеся проценты непропорционально дороги: это не обычный код, а новый exact build, чистая установка, reboot и доказанный update/rollback. Production-оценка дополнительно включает реальный Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture и обязательный 24/72-часовой endurance, которые нельзя заменить disposable signing, privileged Docker/systemd, netns или cross-build.
...
| Firewall / routing | `SCHEMA_V2_KERNEL_NETNS_PASS / HOST_NOT_RUN` | Firewall schema `2` добавляет раздельные user/service counters; renderer/template/integrity markers синхронизированы. Dynamic TUN gate, protocol-186 routes, modem-scoped endpoint sets и nft guard прошли netns, включая восстановление owned table после delete/flush без direct route. Реальный host packet capture ещё не выполнен |
...
| SQLite | `PASS / V13_LOCAL_GATED` | Migrations v1–v13, checksum/version guard, durable settings, WAL/PRAGMAs и integrity/backup/update tests прошли Windows и native Linux. Migration v13 добавляет service traffic totals. Corrupt-main/partial-write/recoverable-WAL fixtures воспроизводимы побайтно; exact signed install migration остаётся частью следующего smoke |
...
| Signed update | `PREVIOUS_EXACT_PASS / SCHEMA_V2_ACCEPTANCE_PENDING` | Exact `.21 → .22` ранее прошёл stage/apply/finalize/rollback/power-cut recovery. Для schema `2` runtime теперь заменяет firewall+guard одной systemd transaction до и после pointer switch; реальный transient-unit probe выявил и устранил self-termination через `Requires=`. Новый exact signed update/rollback schema `1 ↔ 2` ещё не выполнен |
| Packaging | `PREVIOUS_CANDIDATE_SUPERSEDED / REBUILD_PENDING` | Exact `618d617` ранее прошёл double-build/fresh install/new PID 1, но не содержит текущий traffic/schema-v2 инкремент и больше не является выдаваемым кандидатом. Нужны новый clean commit, double-build и fresh systemd acceptance; permanent key, hardware LAN interface, push/tag/assets и публичная GitHub installation остаются отдельными рубежами |
...
| Traffic accounting | `WINDOWS_LINUX_KERNEL_PASS / HARDWARE_PENDING` | Option A реализован полностью: authoritative `user_upload/user_download/service_upload/service_download`, reset/epoch handling, session/daily/monthly totals, Mihomo cross-check, API/CSV/UI. Production Linux reader получил из real kernel nftables точные `12345/67890/111/222`; мобильный hardware budget/cross-check ещё `NOT_RUN` |
| Автоматические тесты | `CURRENT_WORKTREE_WINDOWS_LINUX_PASS / EXACT_REBUILD_PENDING` | Текущий незакоммиченный schema-v2 worktree прошёл полный Windows и native Linux `go test ./... -count=1`/`go vet ./...`, fixture byte-reproducibility, nft kernel/traffic reader, netns guard и systemd dependency probe. Новый exact signed tree ещё не собран; remote CI ждёт будущий push |

## Ближайший следующий инкремент

Закрыть новый traffic/schema-v2 инкремент как clean exact candidate: проверить итоговый diff, создать локальный commit, выполнить две независимые signed сборки, затем на fresh Ubuntu 24.04 доказать install/reboot и обязательный update/rollback со сменой firewall schema `1 ↔ 2`. Только после этого кандидат снова получает статус готового к первой установке на железо. Публичный push/tag/release, production signing identity и фактический LAN interface остаются внешними решениями и без разрешения пользователя не выполняются.

## Критический путь до release

Функциональный код traffic/schema-v2 **прошёл локальные Windows/Linux/kernel gates**, но новый install-ready артефакт ещё не собран. При отсутствии нового дефекта ориентир до кандидата для первой установки на железо — `4–8` часов активной работы: итоговый audit/commit, double-build и fresh install/reboot/update/rollback. Это диапазон с риском, а не обещание; обнаруженный acceptance-дефект возвращает цикл к исправлению и повторной проверке.
