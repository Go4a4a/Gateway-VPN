...
**Текущий этап:** exact candidate `0.1.0-traffic.8180072` уже прошёл reproducible signed build, fresh Ubuntu 24.04 install, строгую idempotency, новый PID 1, schema `1 → 2` signed update и автоматический schema `2 → 1` rollback/reboot. Acceptance обнаружил остаточный lifecycle defect finalize timer после terminal rollback и более общий pointer-update invariant. Текущий worktree исправляет terminal no-op/failed-unit cleanup, переводит `recovery` на новый release только после stability finalization и подписывает вычисляемый host lifecycle contract, чтобы update не оставлял несовместимые старые systemd units. Windows и offline Linux test/vet и systemd graph прошли; нужен новый clean exact candidate и сокращённый повтор fresh/update/rollback acceptance.

**Оценка прогресса:** около `97%` программно-релизной готовности и около `88%` полной production-готовности. Функциональный scope и schema-v2 transaction уже доказаны exact acceptance; оставшийся локальный процент — новый immutable build и повтор критического lifecycle сценария после найденного исправления. Production-оценка дополнительно включает реальный Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture и обязательный 24/72-часовой endurance, которые нельзя заменить disposable signing, privileged Docker/systemd, netns или cross-build.
...
| Firewall / routing | `SCHEMA_V2_EXACT_UPDATE_ROLLBACK_PASS / HOST_NOT_RUN` | Schema `2` с раздельными user/service counters прошла kernel/netns, fresh exact install и signed schema `1 → 2 → 1` update/rollback; guard восстановил owned table без direct route. Новый lifecycle-fix candidate ещё пересобирается; реальный host packet capture не выполнен |
...
| SQLite | `V13_EXACT_UPDATE_ROLLBACK_PASS` | Migrations v1–v13, checksum/version guard, durable settings, WAL/PRAGMAs и corruption fixtures прошли local gates. Exact signed update мигрировал `12 → 13`, а controlled health fault восстановил verified schema-12 snapshot; оба reboot состояния открылись успешно |
...
| Signed update | `SCHEMA_V2_APPLY_ROLLBACK_REBOOT_PASS / FIX_REBUILD_PENDING` | Exact schema `1 → 2` миграция и controlled health-failure rollback `2 → 1` прошли с correct firewall generation/current/recovery/DB. Acceptance обнаружил terminal finalize failed-state; worktree добавил successful no-op, post-recovery reset, finalized recovery advancement и signed host-contract equality. Нужен exact повтор исправления |
| Packaging | `8180072_EXACT_FRESH_REBOOT_PASS / FIX_REBUILD_PENDING` | `0.1.0-traffic.8180072` прошёл reproducible build, fresh install, idempotency и новый PID 1, но найденный lifecycle defect требует новой версии. Permanent key, фактический LAN interface, push/tag/assets и публичная GitHub installation остаются отдельными рубежами |
...
| Автоматические тесты | `CURRENT_WORKTREE_WINDOWS_LINUX_PASS / EXACT_REBUILD_PENDING` | Текущий lifecycle/host-contract worktree прошёл полный Windows и offline native Linux `go test ./... -count=1`/`go vet ./...`, actual host-contract CLI и Ubuntu 24.04 `systemd-analyze verify`. Новый exact signed tree ещё не собран; remote CI ждёт будущий push |

## Ближайший следующий инкремент

Закрыть найденный update lifecycle defect как новый clean exact candidate: итоговый diff/commit, две независимые signed сборки, fresh Ubuntu 24.04 install/reboot и повтор schema `1 ↔ 2` rollback/finalize с нулевым `systemctl --failed`, совпадающим host contract и переводом `recovery` на новый release только после finalization. После этого кандидат получает статус готового к первой установке на железо. Публичный push/tag/release, production signing identity и фактический LAN interface остаются внешними решениями и без разрешения пользователя не выполняются.

## Критический путь до release

Функциональный код и первый schema-v2 exact candidate прошли полный disposable systemd цикл; текущий lifecycle fix также прошёл Windows/Linux gates, но ещё не пересобран immutable artifact. При отсутствии нового дефекта ориентир до кандидата для первой установки на железо — `2–5` часов активной работы: commit, double-build и сокращённый fresh/install/reboot/update/rollback/finalize повтор. Это диапазон с риском, а не обещание; новый acceptance-дефект возвращает цикл к исправлению.
