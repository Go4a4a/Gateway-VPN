...
| Packaging | `KEY_LIFECYCLE_HARDENED / PUBLIC_RELEASE_PENDING` | `0.1.0-validation.47297a7` имеет reproducible validation bundle; все private-key CLI operations теперь Linux-only, private key повторно проходит custody checks при каждом load, pair явно проверяется до дорогой bundle build, а verified backup создаётся exclusive только в другом secure directory. Public tags/releases пока отсутствуют; permanent key, новая RC version, LAN interface, draft assets и GitHub installation остаются отдельными рубежами |
