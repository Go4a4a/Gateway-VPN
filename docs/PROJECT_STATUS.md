...
**Общее состояние:** `MAIN_SYNCED / FULL_CI_RUN_33899645155_PASS / PRODUCTION_SIGNED_TESTING_3BB466B_PUBLISHED_IMMUTABLE_PRERELEASE / NATIVE_WINDOWS_ARTIFACT_SMOKE_PASS / FRESH_WINDOWS_GATE_TARGETS_READY / CLEAN_WINDOWS_GUEST_LOCAL_VALIDATION_FAILED_SAFE / WRITABLE_SANDBOX_TEMPLATE_PREPARED / NEW_SIGNED_CANDIDATE_REQUIRED / HARDWARE_AND_ENDURANCE_GATES_PENDING`
**Текущий этап:** exact published candidate `3bb466b6a418298c46d13073edeee5cff505ab1c` был реально запущен пользователем в clean Windows Sandbox и безопасно остановился на `LOCAL_VALIDATION` с `DEPLOY_INPUT_INVALID`, не изменив ни Gateway, ни VPS. Введённый VPS endpoint был без имени пользователя (`172.18.224.1` вместо `USER@HOST`), а старый validator ошибочно запрещал один host даже при разных SSH-портах `42022/42023`, которые используются двумя изолированными target-контейнерами. Source fix `007b6fe` теперь проверяет полный `host+port`, требует `USER@HOST`, печатает понятные русские подсказки с примерами для каждого поля и использует project-local staging directory для Windows one-command вместо системного `%TEMP%`; release-gate harness также ждёт готовности `tcpdump` перед отправкой MSS SYN и использует bounded 10-секундное окно. Для следующего guest запуска подготовлен project-local writable Sandbox template: read-only inputs остаются в `C:\GatewayVPNGate`, а launcher стартует из отдельной writable `C:\GatewayVPNRuntime`, чтобы не пытаться писать в mapped read-only каталог. Новый GitHub CI run `33899645155` полностью зелёный, включая race suite и Linux nftables/netns gate; `origin/main` синхронизирован, оба target-а остаются чистыми. Для повторного clean-Windows gate нужен новый production-signed testing candidate и отдельное явное разрешение на key/tag/release; stable/latest не менять. После успешного гостевого gate остаются физические Ubuntu Gateway/VPS, HiLink/Keenetic и 24/72-часовые endurance gates.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `98%`; относительно текущего расширенного обязательного scope программная часть ориентировочно `98%`. Готовность к первой установке на физический стенд — примерно `97%`, полная production-готовность — примерно `84%`. Durable signed-update scheduler, service-route ladder, отдельный signed Mihomo maintenance discovery поверх полного immutable Gateway release, signed Gateway/VPS foundations, complete Gateway restore-point rollback, live Management Fabric observations, exact schema-34 lifecycle matrix и Windows portable delivery source имеют local evidence. Production-signed testing assembly, publication и independent public download verification завершены; отдельными внешними gates остаются clean Windows guest и hardware acceptance. Privileged Docker и local Windows 10 не заменяют физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Репозиторий | `PRODUCTION_SIGNED_TESTING_3BB466B_PUBLISHED_IMMUTABLE_PRERELEASE / FULL_CI_RUN_33863273880_PASS / PUBLIC_BASE_IMMUTABLE` | Exact testing tag `v0.1.1-testing.3bb466b` опубликован как immutable prerelease; fresh post-publication download `14/14` assets совпал с локальным bundle по имени и SHA-256. Public stable/latest `v0.1.0-successor.5723940` и Release `378316577` неизменны; publisher credential удалён |
...
| Packaging | `PRODUCTION_GVKEY_SIGNED_TESTING_3BB466B / PUBLISHED_IMMUTABLE_PRERELEASE / REMOTE_14_OF_14_SHA256_PASS / NATIVE_WINDOWS_TRUST_SMOKE_PASS / PUBLIC_5723940_IMMUTABLE` | Production signer `fceb4a54…eda60c` подписал exact `0.1.1-testing.3bb466b`: Gateway `57`, VPS `35`, channel `5` artifacts. GitHub Release опубликован как prerelease и immutable; fresh post-publication download `14/14` совпал по имени и SHA-256, channel signature и все пять role artifacts повторно проверены. Текущий Windows EXE принимает exact signed identity и отвергает wrong hash/signer, tampered signature/PE; stable release не изменён |
...
| Автоматические тесты | `FULL_LINUX_RACE_VET_SYNTAX_PASS / SUCCESSOR_FOCUSED_AND_SERIAL_TESTS_PASS / GITHUB_CI_33899645155_ALL_JOBS_PASS / UBUNTU24_SYSTEMD_PASS / WINDOWS_NATIVE_TRUST_AND_FRESH_TARGET_PREP_PASS / GATEWAY_RESTORE_POINT_REBOOT_GATE_PASS / EXACT_VPS_UPDATE_GATE_PASS` | Новый GitHub CI run `33899645155` для commit `007b6fe53cd92d43a1e89ccf34bcf40d80d3e0de` полностью прошёл: secret-history, полный Go/race/vet/fuzz/packaging/syntax, Windows portable contract и Linux nftables/netns включая стабилизированный MSS capture. После source fixes focused и полный serial `go test ./... -count=1 -p 1` проходят; старый published EXE trust-smoke и fresh two-target preparation прошли, clean guest фактически остановился безопасно на local validation и должен быть повторён новым signed candidate. Privileged Gateway gate подтверждает historical rollback, `SIGKILL` safety recovery и новый PID 1; VPS gate сохраняет success/finalize/`SIGKILL` rollback evidence |
...
| 21 | `PARTIAL_EXTERNAL` | Reproducible production-signed Gateway/VPS/bootstrap/Linux+Windows deploy artifacts `0.1.1-testing.3bb466b`, exact tag, hidden prerelease Draft, remote `14/14` SHA-256 check, SSH orchestrator rehearsals, exact remote CI и native Windows PE trust/tamper gates прошли. Публикация testing prerelease, clean Windows guest execution и real two-host `READY` отсутствуют |
...
| 45 | `PARTIAL_EXTERNAL` | Linux SSH orchestrator, Windows OpenSSH adapter и portable production-signed `.exe` `0.1.1-testing.3bb466b` прошли native version/commit, channel/signer/self-identity, wrong hash/signer и tampered signature/PE gates; generated one-command прошла PowerShell 5.1 AST/hash-before-exec/finally-cleanup. Guarded helper создал две fresh Ubuntu 24.04 systemd-роли на exact Default Switch ports, unique host keys, pinned key-only SSH и empty Gateway `lan0`; validated read-only `.wsb` готов. Полный deploy из clean Windows guest ещё не запущен |
