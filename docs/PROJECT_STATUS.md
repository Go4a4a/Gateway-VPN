...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V25 / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / THEMATIC_LOGGING_LOCAL_PASS / WATCHDOG_17_COMPONENT_LOCAL_PASS / WEBUI_UNINSTALL_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation, ручное capability-gated power management, интерактивный SSH/OpenSSH шаг, полный `wg-ingress`, тематические WebUI/SFTP-логи, доступные contextual help и 17-component watchdog contour реализованы и локально проверены. Exact source `c6328682541a220b33b7b40ca3df3fe6be30dd54` дважды offline воспроизводимо собрался как `0.1.0-successor.gc632868`; immutable public baseline schema `16` успешно обновился до exact schema/history `24`, controlled candidate failure полностью вернул baseline, а двойной SIGKILL upgrade+recovery с пустым `/run` и новым PID 1 автоматически повторил rollback. Schema 25 добавляет re-authenticated WebUI uninstall с preserve/purge, fixed root guardian, durable root-only tooling и terminal receipt; полный source/Bash/JavaScript/Linux cross-build/offline systemd gate проходит. Следующий gate — exact committed schema-25 candidate и Ubuntu 24 preserve/reinstall/purge/SIGKILL/new-PID1 matrix. Firmware/USB recovery, RTC wake-from-S5 и физические gates остаются впереди. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** текущая программная реализация оценивается примерно в `97%`, общая production-готовность — примерно в `79%`. WebUI uninstall orchestration реализована и проходит local gates; незавершённый программный объём сосредоточен в exact Ubuntu 24 uninstall/recovery matrix, clean SSH/WG OFF audit и исправлениях финального release/security review. Затем обязательны физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| WireGuard management | `LOCAL_RUNTIME_CONTOUR_PASS / EXACT_SIGNED_SYNTHETIC_HANDSHAKE_PASS / HOST_NOT_RUN` | Controller теперь идемпотентно создаёт обязательный `10.80.0.0/24 dev wg-mgmt protocol 186`; реальный Ubuntu `ip -N -json` parser принимает строгие numeric string protocol/table формы. Неизменённая signed `.13` ранее доказала endpoint/fwmark/address/handshake; новый schema-v25 contour ещё не прошёл exact signed/physical VPS gate |
...
| Self-health / watchdog | `V2_17_COMPONENT_LOCAL_PASS / EXACT_SIGNED_PREDECESSOR_PASS / HARDWARE_PENDING` | Schema 24 и policy v2 покрывают control/SQLite/firewall/network/DNS/SSH/Mihomo/WireGuard/routing/workers/convergence/backup/resources и новый `logging_pipeline`, per-component `MONITOR_ONLY/RECONCILE/RESTART`, durable budgets и external-outage suppression. Linux parser/netns/systemd/WebUI gates проходят; новый exact signed PID1/kill/hang gate ещё нужен |
...
| Safe network apply | `ETHERNET_LIFECYCLE_RUNTIME_LOCAL_PASS / REAL_NETWORKD_NOT_RUN` | LAN и Ethernet `CREATE/REPLACE_INTERFACE/UPDATE_ADDRESS/DELETE` используют один root broker, stable NIC ID, optimistic generation, persistent networkd snapshot/apply/rollback+reload, 60-секундное подтверждение и reboot recovery. Linux observer учитывает carrier, DHCP/static readiness, DNS и subnet conflicts, сохраняет configured state и синхронизирует authoritative routes. Реальное железо/networkd ещё не проверены |
| API / Web UI | `WG_INGRESS_LOG_TABS_CONTEXTUAL_HELP_BROWSER_PASS` | Отдельная вкладка WireGuard управляет server/managed/external peers, policies, probe, revoke/delete/rotation и re-authenticated config/QR. Журнал имеет десять тематических вкладок. Все формы сохраняют точные подсказки и получают доступную кнопку `?` для mouse/keyboard/touch плюс `aria-label`; browser preview не показал console errors |
...
| Uninstall | `WEBUI_DURABLE_GUARDIAN_LOCAL_PASS / EXACT_SCHEMA25_SYSTEMD_MATRIX_PENDING / HOST_NOT_RUN` | Password re-auth, CSRF, точная фраза, impact acknowledgements, preserve/purge, typed broker, exclusive fsync marker, signed root-only tooling, boot guardian и fsync terminal receipt реализованы. CLI/WebUI применяют `PATH_BLOCKED`, purge не создаёт скрытую DB-копию и packages остаются. Предыдущий CLI preserve/reinstall/purge systemd gate прошёл; exact schema-25 WebUI/SIGKILL/new-PID1 matrix ещё не запускался |
...
**Итог аудита:** `21 PASS_LOCAL`, `0 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: текущему расширенному successor ещё нужен exact signed rehearsal, после него остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

После фиксации текущего source commit создать только локальный disposable-signed exact successor и выполнить fresh Ubuntu 24.04 systemd rehearsal: SSH ON/OFF, optional `wg-ingress` ON/OFF, SFTP group/path/modes, logging rotation/watchdog recovery, schema `13 → 24`, update/rollback, strict idempotency и новый PID1/reboot recovery. Production `.gvkey` не открывать, tag/Release не создавать. После PASS передать install-ready candidate для первой установки на физический Gateway и реальный VPS.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, management interface, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP-логи и понятный выбор каждого install-компонента. Статус `INSTALL_READY` для нового exact commit будет выставлен только после повторного disposable-signed fresh systemd acceptance. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.
