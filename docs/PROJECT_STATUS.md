...
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact source `1b90ffcb99b25f79954cbc1b4bde7bcc0140175d` и disposable-signed `0.1.0-successor.1b90ffc` сохраняются как доказательство прежнего universal-installer scope, но больше не являются кандидатом текущего расширенного контракта. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation, ручное capability-gated power management, интерактивный SSH/OpenSSH шаг, полный `wg-ingress`, тематические WebUI/SFTP-логи, доступные contextual help и 17-component watchdog contour реализованы и локально проверены. Exact source `3c20f2ad2af8d28f1f803fe4a65e8e7f5b9910a4` дважды воспроизводимо собрался; fresh Ubuntu прошёл исправленный base readiness и создал `wg-ingress`, но post-bootstrap restart выявил systemd start-limit для идемпотентного `gateway-vpn-network-recovery.service`, который несколько последовательных dependents штатно запускают во время first install. Recovery после отказа уже завершился полностью. Unit получил явный unlimited start interval, focused/full source и Ubuntu systemd gates проходят; следующий шаг — новый exact commit/build и повтор fresh acceptance. Firmware/USB recovery, RTC wake-from-S5 и физические gates остаются впереди. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** прежний scope остаётся воспроизводимо завершённым примерно на `99%`, но это больше не процент текущего проекта. После закрытия локальных `wg-ingress`, тематических логов и contextual help текущая программная реализация оценивается примерно в `94%`, общая production-готовность — примерно в `76%`. Незавершённый программный объём сосредоточен в повторном exact signed fresh-install/update/rollback/reboot audit и исправлениях, которые он может выявить. Затем обязательны физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Packaging | `PRODUCTION_GVKEY_READY / REPRODUCIBLE_3C20F2A / PUBLIC_5723940_IMMUTABLE` | Две offline сборки exact `3c20f2a` совпали побайтно: Gateway `c3e23cda…`, VPS `4c2932cb…`, bootstrap `f334044a…`, deploy `b8dc8a3a…`, channel `644b570b…`; disposable signer `8231e4d3…`. Artifact дошёл до `wg-ingress`, но исключён из install-ready из-за recovery start-limit. Production key не открывался. Предыдущий public immutable Release по-прежнему имеет проверенные `10/10` assets |
