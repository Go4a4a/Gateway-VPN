...
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Windows 10/11 x64 portable deploy source, двухплатформенный signed channel, PowerShell hash-before-exec bootstrap и один persistent Win32 OpenSSH TCP на host прошли local/native gates; guarded helper подготовил две fresh Ubuntu SSH-роли, но clean Windows VM с полным deploy ещё не запускалась. Source `f9a2cb2730717243304e6523bbb8f5c2df1f2aec` дважды независимо и offline собрал candidate `0.1.0-successor.gf9a2cb2.crypto4` с Mihomo `v1.19.30`. Exact Gateway/VPS fresh systemd gates, same-version reinstall, Gateway Mihomo/TUN/API и preserve-uninstall прошли; release validator теперь действительно отвергает активный без LKG Mihomo и непустые fail-closed sets. Exact native Windows PE дополнительно подтвердил собственные version/commit, подписанный channel/signer и fail-safe rejection подменённых manifest hash, signer, signature и EXE. Этот candidate не опубликован. Следующий delivery gate — запуск уже проверенной hash-before-exec PowerShell-команды и настоящий two-target deploy из clean Windows guest либо, после отдельного разрешения production signing/publication, первая установка на физический стенд. Production tag/Release/push запрещены без отдельного разрешения пользователя, а bare-metal/hardware/endurance gates остаются обязательными.
...
| Репозиторий | `F9A2CB2_EXACT_BOTH_ROLES_AND_UNINSTALL_PASS / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Текущая candidate source identity — `f9a2cb2730717243304e6523bbb8f5c2df1f2aec`; после фиксации этого журнала ветка `main` будет на 34 commit впереди `origin/main`. Push/tag/Release не выполнялись |
...
| Packaging | `PRODUCTION_GVKEY_READY / F9A2CB2_REPRODUCIBLE_EXACT_LIFECYCLE_AND_WINDOWS_PE_PASS / PUBLIC_5723940_IMMUTABLE` | Candidate `gf9a2cb2.crypto4` дважды одинаково собран disposable signer-ом, повторно проверен по Gateway/VPS/channel signatures, прошёл fresh+reinstall Ubuntu 24 systemd gates обеих ролей и exact native Windows PE positive/negative smoke. Production key не открывался, production-signed artifact не собирался, public Release неизменён |
...
| 21 | `PARTIAL_EXTERNAL` | Reproducible signed Gateway/VPS/bootstrap/Linux+Windows deploy artifacts, SSH orchestrator rehearsals, exact remote CI и native Windows PE self-identity/signature/tamper gates прошли; production GitHub version/tag/assets и real two-host `READY` отсутствуют |
...
3. Docker Desktop privileged execution явно разрешён пользователем. Exact `gf9a2cb2.crypto4` прошёл fresh apply, idempotency обеих ролей, Gateway preserve-uninstall и финальный native Windows PE smoke; ранее exact lifecycle отдельно доказал new-PID1 и forced rollback. Docker в любом случае не заменяет реальный systemd host reboot, USB HiLink и двухмашинный VPS gate.
...
7. Disposable-signed candidate `0.1.0-successor.gf9a2cb2.crypto4` прошёл clean Ubuntu 24.04 Gateway/VPS dependency flow, apply, strict idempotency, tracked validator, exact Mihomo/TUN/API, preserve-uninstall и native Windows PE smoke. Предыдущие exact signed update/rollback/finalize, restore, kernel/netns, new-PID1 и forced-rollback gates также проходят. Это не заменяет bare-metal reboot/power cut, произвольный физический NIC topology или USB hotplug.
...
9. Schema 34 Gateway / schema 4 VPS Agent, обязательные role backups, topology safe apply, remote update/complete restore points, Management Resource health, `x/crypto v0.55.0` и privileged Gateway/VPS runtimes прошли current exact disposable lifecycle. Опубликованный production-signed release по-прежнему относится к прежнему scope; clean Windows VM и новый production successor не закрыты.
