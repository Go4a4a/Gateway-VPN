...
| Репозиторий | `EXACT_SCHEMA34_LOCAL_COMMIT / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Schema-34 source зафиксирован локальным commit `429810f63fae3e03a494bbe55cb1678bdba32fe7` / tree `53b956a8fe1a38c7668c96a851d447d4c16cab3f`; ветка `main` на 21 commit впереди `origin/main`, push/tag/Release не выполнялись |
...
| Packaging | `PRODUCTION_GVKEY_READY / EXACT_SCHEMA34_DISPOSABLE_LIFECYCLE_PASS / PUBLIC_5723940_IMMUTABLE` | Installer/uninstall/tmpfiles/watchdog включают fixed Gateway/VPS rollback и recovery units; exact disposable-signed schema-34 candidate прошёл Ubuntu 24 systemd lifecycle обеих ролей. Production key не открывался, новый production-signed artifact не собирался, public Release неизменён |
...
| 45 | `PARTIAL_EXTERNAL` | Linux SSH orchestrator, Windows OpenSSH adapter, portable signed `.exe`, hash-before-exec PowerShell bootstrap и native Windows source gates проходят; clean Windows VM с настоящими Gateway/VPS остаётся внешним gate |
