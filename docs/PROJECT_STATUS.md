...
| Этап 1: bootstrap | `UNIVERSAL_WIZARD_V3_SSH_WG_INGRESS_LOCAL_PASS / F9A2CB2_FRESH_REINSTALL_UNINSTALL_PASS / G2203D0B_FRESH_REINSTALL_UNINSTALL_PASS / PRODUCTION_RELEASE_PENDING / HARDWARE_PENDING` | Мастер единообразно объясняет выборы для каждого компонента, выбирает несколько LAN-портов, DHCP/CIDR/dependencies, default-on либо явный opt-out SSH/SFTP, optional входящий WireGuard, no-wait и GRUB, выводит exact mutation plan. Candidate `g2203d0b.crypto5` повторно прошёл signed Gateway/VPS fresh apply и same-version reinstall; Gateway preserve-uninstall восстановил SSH socket/service, LAN, sysctl/firewall и сохранил целую schema-34 DB. VPS dependency gate установил только недостающие пакеты без upgrades/removals |
...
| Packaging | `PRODUCTION_GVKEY_READY / G2203D0B_REPRODUCIBLE_EXACT_BUILD_WINDOWS_PE_AND_SYSTEMD_PASS / PUBLIC_5723940_IMMUTABLE` | Candidate `g2203d0b.crypto5` дважды одинаково собран disposable signer-ом, повторно проверен по Gateway/VPS/channel signatures, 110 dist files совпали побайтно, exact native Windows PE positive/negative smoke и fresh/reinstall Ubuntu 24 systemd lifecycle обеих ролей прошли. Gateway preserve-uninstall также закрыт. Production key не открывался, production-signed artifact не собирался, public Release неизменён |
...
**Read-only host discovery:** текущая ОС — Windows 10 Pro x64 `10.0.19045`. Hyper-V и `VirtualMachinePlatform` включены, `vmcompute`/`vmms` работают, Default Switch существует. Готовых Hyper-V VM нет; в пользовательских Downloads/Documents/Desktop не найдено локальных `.iso`, `.vhd`, `.vhdx`, `.wim` либо `.esd`. Windows Sandbox component `Containers-DisposableClientVM` выключен, `WindowsSandbox.exe` отсутствует.

**Граница:** включение Windows Sandbox изменит optional components текущей ОС и может потребовать restart, поэтому оно не выполнялось без отдельного согласования. Загрузка многогигабайтного Windows ISO и создание VM также не подменялись неявным действием. Production key, GitHub и candidate containers не затрагивались.

**Следующий шаг:** после явного разрешения включить Windows Sandbox без автоматической перезагрузки, сообщить фактический `RestartNeeded`, а после доступного reboot подготовить ephemeral `.wsb` и выполнить полный signed Windows two-target deploy. Альтернатива — предоставленная пользователем clean Windows 10/11 VM.
