...
| Packaging | `PRODUCTION_GVKEY_READY / EF7712F_REPRODUCIBLE_EXACT_LIFECYCLE_PASS / PUBLIC_5723940_IMMUTABLE` | Candidate `gef7712f.crypto2` дважды одинаково собран disposable signer-ом, повторно проверен по Gateway/VPS/channel signatures и прошёл fresh+reinstall Ubuntu 24 systemd gates обеих ролей. Production key не открывался, production-signed artifact не собирался, public Release неизменён |
...
3. Docker Desktop privileged execution явно разрешён пользователем. Exact `gef7712f.crypto2` прошёл fresh apply и idempotency обеих ролей; ранее exact lifecycle отдельно доказал new-PID1 и forced rollback. Docker в любом случае не заменяет реальный systemd host reboot, USB HiLink и двухмашинный VPS gate.
...
9. Schema 34 Gateway / schema 4 VPS Agent, обязательные role backups, topology safe apply, remote update/complete restore points, Management Resource health и privileged Gateway/VPS runtimes прошли current exact disposable lifecycle. `x/crypto v0.55.0` прошёл source/security gates, но новый exact post-update lifecycle ещё не выполнен. Опубликованный production-signed release по-прежнему относится к прежнему scope; clean Windows VM и новый production successor не закрыты.
