...
**Текущий этап:** Четыре unique exact signed candidate прошли build/verify и каждый небезопасный DHCP apply штатно откатился. Candidate `b8d192f` подтвердил исправленный TCP/UDP port-53 gate, но реальный systemd dnsmasq всё ещё не мог создать lease внутри общего application state tree. Отдельный transient test доказал безопасную модель: systemd-owned `/var/lib/gateway-vpn-dnsmasq` mode `0700`, без записи или перечисления service UID в `/var/lib/gateway-vpn`. Модель реализована во всех config/safe-apply/systemd/install/recovery/uninstall boundaries; полный Linux test/vet, shell syntax и Ubuntu 24.04 `systemd-analyze verify` PASS. Нужны commit, пятая unique signed apply/reboot и затем реализация принятого PLAN §9.8 self-health supervisor до 24-часового endurance
...
| Packaging | `SIGNED_REHEARSAL_REGRESSION_FIX / PRODUCTION_RELEASE_PENDING` | Signed `.13` double-build/one-command orchestration и handshake прошли; disposable `.21/.22` доказали systemd update/recovery, `.27` — destructive restore/recovery. Текущий DHCP installer regression исправлен и прошёл static/systemd gates, но новый exact signed apply ещё требуется. Permanent key, production tag/assets и установка с публичного GitHub release ещё не выполнены |
...
**Следующий шаг:** зафиксировать clean commit, собрать не переиспользующий старый `dist/` пятый exact signed candidate и выполнить fresh Ubuntu 24.04 dry-run/apply, dnsmasq UID/GID/mode/lease, HTTPS/broker/firewall/PATH_BLOCKED, idempotent reinstall и новый PID 1/reboot. Только после installer PASS начинается реализация self-health supervisor PLAN §9.8.
