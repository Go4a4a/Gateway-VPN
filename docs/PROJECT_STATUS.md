...
**Текущий этап:** Пятый unique exact signed candidate `89c6a8d` впервые прошёл fresh Ubuntu 24.04 DHCP apply: dnsmasq active под dedicated UID/GID, отдельный state root `0700`, DNS/DHCP/HTTPS sockets, broker/control/firewall и `PATH_BLOCKED` проверены, restart counters равны нулю. Обязательная повторная команда обнаружила два idempotency-дефекта: собственный LAN DNS listener считался fresh-host конфликтом, а audit ожидал lease `0600` вместо фактического безопасного `0644` внутри каталога `0700`. Исправление оставляет port-53 gate только в доказанной fresh-install ветке и усиливает existing audit точным generated config; полный Linux test/vet и shell syntax PASS. Нужны commit, шестая unique signed idempotency/reboot acceptance и затем реализация PLAN §9.8 self-health supervisor
...
| Packaging | `SIGNED_DHCP_APPLY_PASS / IDEMPOTENCY_RETEST_PENDING` | Signed `.13` double-build/one-command orchestration и handshake прошли; disposable `.21/.22` доказали systemd update/recovery, `.27` — destructive restore/recovery. Exact `89c6a8d` впервые прошёл fresh DHCP apply и runtime readiness; найденный только повторной командой audit defect исправлен локально, но требует новой signed idempotency/reboot acceptance. Permanent key, production tag/assets и установка с публичного GitHub release ещё не выполнены |
