...
**Текущий этап:** локальный install-ready candidate `0.1.0-endurance.618d617` из exact commit `618d6177812727afc6289d5c0ec8d295bc8368bc` прошёл double-build reproducibility, fresh Ubuntu 24.04 dry-run/apply, strict idempotent reinstall, несколько quiet reconcile/watchdog cycles и новый PID 1 boot/readiness. Следующий рубеж — выбрать фактический LAN interface физического Gateway, создать/защитить production signing identity либо явно принять test identity, затем с разрешения пользователя push и immutable GitHub release; после этого начинается установка и hardware/VPS/HiLink/Keenetic проверка.

**Оценка прогресса:** около `99%` программной реализации и около `89%` полной production-готовности. Локальный exact signed install/reboot integration закрыт; оставшийся программно-релизный процент — permanent signing/public GitHub distribution и возможные исправления только по фактическому hardware gate. Production-оценка дополнительно включает реальный Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture и обязательный 24/72-часовой endurance, которые нельзя заменить disposable signing, privileged Docker/systemd, netns или cross-build.
...
| Packaging | `LOCAL_INSTALL_READY / GITHUB_RELEASE_PENDING` | Exact `618d617` дважды побайтно собран (`76/76` files), все Gateway/VPS/bootstrap/deploy/channel signatures перепроверены; fresh DHCP apply, idempotency и new PID 1 PASS. Bundle использует disposable acceptance signer и test `lan0`; permanent backed-up key, фактический hardware LAN interface, push/tag/assets и публичная GitHub installation ещё не выполнены |
...
| Автоматические тесты | `WINDOWS_LINUX_EXACT_PASS / REMOTE_CI_PENDING` | Commit `618d617` прошёл полный Windows/native Linux `go test ./... -count=1`/`go vet ./...`; systemd lifecycle parent `b2ea2fe` прошёл Ubuntu graph/fresh install/new PID 1, а exact child дополнительно прошёл double-build, fresh apply/idempotency/quiet cycles/new PID 1. Remote CI ждёт push |

## Ближайший следующий инкремент

Локальная разработка до install-ready кандидата завершена. Следующий инкремент требует внешних решений: фактическое имя выделенного LAN interface физического Gateway; production signing key с отдельной резервной копией (либо явное согласие на disposable test release); разрешение push commits/tag/assets и создание/публикацию immutable GitHub release. После проверки публичной one-command начинается первая установка. Реальные USB/HiLink/Keenetic/VPS packet capture и 24/72-часовые endurance gates выполняются после неё; до их прохождения production status остаётся `NOT_DONE`.

## Критический путь до release

Код и локальный signed candidate **готовы к подготовке первой установки на железо**. После получения LAN interface, решения по production key и разрешения внешних GitHub действий публикация/проверка one-command ориентировочно занимает `1–2` часа активной работы без найденного distribution defect. После установки отдельный путь до production release включает реальные Gateway/VPS/HiLink/Keenetic проверки, найденные исправления, 24-часовой developer и несокращаемый 72-часовой release endurance.
