...
**Общее состояние:** `UNIFIED_ACCESS_API_UI_REMOTE_CI_PASS / HARDWARE_VALIDATION_PENDING`
**Текущий этап:** единый API/OpenAPI/WebUI способов доступа и durable operation panel опубликованы в `22709f9`. GitHub Actions run `33114255191` завершён `success`: `Go, packaging and syntax gates` и `Linux nftables fail-closed gate` зелёные. Direct и subscriptions управляются одним ordered list; startup gate, service-refresh permission, hysteresis и boot-scoped direct-only имеют authenticated API. Manual refresh немедленно возвращает operation ID, параллельный запрос присоединяется к тому же durable lease, bounded workers не используют request context после ответа, а незавершённая previous-process operation получает terminal `PROCESS_RESTART`.
...
| Репозиторий | `PUBLIC_MAIN_DE5F7ED / REMOTE_CI_PASS` | Boot-scoped startup и retention fix опубликованы в `Go4a4a/Gateway-VPN`; exact GitHub Actions run `33109704378` завершён `success`. Tag/release не создавались |
...
| Unified access methods | `API_UI_OPERATIONS_REMOTE_PASS / HARDWARE_PENDING` | Direct + subscriptions имеют один authenticated ordered list, enable/reorder, startup/service/hysteresis policy и boot-scoped direct-only control. Selector/actuator и exact boot recovery не ослаблены; direct-per-modem status ещё нужно включить в общий UI read model |
...
| Автоматические тесты | `REMOTE_CI_22709F9_PASS` | Exact commit `22709f9dd38b85ab516c7ec2920774ae04e65861` прошёл GitHub Actions run `33114255191`: Go/race/packaging/syntax job — 2m27s, Linux nftables/netns job — 59s, весь run — 3m33s; обе обязательные jobs завершены `success` |
...
6. Публичный remote и GitHub CI работают; GitHub release immutability включена. Exact run `33109704378` для `de5f7ed` завершён `success` после исправления endurance fixture regression предыдущего run. CI не получает release secrets. Permanent encrypted production key и byte-identical backup готовы, но tag/release не создавались.
