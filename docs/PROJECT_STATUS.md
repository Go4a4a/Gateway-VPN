...
**Текущий этап:** exact signed `aa15477` отклонён обязательным fresh Ubuntu 24.04 apply: найден race между update/network recovery, watchdog и control, а journal дополнительно доказал rejection Go-thread notification при `NotifyAccess=main`. Исправленный systemd lifecycle, packaging regression tests и эксплуатационная документация находятся в текущем рабочем дереве; полный Windows/Linux test/vet и Ubuntu 24.04 unit-graph verify прошли. Следующий рубеж — clean commit, новый exact signed candidate и повторный fresh install/idempotency/new-PID1 smoke перед передачей команды установки на физический Gateway.

**Оценка прогресса:** около `98%` программной реализации и около `88%` полной production-готовности. Оставшиеся программные проценты — exact signed install/reboot integration и возможные исправления по её результату, а не новые подсистемы. Production-оценка дополнительно включает permanent signing/public GitHub release, реальный Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture и обязательный 24/72-часовой endurance, которые нельзя заменить disposable signing, privileged Docker/systemd, netns или cross-build.
...
| Self-health / watchdog | `FIX_GATED / NEW_EXACT_SIGNED_PENDING` | Migration v12 и supervisor прошли code/synthetic gates. Exact `aa15477` выявил и доказал recovery-start race, ephemeral runtime-directory hazard и rejected Go-thread systemd notification; fix убирает self-start из update recovery, связывает control lifecycle с watchdog, сохраняет runtime dir через restart и использует cgroup-scoped `NotifyAccess=all`. Windows/Linux tests и Ubuntu unit graph PASS; новый exact PID 1 smoke обязателен |
| SQLite | `PASS / V12_LOCAL_GATED` | Migrations v1–v12, checksum/version guard, durable settings, WAL/PRAGMAs и integrity/backup/update tests прошли Windows и native Linux. Migration v12 добавляет safe-default watchdog policy; exact signed install migration остаётся частью следующего smoke |
...
| Packaging | `PREVIOUS_EXACT_PASS / WATCHDOG_CANDIDATE_REJECTED` | Signed `.13` double-build/one-command orchestration и handshake прошли; disposable `.21/.22` доказали systemd update/recovery, `.27` — destructive restore/recovery, `d471c12` — DHCP install/idempotency/new PID 1. `aa15477` byte-reproducible build прошёл, но fresh apply штатно откатился из-за нового watchdog lifecycle race; исправленный exact candidate ещё не собран. Permanent key/tag/assets и публичная GitHub installation не выполнены |
...
| Автоматические тесты | `WINDOWS_AND_LINUX_PASS / NEW_COMMIT_REMOTE_PENDING` | Исправленный worktree прошёл полный Windows и native Linux/amd64 `go test ./... -count=1`/`go vet ./...`, focused packaging regression и Ubuntu 24.04 `systemd-analyze verify`; предыдущие JS/shell/netns gates остаются действующими для неизменённых частей. Новый clean commit, exact signed smoke и его remote CI ещё не выполнены |

## Ближайший следующий инкремент

Следующий локальный инкремент: зафиксировать исправление exact-install race, собрать новый VCS-stamped exact signed candidate и повторить clean Ubuntu 24.04 archive dry-run, fresh apply, strict idempotent reinstall и новый PID 1/reboot smoke. Обязательно проверить fresh watchdog/control files и notification acceptance, ownership, отсутствие restart/reboot loop, default-off reboot, `PATH_BLOCKED`, HTTPS/DNS/DHCP и installer readiness. После PASS кандидат и однострочная команда передаются пользователю для первой установки на физический Gateway. Реальные USB/HiLink/Keenetic/VPS packet capture и 24/72-часовые endurance gates начинаются после пользовательской установки; до их прохождения production status остаётся `NOT_DONE`.

## Критический путь до release

Критический путь до **готовности к первой установке на железо**: clean fix commit → exact signed build/verify → clean-install/idempotency/reboot smoke → команда установки. После локализации `aa15477` failure оценка на 2026-08-26 — `2–5` часов активной работы при чистом повторе, с резервом до одного рабочего дня, если новый signed PID 1 gate обнаружит ещё один defect. После установки отдельный путь до production release включает реальные Gateway/VPS/HiLink/Keenetic проверки, найденные исправления, 24-часовой developer и несокращаемый 72-часовой release endurance.

Если целевые Linux Gateway, VPS и модемы доступны без пауз, ориентир до проверенного production release остаётся `4–8 дней`: менее дня на signed integration candidate, затем реальные hardware/VPS проверки, исправления и минимум `72 часа` release endurance. Это не обещанная дата: routing/hardware defects либо новое расширение scope сдвигают срок. Без фактического доступа к Linux/VPS/оборудованию можно передать install-ready candidate, но нельзя честно поставить production status `DONE`.
...
**Следующий шаг:** clean commit исправления, новый unique versioned double-build, затем fresh Ubuntu 24.04 dry-run/apply/idempotency/new-PID1 и watchdog notification/restart checks. `aa15477` не устанавливать.
