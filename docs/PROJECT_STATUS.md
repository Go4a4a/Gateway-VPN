...
| Репозиторий | `VPS_SECURITY_HEADER_FIX_SOURCE_READY / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Mihomo maintenance source identity `751669c36fcb2574a859a1a6ffe1998f679b78b6` сохраняется как exact finding evidence, но не продвигается; после локальной фиксации header fix ветка `main` будет на 26 commit впереди `origin/main`. Push/tag/Release не выполнялись |
...
| Packaging | `PRODUCTION_GVKEY_READY / G751669C_GATEWAY_PASS_VPS_HEADER_FAIL / SUCCESSOR_REBUILD_REQUIRED / PUBLIC_5723940_IMMUTABLE` | Candidate `g751669c.crypto1` прошёл exact Gateway fresh/reinstall/new-PID1 и VPS install/reinstall/uninstall, но строгий VPS HTTPS gate нашёл отсутствующий `X-Frame-Options`; исправленный successor ещё не собран. Production key не открывался, новый production-signed artifact не собирался, public Release неизменён |
