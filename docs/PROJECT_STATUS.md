...
| Репозиторий | `PRODUCTION_SIGNED_TESTING_DRAFT_VERIFIED / FULL_CI_RUN_33636614345_PASS / PUBLIC_BASE_IMMUTABLE` | Exact testing tag `v0.1.1-testing.645500a` и скрытый prerelease draft созданы; `14/14` remote assets совпадают с локальными. Draft не опубликован. Public stable/latest `v0.1.0-successor.5723940` и Release `378316577` неизменны |
...
| Packaging | `PRODUCTION_GVKEY_SIGNED_TESTING_BUNDLE / REMOTE_14_OF_14_SHA256_PASS / PUBLIC_5723940_IMMUTABLE` | Production signer `fceb4a54…eda60c` подписал exact `0.1.1-testing.645500a`: Gateway `57`, VPS `35`, channel `5` artifacts. GitHub draft остаётся hidden prerelease; remote download `14/14` совпал по SHA-256. Stable release не изменён; физический lifecycle этого exact artifact ещё не выполнен |
