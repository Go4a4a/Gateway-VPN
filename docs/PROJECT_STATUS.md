...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V28 / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_GATEWAY_PRIVILEGED_NETNS_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V3_MANAGED_ADMIN_CONFIG_LOCAL_PASS / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / GATEWAY_VPS_AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_GATEWAY_PACKET_PASS / GROUPED_RESPONSIVE_WEBUI_BROWSER_PASS / FIREWALL_SCHEMA_V5 / WG_INGRESS_LOCAL_PASS / WATCHDOG_19_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source Gateway schema 28 реализует durable Management Fabric foundation и simultaneous `gvm<N>` runtime; новый privileged Ubuntu netns gate доказал два одновременных handshake, marked endpoint routing, prefix-DNAT/ACL/return path, selective link removal и сохранность foreign Docker/UFW/Amnezia-like objects. Root-owned management keys теперь принимаются только при exact `root:root 0700/0600`, а зашифрованный Gateway backup собирается root broker без выдачи plaintext WebUI-процессу. VPS Agent schema 3 сохраняет pairing/Gateway/resources/ACL control plane, managed administrator lifecycle и проверенный privileged Hub runtime. Gateway/VPS WebUI сгруппированы и прошли responsive browser gate. В расширенном scope ещё отсутствуют end-to-end `wg-admin` relay, полный Hub logs/update/diagnostics и единая post-install transaction переключения всех topology profiles. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `92%`; общая production-готовность расширенного проекта — ориентировочно `74%`. Gateway/VPS privileged Management Fabric, local-resource packet path, root/encrypted backup boundary, grouped responsive WebUI и обе обязательные backup-роли теперь имеют локальное evidence. End-to-end inner `wg-admin` relay, topology-profile transaction и оставшиеся Hub lifecycle-функции ещё впереди. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Firewall / routing | `SCHEMA_V5_WG_INGRESS_AND_MANAGEMENT_FABRIC_NETNS_PASS / HARDWARE_PENDING` | Schema `5` сохраняет counters, TUN/direct gates и listener allowlist, а также добавляет owned Management Fabric sets/chains. Privileged netns подтвердил startup/flush recovery, multi-LAN SSH, `wg-ingress` и два `gvm<N>` с DNAT/ACL/return path; физический capture остаётся обязательным |
...
| Management Fabric / VPS Hub | `BOTH_ROLES_PRIVILEGED_APPLY_AND_COEXISTENCE_PASS / LIFECYCLE_UI_PARTIAL` | Обе роли имеют parameter-free root apply, receipt/recovery и owned nft/routes/WireGuard projection. VPS gate подтвердил four-peer ACL; Gateway gate — two-link ACL/DNAT/selective removal. На обеих сторонах foreign UFW/Docker/Amnezia objects сохраняются. Hub logs/update/diagnostics ещё отсутствуют |
| Удалённые локальные ресурсы | `GATEWAY_PREFIX_DNAT_ACL_RETURN_PATH_NETNS_PASS / E2E_ADMIN_RELAY_PENDING` | Typed publications/ACL применяются на VPS и Gateway. Kernel gate доказал разрешённый TCP через два VPS alias prefix, запрет cross-link/неразрешённого порта и masqueraded return path к локальному `/24`; inner `wg-admin` end-to-end relay ещё не реализован |
...
| Self-health / watchdog | `GATEWAY_19_LOCAL_PASS / VPS_FABRIC_ROOT_WATCHDOG_PASS / MANAGEMENT_FABRIC_KERNEL_PROJECTION_PASS` | Gateway имеет 19 fixed components, включая `management_fabric_routes` и `wireguard_admin`; supervisor tests проходят. VPS root watchdog сверяет desired/receipt/runtime. Gateway projection фактически проверена в kernel, но hardware recovery и будущий inner `wg-admin` компонент остаются внешним/следующим gate |
| SQLite | `GATEWAY_V28_AND_VPS_AGENT_V3_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Gateway migrations 26–28 хранят Management Fabric, applied generations и watchdog settings/state. Contiguous checksum-verified VPS Agent schema 3 добавила config lifecycle/rotation source к pairing/Gateway/admin/resource/ACL/health/generations; v1→current migration, integrity/FK/lifecycle и backup tests проходят. Exact signed host evidence пока относится к Gateway schema 25 |
...
| API / Web UI | `GROUPED_GATEWAY_AND_VPS_LOCAL_PASS / GATEWAY_ALL_TABS_320PX_BROWSER_PASS` | Gateway и VPS Hub используют предметно сгруппированную навигацию. Длинные русские подписи растят controls, карточки/forms не расширяют viewport, а широкие таблицы прокручиваются внутри контейнера. Gateway browser gate прошёл все вкладки при 320×720 и обычной desktop-ширине: table-action buttons сохраняют читаемую intrinsic width вместо 3–8 строк/вертикального текста, чрезмерно узкие controls и console warn/error отсутствуют; Hub Logs/Diagnostics как функции ещё не реализованы |
...
| Автоматические тесты | `GATEWAY_AND_VPS_PRIVILEGED_FABRIC_3X_PASS / FULL_GO_VET_BUILD_PASS / SCHEMA25_EXACT_HOST_EVIDENCE` | Полные Windows и offline Linux test/vet/build, Node syntax, format/diff проходят. VPS privileged suite трижды подтвердил nft parser/coexistence, два Gateway + два administrator handshakes/ACL, actual rollback и atomic watchdog status; Gateway privileged suite трижды подтвердил два `gvm<N>`, packet ACL/DNAT/return path и selective removal без пересоздания второго link. Exact signed host artifact всё ещё относится к schema 25 |
...
| 41 | `PASS_LOCAL` | Gateway и VPS Hub имеют сгруппированные предметные разделы, отдельный Management Fabric contour, thematic logs и accessible contextual help. Responsive browser gate на 390×844 подтвердил читаемую горизонтальную mobile navigation, отсутствие page overflow и необрезанные controls; новые функции обязаны добавляться в существующую information architecture, а не возвращать плоское меню |
...
**Итог аудита:** `26 PASS_LOCAL`, `6 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: обе backup-роли, information architecture и privileged Gateway/VPS Management Fabric закрыты локально; end-to-end administrator relay, единая topology transaction, полный VPS Hub lifecycle и Windows delivery требуют реализации/evidence. Затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Следующий инкремент — end-to-end `wg-admin` relay, затем единая post-install topology transaction и оставшиеся Hub logs/update/diagnostics. До exact release повторить privileged Gateway↔VPS systemd/browser/coexistence gates. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP-логи и privileged many-to-many Management Fabric с local-resource packet path. Inner `wg-admin` relay, единая post-install смена topology и полный Hub lifecycle ещё не завершены, поэтому exact `a7a783b` нельзя объявлять окончательным `INSTALL_READY` для всего расширенного проекта, хотя schema-25 recovery gate закрыт. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.

После завершения оставшихся Management Fabric функций отдельный путь до production release включает реальные multi-Gateway/multi-VPS/Keenetic/HiLink проверки, найденные исправления, 24-часовой developer и несокращаемый 72-часовой release endurance. Прежний ориентир `4–8 дней` относился к старому single-VPS scope и больше не используется как обещание. Schema 28 и первый двух-VPS Gateway netns gate уже закрыты; без фактического доступа к Linux/VPS/оборудованию можно передать install-ready candidate, но нельзя честно поставить production status `DONE`.
...
9. Schema 28 Management Fabric, обязательные role backups и privileged Gateway/VPS runtimes существуют в current source, но опубликованные/exact signed artifacts относятся к прежнему schema-25 scope. Gateway/VPS multi-peer ACL/coexistence прошли privileged Docker; inner admin-relay, полный Hub lifecycle и новый exact signed install gate ещё не закрыты.
