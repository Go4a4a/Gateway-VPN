...
**Текущий этап:** exact code candidate `0.1.0-traffic.3c13b09` из clean commit `3c13b093718e9e0ac2b38da9617baf05039ba732` завершил локальный release gate. Две независимые signed сборки побайтно совпали; fresh Ubuntu 24.04 install/idempotency, новый PID 1, format-2 schema `1 → 2` update, автоматический rollback `2 → 1`, terminal timer no-op, successful finalization с переводом `recovery` на новый release и повторный finalized PID 1 прошли без failed units. Более новый main содержит только журнал и исправленный hardware runbook. Публичный hardware release должен получить новую immutable version из актуального clean commit и production/hardware-test signer, а не подменять уже проверенную локальную version другим artifact.
...
| Этап 1: bootstrap | `3C13B09_DOCKER_SYSTEMD_PASS / HOST_NOT_RUN` | Exact format-2 Ubuntu 24.04 installer прошёл clean dry-run/apply/idempotency, persistent `lan0`, HTTPS bind, DB/config ownership, recovery markers и два независимых fresh-systemd boot; реальный bare-metal/VM host ещё не проверен |
...
| Packaging | `3C13B09_CODE_CANDIDATE_PASS / PUBLISH_SUCCESSOR_PENDING` | `0.1.0-traffic.3c13b09` имеет reproducible 76-file bundle, verified Ed25519 tree, SBOM/provenance/channel consistency, fresh install/idempotency и два новых PID 1. README/hardware runbook исправлены позже, поэтому public release строится новой version из актуального commit; permanent key, LAN interface, push/tag/assets и GitHub installation остаются отдельными рубежами |
...
| Автоматические тесты | `3C13B09_WINDOWS_LINUX_EXACT_PASS / REMOTE_CI_PENDING` | Exact commit прошёл полный Windows и offline native Linux `go test ./... -count=1`/`go vet ./...`, четыре Linux/amd64 builds, host-contract CLI, signed-tree verification и Ubuntu 24.04 `systemd-analyze verify`. Remote CI ждёт будущий push |

## Ближайший следующий инкремент

Следующий инкремент — внешний release/hardware gate. После явного разрешения пользователя: push локальных commits, remote CI, создание отдельной production/hardware-test signing identity и новой immutable publish version из актуального clean commit. Затем — установка одной командой на физический Ubuntu 24.04 Gateway и реальный VPS, фиксация фактического LAN interface, HiLink/Keenetic/WireGuard packet captures и исправление только реально найденных отклонений. Version `0.1.0-traffic.3c13b09` другим signed artifact не заменяется; push/tag/release и работа с долгоживущим key без отдельного разрешения не выполняются.
...
6. Публичный remote и GitHub CI работают; GitHub release immutability включена. Exact disposable signed `0.1.0-traffic.3c13b09` прошёл local release gate, но отдельный backed-up production key и реальный tag/release ещё не подготовлены; CI не получает release secrets, а 40 локальных commits не отправлены без разрешения пользователя.
7. Gateway installer/systemd/networkd/nftables/sysctl/HTTPS, exact signed update/rollback/finalize `3c13b09`, restore и uninstall прошли privileged Ubuntu 24.04 Docker acceptance, включая fresh rootfs, новые PID 1 и durable recovery. Это не заменяет bare-metal reboot/power cut, APT dependency installation на произвольной машине или USB hotplug.
