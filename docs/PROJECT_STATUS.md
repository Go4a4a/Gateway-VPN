...
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source Gateway schema 30 и VPS Agent schema 4 дополнительно получили тематические Hub-журналы и bounded diagnostic ZIP. Fixed root collector публикует только sanitized allowlisted snapshot `root:gateway-vpn-vps 0640`; privileged Ubuntu gate доказал чтение Agent-ом и недоступность sibling restore/fabric state. Desktop 1280×720 и mobile 320×720 WebUI прошли без горизонтального переполнения или ошибок консоли. Следующий крупный блок — подписанная VPS update transaction с crash-safe rollback, затем Windows deploy delivery и exact release audit. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `96%`; общая production-готовность расширенного проекта — ориентировочно `78%`. Gateway/VPS privileged Management Fabric, local-resource packet path, root/encrypted backup boundary, обе backup-роли, end-to-end relay, единая topology-profile transaction и Hub logs/diagnostics теперь имеют локальное evidence. В программном scope остаются signed VPS update lifecycle, Windows deploy delivery и финальная exact release/security сборка. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Management Fabric / VPS Hub | `BOTH_ROLES_PRIVILEGED_APPLY_AND_COEXISTENCE_PASS / E2E_RELAY_PACKET_3X_PASS / OPERATIONS_DIAGNOSTICS_LOCAL_PASS` | Обе роли имеют parameter-free root apply, receipt/recovery и owned nft/routes/WireGuard projection. VPS gate подтвердил four-peer ACL; Gateway gate — two-link ACL/DNAT/selective removal. Fixed root operations timer публикует bounded sanitized snapshot для непривилегированного Agent; privileged gate доказал `root:gateway-vpn-vps 0640` и запрет sibling privileged state. Signed VPS update ещё не реализован |
...
| API / Web UI | `GROUPED_GATEWAY_AND_VPS_LOCAL_PASS / TOPOLOGY_AND_VPS_OPERATIONS_DESKTOP_320PX_BROWSER_PASS` | Обе роли прошли предметные вкладки при 320×720. VPS Hub теперь имеет отдельные «Журналы» и «Диагностика»: категории, поиск, bounded limit, локальную прокрутку, очистку только окна и скачивание ZIP. Browser gate на 1280×720 и 320×720 подтвердил отсутствие page overflow/clipped buttons и пустую console |
...
| Автоматические тесты | `TOPOLOGY_AND_ONE_ARM_PRIVILEGED_PASS / GATEWAY_AND_VPS_FABRIC_3X_PASS / E2E_RELAY_PRIVILEGED_3X_PASS / VPS_OPERATIONS_BOUNDARY_PASS / FULL_GO_TEST_PASS / SCHEMA25_EXACT_HOST_EVIDENCE` | Полный `go test ./... -count=1 -p 1`, `go vet ./...`, Node/shell syntax и diff gates проходят. Новый Ubuntu 24 gate доказал root-owned VPS snapshot, чтение Agent-ом и недоступность sibling privileged state; повторный systemd/GRUB gate зелёный. Exact signed host artifact всё ещё относится к schema 25 |
...
| 43 | `IN_PROGRESS_LOCAL` | Lightweight Go Agent, separate schema-v4 SQLite/auth, restricted Hub, pairing/Gateway/external+managed/E2E-admin/resource/ACL/relay APIs, grouped pages, one-use config/rotation, backup, owned host apply/watchdog, operational logs и diagnostics реализованы. Signed update transaction и portable Windows delivery ещё не завершены |
