...
**Общее состояние:** `MAIN_SYNCED / FULL_CI_RUN_33904761787_PASS / PRECOMMIT_SECRET_GUARD_PASS / PRODUCTION_SIGNED_TESTING_3BB466B_PUBLISHED_IMMUTABLE_PRERELEASE / NATIVE_WINDOWS_ARTIFACT_SMOKE_PASS / FRESH_WINDOWS_GATE_TARGETS_READY / CLEAN_WINDOWS_GUEST_LOCAL_VALIDATION_FAILED_SAFE / WRITABLE_SANDBOX_TEMPLATE_PREPARED / NEW_SIGNED_CANDIDATE_REQUIRED / HARDWARE_AND_ENDURANCE_GATES_PENDING`
**Текущий этап:** exact published candidate `3bb466b6a418298c46d13073edeee5cff505ab1c` был реально запущен пользователем в clean Windows Sandbox и безопасно остановился на `LOCAL_VALIDATION` с `DEPLOY_INPUT_INVALID`, не изменив ни Gateway, ни VPS. Введённый VPS endpoint был без имени пользователя (`172.18.224.1` вместо `USER@HOST`), а старый validator ошибочно запрещал один host даже при разных SSH-портах `42022/42023`, которые используются двумя изолированными target-контейнерами. Current source `48433fc9a8614e7e80f64a20c5e58b4ae233f908` проверяет полный `host+port`, требует `USER@HOST`, печатает понятные русские подсказки с примерами для каждого поля, использует project-local staging directory для Windows one-command и содержит opt-in staged-snapshot secret guard. Для следующего guest запуска подготовлен project-local writable Sandbox template: read-only inputs остаются в `C:\GatewayVPNGate`, а launcher стартует из отдельной writable `C:\GatewayVPNRuntime`. GitHub CI run `33904761787` полностью зелёный: secret-history, race suite, vet/fuzz/packaging/syntax, Windows portable contract и Linux nftables/netns gate прошли; `origin/main` синхронизирован. Для повторного clean-Windows gate нужен новый production-signed testing candidate и отдельное явное разрешение на key/build/tag/prerelease; stable/latest не менять. После успешного гостевого gate остаются физические Ubuntu Gateway/VPS, HiLink/Keenetic и 24/72-часовые endurance gates.
...
| Репозиторий | `MAIN_48433FC_SYNCED / FULL_CI_RUN_33904761787_PASS / PRODUCTION_SIGNED_TESTING_3BB466B_PUBLISHED_IMMUTABLE_PRERELEASE / PUBLIC_BASE_IMMUTABLE` | Current source и opt-in secret guard находятся в `origin/main`; exact testing tag `v0.1.1-testing.3bb466b` опубликован как immutable prerelease, но предшествует текущим deploy fixes. Public stable/latest `v0.1.0-successor.5723940` и Release `378316577` неизменны; publisher credential удалён |
...
| Автоматические тесты | `FULL_LINUX_RACE_VET_SYNTAX_PASS / SUCCESSOR_FOCUSED_AND_SERIAL_TESTS_PASS / GITHUB_CI_33904761787_ALL_JOBS_PASS / UBUNTU24_SYSTEMD_PASS / WINDOWS_NATIVE_TRUST_AND_FRESH_TARGET_PREP_PASS / GATEWAY_RESTORE_POINT_REBOOT_GATE_PASS / EXACT_VPS_UPDATE_GATE_PASS` | GitHub CI run `33904761787` для current commit `48433fc9a8614e7e80f64a20c5e58b4ae233f908` полностью прошёл за `19m57s`: secret-history `9s`, полный Go/race/vet/fuzz/packaging/syntax `17m47s`, Windows portable contract `1m19s` и Linux nftables/netns `1m50s`. Локально focused и полный serial `go test ./... -count=1 -p 1`, vet и diff-check проходят; clean guest должен быть повторён новым signed candidate. Privileged Gateway/VPS recovery evidence не изменено |
...
| 45 | `PARTIAL_EXTERNAL` | Linux SSH orchestrator, Windows OpenSSH adapter и portable production-signed `.exe` `0.1.1-testing.3bb466b` прошли native version/commit, channel/signer/self-identity, wrong hash/signer и tampered signature/PE gates; generated one-command прошла PowerShell 5.1 AST/hash-before-exec/finally-cleanup. Candidate реально запущен в clean Windows Sandbox и безопасно остановился до mutation на `DEPLOY_INPUT_INVALID`; найденные USER@HOST/host+port/staging defects исправлены в current green source. Полный two-target deploy нового signed successor ещё не выполнен |
...
Следующий локальный инкремент — после отдельного явного разрешения собрать и подписать новый testing successor из current green `48433fc`, создать новый exact immutable tag/prerelease, обновить read-only Sandbox inputs и выполнить clean Windows 10/11 x64 two-target workflow. Production `.gvkey` не открывать и tag/Release не создавать до такого разрешения; stable/latest не менять.
