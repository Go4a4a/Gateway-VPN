...
| Packaging | `PRODUCTION_GVKEY_READY / REPRODUCIBLE_D4EFD0F / PUBLIC_5723940_IMMUTABLE` | Две offline сборки exact `d4efd0f` совпали побайтно: Gateway `9654655c…`, VPS `4e3fba3f…`, bootstrap `95a3256f…`, deploy `60da9364…`, channel `f0ff01cb…`; disposable signer `8231e4d3…`. Acceptance defects исправляются новым exact successor. Production key не открывался. Предыдущий public immutable Release по-прежнему имеет проверенные `10/10` assets |
