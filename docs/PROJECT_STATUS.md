...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V26 / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_DATA_PAIRING_RENDERER_LOCAL_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / AMNEZIA_COEXISTENCE_REQUIRED / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_ACCESS_FOUNDATION / GROUPED_WEBUI_SPEC_ADDED / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / WATCHDOG_17_IMPLEMENTED_19_SPECIFIED / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source Gateway schema 26 реализует durable Management Fabric data model и read-only route/alias/ACL renderer, а VPS Agent schema 2 — одноразовое pairing, inventory Gateway/administrators, typed resources, ACL/access matrix и ownership-only health read model. Restricted VPS Hub получил функциональные предметные страницы для этих сущностей и отдельный полный `.gvpn-vps` backup/preview/restore; обязательный Gateway `.gvpn` остаётся отдельной ролью. Signed VPS installer локально интегрирует Agent account/state/config/TLS/admin/systemd и Admin-only TCP/9443 firewall. Для расширения всё ещё отсутствуют managed admin config/key lifecycle, privileged many-to-many WireGuard/route/double-ACL apply, реальный host watchdog/recovery, logs/update/diagnostics Hub, Amnezia coexistence lifecycle и grouped Gateway WebUI. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `87%`; общая production-готовность расширенного проекта — ориентировочно `70%`. Schema 26 foundation, VPS Hub control plane и обе обязательные WebUI backup-роли закрыты локальным кодом и тестами, но privileged many-to-many host runtime, managed administrator secrets/configs, полный Hub watchdog/lifecycle и coexistence ещё впереди. Далее обязательны multi-site/multi-VPS/Amnezia netns/systemd/security tests и физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| WireGuard management | `SINGLE_VPS_LOCAL_RUNTIME_PASS / MANY_TO_MANY_DATA_PAIRING_RENDERER_PASS / HOST_NOT_RUN` | Прежний controller идемпотентно создаёт один `10.80.0.0/24 dev wg-mgmt protocol 186`; schema 26 теперь хранит независимые per-link key refs/interfaces/subnets/endpoints/generations, монотонные slots и bounded pairing. Privileged simultaneous `gvm<N>` runtime, rotation и VPS peers ещё не реализованы |
| Management Fabric / VPS Hub | `CONTROL_PLANE_AND_BACKUP_LOCAL_PASS / PRIVILEGED_FABRIC_RUNTIME_PENDING` | VPS Agent schema 2 хранит одноразовые invitations, Gateway/admin peers, typed resources, ACL и desired/applied generations. Authenticated API и предметные Hub pages управляют Gateway, channels, external-key administrators, resources/access matrix и ownership-only health; `.gvpn-vps`, safe restore и signed installer wiring проходят. Managed admin configs, root host apply/rotation, operational logs/update/diagnostics и coexistence отсутствуют |
| Удалённые локальные ресурсы | `SCHEMA26_TYPED_VALIDATION_RENDERER_PASS / HOST_NOT_IMPLEMENTED` | Schema и renderer покрывают default-off typed resources, `LOCAL_SUBNET` acknowledgement, same-site publications, unique non-overlapping aliases и конкретные TCP/UDP/ICMP ACL. Текущий runtime по-прежнему не forwards Home LAN через management links |
...
| Self-health / watchdog | `V2_17_COMPONENT_LOCAL_PASS / VPS_OWNERSHIP_HEALTH_READ_MODEL_PASS / SUCCESSOR_HOST_RECOVERY_PENDING` | Текущие 17 Gateway components локально проходят. VPS Hub показывает только owned desired/observed health и честно сообщает `HOST_APPLY_NOT_IMPLEMENTED`; root reconciliation/recovery для `management_fabric_routes` и `wireguard_admin` ещё не реализован и не подменяется UI status |
| SQLite | `GATEWAY_V26_AND_VPS_AGENT_V2_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Gateway migration 26 хранит Management Fabric. Отдельная contiguous checksum-verified VPS Agent schema 2 хранит pairing/Gateway/admin/resource/ACL/health/generation state; fresh/migration/integrity/FK и backup tests проходят. Exact signed host evidence пока относится к Gateway schema 25, physical power cut остаётся внешним gate |
...
| API / Web UI | `VPS_HUB_GROUPED_PAGES_BROWSER_PASS / GATEWAY_GROUPED_NAV_PENDING` | VPS Hub имеет функциональные Overview, Gateway, Channels, Administrators, Resources, Access matrix, Watchdog и Backup/restore pages; login/pairing/CRUD/access/health browser smoke прошёл без console errors. Hub Logs/Diagnostics и новая шестиуровневая grouped navigation основного Gateway WebUI ещё не реализованы |
...
| Backup / restore | `GATEWAY_EXACT_SIGNED_POWER_CUT_PASS / VPS_ROLE_LOCAL_SIGKILL_RECOVERY_PASS / HOST_NOT_RUN` | VPS `.gvpn-vps` имеет отдельный authenticated role, bounded encryption/manifest, WebUI re-auth/preview/typed apply, cross-role rejection, same-VPS quarantine и import-as-new. Unit/integration test имитирует SIGKILL между replacements и fresh boot recovery; реальный bare-metal VPS power cut ещё не выполнялся |
...
| Автоматические тесты | `VPS_HUB_CONTROL_AND_BACKUP_FULL_GO_BROWSER_PASS / VPS_SYSTEMD_NFT_SOURCE_PASS / SCHEMA25_EXACT_HOST_EVIDENCE` | Полные Windows и offline Linux `go test ./... -count=1 -p 1`, vet и Linux builds проходят; VPS Hub browser smoke покрывает login, invitation, Gateway, administrators, resources, ACL matrix и owned health. Ubuntu 24 systemd graph и загрузка owned VPS nft table ранее проходят; privileged many-to-many runtime для нового source ещё не выполнялся |
...
| 37 | `IN_PROGRESS_LOCAL` | Один Gateway↔VPS runtime contour и endpoint uplink failover уже существуют; VPS Agent теперь хранит и управляет несколькими Gateway peers через одноразовое pairing. Multiple simultaneous Gateway `gvm<N>` links, per-link host interfaces/routes и rotation apply пока отсутствуют |
...
| 43 | `IN_PROGRESS_LOCAL` | Lightweight Go Agent, separate schema-v2 SQLite/auth, restricted Hub, pairing/Gateway/external-admin/resource/ACL APIs, grouped pages, backup и ownership-only health read model реализованы. Managed admin configs/QR/key rotation, root host apply/watchdog recovery, operational logs/update/diagnostics ещё не завершены |
...
9. Schema 26 Management Fabric и backup-capable VPS Hub уже существуют в current source, но опубликованные/exact signed artifacts относятся к прежнему schema-25 scope. Privileged many-to-many/local-resource apply, полный Hub/watchdog, coexistence и grouped WebUI ещё не прошли netns/systemd/browser/exact install gates; прежний single-VPS PASS их не доказывает.
