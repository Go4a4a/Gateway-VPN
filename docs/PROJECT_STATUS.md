...
| Packaging | `KEYLIFE_REPRO_PASS / PUBLIC_RELEASE_PENDING` | Exact `4ab1668` с новым use-time custody contract прошёл две независимые offline bundle-сборки одним disposable verified primary/backup signer: все 105 tree entries/76 files побайтно совпали, Gateway/VPS/channel повторно проверены backup public key, mismatch остановлен до `dist`, private-key material отсутствует. Public tags/releases пока отсутствуют; permanent key, новая RC version, LAN interface, draft assets и GitHub installation остаются отдельными рубежами |
