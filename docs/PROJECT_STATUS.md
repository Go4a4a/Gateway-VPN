...
**Текущий этап:** Два unique exact signed candidate прошли build/verify и Ubuntu 24.04 dry-run, но DHCP apply оба раза fail-closed откатился. Первый дефект state-root `0700` исправлен; повторный acceptance доказал дополнительный dnsmasq internal privilege-drop defect и wildcard TCP/UDP 53 conflict в rehearsal image. Текущий fix запускает dnsmasq сразу под systemd dedicated UID без `CAP_SETUID/GID`, использует `dnsmasq-base` и pre-mutation port conflict gate; полный Windows/Linux test/vet, Linux shell syntax и Ubuntu systemd graph PASS, третья signed acceptance ещё впереди. Новый обязательный self-health supervisor принят в PLAN §9.8 и должен быть реализован до 24-часового endurance
...
**Следующий шаг:** полный test/vet для systemd-owned dnsmasq fix, commit, rebuild clean Ubuntu rehearsal image с `dnsmasq-base` и третья unique signed install; затем реализация supervisor до harness smoke и 24-часового developer endurance.
