...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V22 / ETHERNET_RUNTIME_LOCAL_PASS / WATCHDOG_CONTOUR_LOCAL_PASS / SSH_INSTALLER_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact source `1b90ffcb99b25f79954cbc1b4bde7bcc0140175d` и disposable-signed `0.1.0-successor.1b90ffc` сохраняются как доказательство прежнего universal-installer scope, но больше не являются кандидатом текущего расширенного контракта. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation, ручное capability-gated power management и полный 16-component watchdog contour реализованы локально. First-install wizard теперь интерактивно проверяет/предлагает OpenSSH/SFTP, безопасно включает его только через management LAN либо сохраняет явный opt-out. Затем остаются полный `wg-ingress`, тематические log exports/tabs и оставшийся contextual help. Firmware/USB recovery и RTC wake-from-S5 остаются аппаратными gates конкретного Gateway. Новый tag/Release запрещён до реализации и проверки всего расширения.

**Оценка прогресса:** прежний scope остаётся воспроизводимо завершённым примерно на `99%`, но это больше не процент текущего проекта. После расширения знаменателя текущая программная реализация оценивается примерно в `88%`, общая production-готовность — примерно в `70%`. Это не потеря уже сделанного: installer, update/rollback, fail-closed, подписки, multi-uplink policy, API/auth/logging/watchdog и release pipeline переиспользуются. Незавершённый объём теперь включает hardware-validated Ethernet/firmware/USB recovery, полный `wg-ingress`, тематические log exports/tabs, оставшийся contextual help и повторный сквозной signed audit. После этого всё равно остаются physical Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture, RTC S5 и обязательные 24/72-часовые endurance.
...
| Репозиторий | `EXPANDED_SUCCESSOR_WORKTREE / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Новый schema-v22/watchdog/SSH increment проходит локальный audit, не подписан, не тегирован и не опубликован |
...
| Этап 1: bootstrap | `UNIVERSAL_WIZARD_V2_SSH_LOCAL_PASS / PRODUCTION_RELEASE_PENDING / HARDWARE_PENDING` | Мастер единообразно объясняет выборы для каждого компонента, выбирает несколько LAN-портов, DHCP/CIDR/dependencies, default-on либо явный opt-out SSH/SFTP, no-wait и GRUB, выводит обязательные/WebUI-настройки и exact mutation plan. Candidate `0.1.0-successor.1b90ffc` прошёл прежний fresh apply/reinstall/no-carrier/rollback; schema-v22 SSH increment ещё не собран как exact signed release |
...
| Self-health / watchdog | `V2_16_COMPONENT_LOCAL_PASS / EXACT_SIGNED_PREDECESSOR_PASS / HARDWARE_PENDING` | Schema 22 и policy v2 покрывают control/SQLite/firewall/network/DNS/SSH/Mihomo/WireGuard/routing/workers/convergence/backup/resources, per-component `MONITOR_ONLY/RECONCILE/RESTART`, durable budgets и external-outage suppression. Linux parser/netns/systemd/WebUI gates проходят; новый exact signed PID1/kill/hang gate ещё нужен |
| SQLite | `V22_WATCHDOG_MIGRATION_LOCAL_PASS / UPDATE_AUDIT_PENDING` | Migration 22 сохраняет прежние operator choices и добавляет typed thresholds/recovery map всех 16 компонентов; schema 21→22, idempotency, backup/recovery/update fixtures и foreign-key checks проходят. Signed 13→22 update/recovery gate ещё не запускался |
...
| API / Web UI | `WATCHDOG_V2_BROWSER_PASS / WG_INGRESS_LOG_TABS_HELP_PENDING` | «Система и безопасность» показывает полный component status, thresholds и допустимый recovery mode каждого компонента. Browser gate обнаружил и исправил перекрытие «Выйти»/системной вкладки; sidebar теперь прокручивается отдельно, длинный state не обрезается. Полный WG-ingress clients UI, thematic log tabs/exports и оставшийся contextual help ещё не завершены |
...
| Автоматические тесты | `SCHEMA_V22_FULL_LINUX_NETNS_SYSTEMD_BROWSER_PASS` | Полные offline Linux `go test ./... -count=1` и `go vet ./...`, Go format, JavaScript/shell syntax, diff check, реальный Ubuntu WireGuard JSON contour, SSH multi-LAN/uplink/opt-out netns, systemd/GRUB verify и browser preview проходят. Новый exact signed update/hardware gate ещё нужен |
