...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / GATEWAY_REMOTE_SIGNED_SOURCE_LOCAL_PASS / GATEWAY_COMPLETE_RESTORE_POINT_SYSTEMD_REBOOT_PASS / VPS_UPDATE_SYSTEMD_SIGKILL_ROLLBACK_PASS / BOTH_ROLES_SHARED_LIFECYCLE_LOCK_PASS / VPS_UNINSTALL_LIFECYCLE_SYSTEMD_PASS / SCHEMA_V31_SOURCE / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_GATEWAY_PRIVILEGED_NETNS_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V4_SOURCE / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / VPS_OPERATIONS_DIAGNOSTICS_LOCAL_PASS / VPS_OPERATIONS_ROOT_BOUNDARY_PRIVILEGED_PASS / GATEWAY_VPS_AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_PRIVILEGED_PACKET_3X_PASS / E2E_RELAY_ROLE_BACKUP_ROUNDTRIP_PASS / WG_ADMIN_ROTATION_ROLLBACK_PASS / LOCAL_RESOURCE_GATEWAY_PACKET_PASS / TOPOLOGY_PROFILE_SAFE_APPLY_PRIVILEGED_PASS / INITIAL_LAN_ROLE_IMPORT_LOCAL_PASS / GROUPED_RESPONSIVE_WEBUI_BROWSER_PASS / FIREWALL_SCHEMA_V6 / WG_INGRESS_LOCAL_PASS / WATCHDOG_19_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. Текущий schema-31 source реализует signed Stable/Testing и exact HTTPS staging, software-update policy, complete restore points, protected retention и password re-authenticated manual rollback. Disposable privileged Ubuntu 24/systemd gate доказал historical rollback, `SIGKILL` safety recovery и новый PID 1 с сохранением foreign state. Следующий обязательный блок — durable automatic check/download/apply scheduler, затем Windows deploy delivery и exact release/security audit. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `98%`; относительно текущего расширенного обязательного scope программная часть ориентировочно `95%`. Готовность к первой установке на физический стенд — примерно `91%`, полная production-готовность — примерно `80%`. Signed Gateway/VPS update foundations и complete Gateway restore-point rollback прошли privileged apply/recovery gates. В программном scope остаются automatic Gateway update scheduling, Windows deploy delivery и финальная exact release/security сборка. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Репозиторий | `EXPANDED_SCHEMA31_WORKTREE / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Exact local `a7a783b` schema-v25 baseline сохранён; текущий schema-31 блок ещё не является production-signed/tagged release |
...
| Management Fabric / VPS Hub | `BOTH_ROLES_PRIVILEGED_APPLY_AND_COEXISTENCE_PASS / E2E_RELAY_PACKET_3X_PASS / OPERATIONS_DIAGNOSTICS_LOCAL_PASS / VPS_UPDATE_SYSTEMD_GATE_PASS / VPS_LIFECYCLE_GUARD_PASS` | Обе роли имеют parameter-free root apply/recovery. VPS updater реализует strict upload, pointer-compatible host contract, DB copy migration, atomic switch, independent recovery и path-free WebUI status; privileged gates подтвердили success/finalize, rollback после `SIGKILL` и mutual exclusion update/install/uninstall с terminal-journal recovery |
...
| SQLite | `GATEWAY_V31_AND_VPS_AGENT_V4_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Gateway schema 31 добавляет typed software-update policy поверх schema-30 topology state. Contiguous VPS Agent schema 4 хранит public relay allocation и external administrator trust mode без inner private key. v1→current migration, integrity/FK/lifecycle и оба encrypted backup/restore round-trip проходят. Exact production-signed host evidence пока относится к Gateway schema 25 |
...
| API / Web UI | `GROUPED_GATEWAY_AND_VPS_LOCAL_PASS / UPDATE_RETENTION_BROWSER_PASS` | Обе роли прошли предметные вкладки при 320×720. Gateway «Система и безопасность» показывает separate update policy, signed sources, complete restore-point history и re-authenticated destructive rollback. Desktop/mobile browser gate не выявил document overflow, clipped actions или console errors |
...
| Signed update | `GATEWAY_REMOTE_MANUAL_SOURCE_AND_COMPLETE_RESTORE_SYSTEMD_PASS / VPS_TRANSACTION_SYSTEMD_SIGKILL_ROLLBACK_PASS / BOTH_ROLES_SHARED_LIFECYCLE_LOCK_PASS / AUTOMATIC_SCHEDULER_PENDING / HARDWARE_POWER_CUT_PENDING` | Gateway Stable/Testing check+stage, exact HTTPS, upload, policy, protected retention, manual rollback и complete safety recovery реализованы. Gateway и VPS root update/recovery/finalize используют общий install/uninstall lifecycle lock; terminal journals не создают permanent block, corrupt/nonterminal fail safe. Durable automatic check/download/apply worker и real bare-metal power-cut pending |
| Packaging | `PRODUCTION_GVKEY_READY / GATEWAY_UPDATE_ROLLBACK_SYSTEMD_PASS / VPS_AGENT_SOURCE_GATES_PASS / VPS_OPERATIONS_SYSTEMD_PASS / REPRODUCIBLE_A7A783B_BASELINE / PUBLIC_5723940_IMMUTABLE` | Installer/uninstall/tmpfiles/watchdog включают fixed Gateway rollback unit и root directories; полный Ubuntu 24 systemd graph прошёл. Новый exact production-signed artifact ещё не собирался; production key не открывался, public Release неизменён |
...
| Автоматические тесты | `FULL_LINUX_RACE_VET_SYNTAX_PASS / UBUNTU24_SYSTEMD_PASS / GATEWAY_RESTORE_POINT_REBOOT_GATE_PASS / EXACT_VPS_UPDATE_GATE_PASS` | Финальный native Linux `go test -race ./... -count=1 -p 1`, `go vet ./...`, Node/shell syntax, diff и Ubuntu 24 systemd/GRUB gates проходят. Privileged Gateway gate подтверждает historical rollback, `SIGKILL` safety recovery и новый PID 1; VPS gate сохраняет success/finalize/`SIGKILL` rollback evidence |
...
| 47 | `IN_PROGRESS_LOCAL` | Signed GitHub Stable/Testing manual check/download, exact HTTPS/upload, two-stage pointer transaction, complete restore-point retention/UI и privileged manual rollback/reboot recovery прошли. Durable automatic check/download/apply scheduler, multi-VPS observation, exact production-signed tree и bare-metal power-cut ещё отсутствуют |

**Итог аудита:** `29 PASS_LOCAL`, `4 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: обе backup-роли, information architecture, privileged Gateway/VPS Management Fabric, end-to-end administrator relay, единая topology transaction, Hub operations/diagnostics и signed VPS lifecycle закрыты локально; automatic remote Gateway update scheduler и Windows delivery требуют реализации/evidence. Затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Следующий инкремент — реализовать durable Gateway automatic update scheduler поверх schema-31 policy: restart-safe deadlines/jitter, separate check/download/apply gates, UTC maintenance window, service-route ladder, operation conflict suppression, reboot recovery и sanitized status. Затем выполнить focused interruption/systemd gate, после чего перейти к Windows deploy delivery. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP/Hub-логи, bounded diagnostics, privileged many-to-many Management Fabric, проверенный inner `wg-admin` relay с role backup round-trip, единую post-install смену topology, signed VPS update и manual remote Gateway update с complete restore-point recovery. Durable automatic Gateway update scheduler, Windows delivery и новый exact production-signed audit ещё не завершены. Поэтому exact `a7a783b` нельзя объявлять окончательным `INSTALL_READY` для всего расширенного проекта, хотя schema-25 recovery gate закрыт. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.
...
9. Schema 31 Gateway / schema 4 VPS Agent, обязательные role backups, topology safe apply, remote update/complete restore points и privileged Gateway/VPS runtimes существуют в current source, но опубликованные/exact production-signed artifacts относятся к прежнему schema-25 scope. Gateway/VPS multi-peer ACL/coexistence, inner admin-relay packet/anti-spoof/backup round-trip, topology/one-arm и restore-point rollback/reboot gates прошли privileged Docker/local gates; automatic update scheduler, Windows deploy и новый exact signed install gate ещё не закрыты.
