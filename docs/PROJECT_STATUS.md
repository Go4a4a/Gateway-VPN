...
**Текущий этап:** PLAN §9.8 self-health supervisor реализован и прошёл полный Windows/Linux test/vet, Ubuntu 24.04 systemd verify, shell syntax, packaging и privileged fail-closed netns gates. Migration v12, control heartbeat/hang detection, root supervisor, durable budgets, API/OpenAPI/WebUI, diagnostics и install/update/recovery/uninstall lifecycle находятся в текущем рабочем дереве. Следующий рубеж — clean commit, новый exact signed candidate и повторный Ubuntu 24.04 fresh install/idempotency/reboot smoke перед передачей команды установки на физический Gateway.
...
| Self-health / watchdog | `CODE_AND_SYNTHETIC_PASS / EXACT_SIGNED_SYSTEMD_PENDING` | Migration v12, audited bounded policy, fixed root action surface, jitter/thresholds, SIGHUP reconcile, fail-closed-before-restart, durable restart/reboot budgets, default-off reboot, stale heartbeat/status detection, maintenance/external-outage suppression, API/OpenAPI/WebUI и diagnostics прошли Windows/Linux tests. Unit graph прошёл Ubuntu 24.04 verify; actual signed PID 1 crash/hang/reboot-loop smoke ещё предстоит |
...
| Packaging | `EXACT_SIGNED_DHCP_INSTALL_REBOOT_PASS / PRODUCTION_RELEASE_PENDING` | Signed `.13` double-build/one-command orchestration и handshake прошли; disposable `.21/.22` доказали systemd update/recovery, `.27` — destructive restore/recovery. Exact `d471c12` прошёл fresh DHCP apply, strict idempotent reinstall и automatic new-PID1 boot/readiness без restart loop. Permanent key, production tag/assets и установка с публичного GitHub release ещё не выполнены |
...
| Автоматические тесты | `WINDOWS_AND_LINUX_PASS / REMOTE_PENDING` | Текущий watchdog worktree прошёл полный Windows и native Linux/amd64 `go test ./... -count=1`/`go vet ./...`, JS syntax, `git diff --check`, Ubuntu 24.04 shell syntax, `systemd-analyze verify` и privileged firewall/netns. Exact signed watchdog candidate и удалённый CI для будущего commit ещё не выполнены |

## Ближайший следующий инкремент

Следующий локальный инкремент: зафиксировать уже прошедший локальные gates watchdog block, собрать новый VCS-stamped exact signed candidate и повторить clean Ubuntu 24.04 archive dry-run, fresh apply, strict idempotent reinstall и новый PID 1/reboot smoke. Обязательно проверить fresh watchdog/control files, ownership, отсутствие restart loop, default-off reboot, `PATH_BLOCKED`, HTTPS/DNS/DHCP и installer readiness. После PASS кандидат и однострочная команда передаются пользователю для первой установки на физический Gateway. Реальные USB/HiLink/Keenetic/VPS packet capture и 24/72-часовые endurance gates начинаются после пользовательской установки; до их прохождения production status остаётся `NOT_DONE`.

## Критический путь до release

Критический путь до **готовности к первой установке на железо** теперь сокращён до: clean commit → exact signed build/verify → clean-install/idempotency/reboot smoke → команда установки. Оценка на 2026-08-26: `3–7` часов активной работы без гарантии, если signed PID 1 проверка обнаружит новый defect. После установки отдельный путь до production release включает реальные Gateway/VPS/HiLink/Keenetic проверки, найденные исправления, 24-часовой developer и несокращаемый 72-часовой release endurance.
