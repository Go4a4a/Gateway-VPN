...
| Unified access methods | `LOCAL_DOD_REMOTE_CI_PASS / SIGNED_FRESH_GATE_PENDING` | Direct + subscriptions имеют один authenticated ordered list и один server-side read model. Каждый modem показывает direct и все VPN methods; каждая subscription — все modem paths; matrix содержит оба kind. Preferred node order, fingerprint transfer, sticky transition и startup recovery подтверждены exact remote CI без немедленного обрыва active path |
...
| Автоматические тесты | `LOCAL_AND_REMOTE_E5B5E7C_PASS` | Full Windows `go test ./... -count=1`, full `go vet ./...`, Linux app-test cross-compile/vet, Go/shell formatting/syntax и exact remote race/build/netns/systemd run `33128823746` проходят |
...
**Итог аудита:** `15 PASS_LOCAL`, `0 IN_PROGRESS_LOCAL`, `11 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: остаются новый exact signed fresh-install gate, production publish, H1/H2/VPS и реальные 24/72h evidence.
...
Опубликованный baseline локально готов к первой установке по прежней модели, но пользователь выбрал сначала завершить более функциональный unified-access successor. Data-plane, refresh, management interface, direct/VPN read model, node preferences и startup policy successor готовы и опубликованы; до нового install-ready candidate остаётся fresh signed systemd acceptance exact текущего дерева. Production key готов, однако version/tag/draft Release не создаются до завершения successor и отдельного разрешения пользователя.
