...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V26 / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_DATA_PAIRING_RENDERER_LOCAL_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V3_MANAGED_ADMIN_CONFIG_LOCAL_PASS / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_ACCESS_FOUNDATION / GROUPED_WEBUI_SPEC_ADDED / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / WATCHDOG_17_IMPLEMENTED_19_SPECIFIED / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source Gateway schema 26 реализует durable Management Fabric foundation, а VPS Agent schema 3 — pairing/Gateway/resources/ACL control plane и полный managed administrator key/config lifecycle. VPS host-plan больше не read-only: parameter-free trigger, transactional `wg-mgmt`, owned protocol-186 routes, double-ACL nftables projection, root journal/receipt, rollback, boot recovery и periodic watchdog реализованы и прошли privileged multi-peer Linux gates. Restricted VPS Hub показывает desired/applied generation и позволяет безопасно поставить apply/reconciliation в очередь. Обязательные файловые backup/restore обеих ролей реализованы отдельно: Gateway `.gvpn`, VPS `.gvpn-vps`, с WebUI preview/re-auth/pre-restore snapshot/rollback и запретом cross-role restore. В расширенном scope ещё отсутствуют Gateway-side simultaneous `gvm<N>` runtime, end-to-end `wg-admin` relay/local-resource projection, полный Hub logs/update/diagnostics и grouped Gateway WebUI. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `90%`; общая production-готовность расширенного проекта — ориентировочно `72%`. Schema 26 foundation, VPS Hub schema-3 control plane, managed administrator delivery, privileged VPS host projection/watchdog и обе обязательные WebUI backup-роли закрыты локальным кодом и тестами. Gateway-side many-to-many links, end-to-end admin relay/local-resource runtime и оставшиеся Hub/Gateway UI/lifecycle функции ещё впереди. Privileged Docker доказал VPS ACL/rollback/coexistence, но физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance остаются обязательными.
...
| Репозиторий | `EXPANDED_SUCCESSOR_EXACT_LOCAL / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Exact local `a7a783b` schema-v25/firewall-v4 подписан только disposable test signer, не тегирован и не опубликован |
...
| WireGuard management | `VPS_MULTI_PEER_WG_MGMT_PRIVILEGED_PASS / GATEWAY_MANY_TO_MANY_HOST_PENDING` | VPS Hub транзакционно применяет несколько Gateway/admin peers на fixed `wg-mgmt`, сохраняет legacy `10.80.0.1/24` Hub prefix и owned protocol-186 routes. Schema 26 Gateway хранит независимые per-link identities/slots, но simultaneous Gateway-side `gvm<N>` runtime и rotation ещё не реализованы |
| Management Fabric / VPS Hub | `PRIVILEGED_APPLY_ROLLBACK_WATCHDOG_NETNS_PASS / LIFECYCLE_UI_PARTIAL` | VPS Agent управляет invitations, Gateway, external/managed administrators, resources/ACL и generations. Parameter-free trigger применяет WireGuard/routes/double ACL одной root transaction; journal, receipt, rollback, boot recovery и periodic watchdog реализованы. Privileged tests подтвердили четыре handshakes, default-deny ACL и сохранность foreign UFW/Docker/Amnezia objects. Hub logs/update/diagnostics ещё отсутствуют |
| Удалённые локальные ресурсы | `VPS_DOUBLE_ACL_PRIVILEGED_PASS / GATEWAY_FORWARDING_PENDING` | Schema/renderer и VPS root runtime покрывают default-off typed resources, acknowledgement, unique aliases и конкретные TCP/UDP/ICMP ACL. Gateway-side publication/NAT/return-path и end-to-end Home LAN forwarding ещё не реализованы |
...
| Self-health / watchdog | `GATEWAY_17_LOCAL_PASS / VPS_FABRIC_ROOT_WATCHDOG_PASS / WIREGUARD_ADMIN_PENDING` | Текущие 17 Gateway components локально проходят. VPS periodic root watchdog сверяет authenticated desired plan, root receipt, WireGuard, routes и owned nftables, а при расхождении ставит bounded reconciliation в очередь. WebUI читает только свежий display-status и не может через него разрешить root action. Отдельный successor component `wireguard_admin` ещё не реализован |
| SQLite | `GATEWAY_V26_AND_VPS_AGENT_V3_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Gateway migration 26 хранит Management Fabric. Contiguous checksum-verified VPS Agent schema 3 добавила config lifecycle/rotation source к pairing/Gateway/admin/resource/ACL/health/generations; v1→current migration, integrity/FK/lifecycle и backup tests проходят. Exact signed host evidence пока относится к Gateway schema 25 |
...
| API / Web UI | `VPS_HUB_FABRIC_CONTROLS_LOCAL_PASS / BROWSER_RETEST_PENDING / GATEWAY_GROUPED_NAV_PENDING` | VPS Hub pages/API создают external/managed peers, one-use config, rotation/revoke, resources/ACL и теперь позволяют поставить apply/reconciliation в очередь, показывая desired/applied generation и root status. Integration tests проходят; визуальный browser retest именно новых apply-controls, Hub Logs/Diagnostics и grouped Gateway navigation остаются |
...
| Backup / restore | `MANDATORY_ROLE_SEPARATED_WEBUI_PASS / GATEWAY_EXACT_SIGNED_RECOVERY_PASS / VPS_SIGKILL_RECOVERY_PASS / BARE_METAL_PENDING` | Gateway создаёт/восстанавливает encrypted `.gvpn`, VPS Hub — отдельный `.gvpn-vps`; WebUI обеих ролей поддерживает download, upload, verified preview и Apply с re-authentication. Cross-role restore запрещён, перед Apply создаётся verified snapshot, journal/reboot recovery/rollback реализованы. Реальный bare-metal VPS power cut ещё не выполнялся |
| Signed update | `EXACT_13_TO_16_POINTER_PASS / EXACT_16_TO_25_HOST_CONTRACT_PASS / HARDWARE_POWER_CUT_PENDING` | Pointer candidate `9301e05` прошёл controlled health rejection, successful `13 → 16`, finalization/reboot и interruption recovery. Exact `ga7a783b` доказал public-baseline `16 → 25`, затем uninstall/reinstall; bare-metal power cut остаётся внешним gate |
| Packaging | `PRODUCTION_GVKEY_READY / VPS_AGENT_SOURCE_GATES_PASS / REPRODUCIBLE_A7A783B_BASELINE / PUBLIC_5723940_IMMUTABLE` | VPS signed-tree contract теперь требует Agent/config/restore units; installer wiring и recovery/uninstall ownership проверены source tests, Ubuntu systemd graph и nft load. Новый exact signed artifact ещё не собирался; production key не открывался, public Release неизменён |
| Uninstall | `WEBUI_DURABLE_GUARDIAN_EXACT_NEW_PID1_PASS / HARDWARE_POWER_CUT_PENDING` | Exact `ga7a783b` прошёл password re-authenticated `PRESERVE_DATA`, два штатных terminal runs, затем SIGKILL fixed guardian после `tooling-ready`, power-loss PID1, очищенный `/run`, unavailable networkd skip, root-only receipt и отсутствие owned projection. DB/WAL/SHM и четыре secret files совпали побайтно, schema 25/integrity целы, packages не удалены и reinstall PASS |
...
| Автоматические тесты | `VPS_PRIVILEGED_FABRIC_3X_PASS / FULL_GO_VET_BUILD_PASS / SCHEMA25_EXACT_HOST_EVIDENCE` | Полные Windows и offline Linux test/vet/build, Node syntax, format/diff проходят. Privileged suite трижды подтвердил nft parser/coexistence, два Gateway + два administrator handshakes/ACL, actual rollback и atomic watchdog status. Exact signed host artifact всё ещё относится к schema 25 |
...
| 37 | `IN_PROGRESS_LOCAL` | Один Gateway↔VPS runtime contour и endpoint uplink failover уже существуют; VPS Agent управляет несколькими Gateway/admin peers, one-use managed configs и replacement rotation, а read-only host-plan строит exact peers/routes. Parameter-free root apply, simultaneous Gateway `gvm<N>` links и fresh-handshake rotation completion пока отсутствуют |
| 38 | `IN_PROGRESS_LOCAL` | Durable resource/ACL desired generations и canonical access matrix реализованы, текущий firewall имеет management allowlist и запрет uplink exposure; versioned double ACL apply и packet-level inter-site/admin deny для many-to-many ещё отсутствуют |
...
| 41 | `IN_PROGRESS_LOCAL` | VPS Hub уже имеет сгруппированные Overview/Gateway/Channels/Administrators/Resources/Access matrix/Watchdog/Backup pages. Основной Gateway WebUI имеет предметные страницы, thematic logs и accessible help, но его новая шестиуровневая grouped navigation ещё не реализована |
...
| 43 | `IN_PROGRESS_LOCAL` | Lightweight Go Agent, separate schema-v3 SQLite/auth, restricted Hub, pairing/Gateway/external+managed-admin/resource/ACL APIs, grouped pages, one-use config/rotation, backup и owned health/host-plan реализованы. QR, root host apply/watchdog recovery, operational logs/update/diagnostics ещё не завершены |
...
**Итог аудита:** `21 PASS_LOCAL`, `11 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: обе backup-роли закрыты локально, но новый Management Fabric host runtime, полный VPS Hub/watchdog, coexistence и Windows delivery требуют реализации/evidence; затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Следующий инкремент — Gateway-side simultaneous `gvm<N>` runtime и согласованная local-resource projection, затем end-to-end `wg-admin` relay и оставшиеся Hub logs/update/diagnostics. До exact release повторить privileged Gateway↔VPS systemd/browser/coexistence gates. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, single-VPS management interface, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP-логи и понятный выбор каждого install-компонента. VPS-сторона нового Management Fabric теперь имеет проверенный privileged runtime, однако Gateway-side many-to-many/local-resource/admin-relay scope ещё не завершён, поэтому exact `a7a783b` нельзя объявлять окончательным `INSTALL_READY` для всего проекта, хотя schema-25 recovery gate закрыт. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.
...
9. Schema 26 Management Fabric, обязательные role backups и privileged VPS Hub fabric runtime существуют в current source, но опубликованные/exact signed artifacts относятся к прежнему schema-25 scope. VPS multi-peer ACL/rollback/coexistence прошли privileged Docker, однако Gateway-side many-to-many/local-resource/admin-relay, полный Hub lifecycle, grouped Gateway WebUI и exact signed install gates ещё не закрыты.
