...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V29_SOURCE / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_GATEWAY_PRIVILEGED_NETNS_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V4_SOURCE / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / GATEWAY_VPS_AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_PRIVILEGED_PACKET_3X_PASS / E2E_RELAY_ROLE_BACKUP_ROUNDTRIP_PASS / WG_ADMIN_ROTATION_ROLLBACK_PASS / LOCAL_RESOURCE_GATEWAY_PACKET_PASS / GROUPED_RESPONSIVE_WEBUI_BROWSER_PASS / FIREWALL_SCHEMA_V5 / WG_INGRESS_LOCAL_PASS / WATCHDOG_19_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source Gateway schema 29 и VPS Agent schema 4 реализуют Gateway-terminated `wg-admin`, allowlisted VPS UDP relay, external end-to-end administrators, root-owned apply/watchdog и secret-free WebUI/API. Literal privileged gate трижды доказал реальный inner WireGuard handshake, allow/deny, wrong-port, plaintext и spoofed-source отрицательные сценарии; role-specific encrypted backup/restore сохраняет relay topology, но не создаёт inner private key на VPS. Gateway/VPS WebUI прошли повторный полный 320 px responsive gate и используют компактный мобильный selector вместо обрезанной горизонтальной ленты. Следующий крупный блок — единая post-install transaction переключения topology profiles, затем Hub logs/update/diagnostics. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `94%`; общая production-готовность расширенного проекта — ориентировочно `76%`. Gateway/VPS privileged Management Fabric, local-resource packet path, root/encrypted backup boundary, grouped responsive WebUI, обе backup-роли и end-to-end relay packet/anti-spoof lifecycle теперь имеют локальное evidence. Topology-profile transaction и оставшиеся Hub lifecycle-функции ещё впереди. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Management Fabric / VPS Hub | `BOTH_ROLES_PRIVILEGED_APPLY_AND_COEXISTENCE_PASS / E2E_RELAY_PACKET_3X_PASS` | Обе роли имеют parameter-free root apply, receipt/recovery и owned nft/routes/WireGuard projection. VPS gate подтвердил four-peer ACL; Gateway gate — two-link ACL/DNAT/selective removal. Отдельный relay gate трижды доказал inner WireGuard handshake, bounded relay counters и anti-spoof при сохранении foreign UFW/Docker/Amnezia-like tables. Hub logs/update/diagnostics отсутствуют |
| Удалённые локальные ресурсы | `GATEWAY_PREFIX_DNAT_ACL_RETURN_PATH_NETNS_PASS / E2E_ADMIN_RELAY_PACKET_ANTI_SPOOF_PASS` | Typed publications/ACL применяются на VPS и Gateway. Kernel gates доказывают обычный routed path и end-to-end inner `wg-admin`: TCP/8443 разрешён, TCP/8444/wrong-port/plaintext/spoofed-source запрещены. VPS хранит только allowlisted UDP relay state и не имеет inner private key |
...
| Self-health / watchdog | `GATEWAY_19_WITH_WG_ADMIN_LOCAL_PASS / VPS_RELAY_ROOT_WATCHDOG_PASS / MANAGEMENT_FABRIC_KERNEL_PROJECTION_PASS` | Gateway `wireguard_admin` сверяет inner identity/peer set/routes/handshake/relay generations и отличает external stale handshake от local drift. VPS root watchdog проверяет ровно owned relay rules/counters; privileged packet gate подтвердил ненулевые counters. Hardware recovery остаётся внешним gate |
| SQLite | `GATEWAY_V29_AND_VPS_AGENT_V4_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Gateway schema 29 хранит inner contour/relay/tunnel/trust mode; contiguous VPS Agent schema 4 — public relay allocation и external administrator trust mode без inner private key. v1→current migration, integrity/FK/lifecycle и оба encrypted backup/restore round-trip проходят. Exact signed host evidence пока относится к Gateway schema 25 |
...
| API / Web UI | `GROUPED_GATEWAY_AND_VPS_LOCAL_PASS / BOTH_ROLES_ALL_TABS_320PX_BROWSER_PASS` | Обе роли прошли все вкладки при 320×720 без clipped controls/document overflow. Длинные подписи увеличивают высоту кнопки, checkbox/help использует `auto minmax(0,1fr) auto`, таблицы прокручиваются локально, а мобильная навигация обеих ролей использует полный selector с синхронизацией активной desktop-вкладки. Hub Logs/Diagnostics как функции ещё не реализованы |
...
| Автоматические тесты | `GATEWAY_AND_VPS_PRIVILEGED_FABRIC_3X_PASS / E2E_RELAY_PRIVILEGED_3X_PASS / FULL_GO_TEST_PASS / SCHEMA25_EXACT_HOST_EVIDENCE` | Полный Windows `go test ./...`, Node syntax и diff проходят. Отдельный Ubuntu 24 `--network none` relay gate трижды подтвердил inner handshake/allow/deny/anti-spoof, exact five rules/counters и отсутствие inner peer/key на VPS; предыдущие Gateway/VPS fabric suites остаются зелёными. Exact signed host artifact всё ещё относится к schema 25 |
...
| 43 | `IN_PROGRESS_LOCAL` | Lightweight Go Agent, separate schema-v4 SQLite/auth, restricted Hub, pairing/Gateway/external+managed/E2E-admin/resource/ACL/relay APIs, grouped pages, one-use config/rotation, backup и owned host apply/watchdog реализованы. QR, operational logs/update/diagnostics ещё не завершены |
...
**Итог аудита:** `27 PASS_LOCAL`, `5 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: обе backup-роли, information architecture, privileged Gateway/VPS Management Fabric и end-to-end administrator relay закрыты локально; единая topology transaction, полный VPS Hub lifecycle и Windows delivery требуют реализации/evidence. Затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Следующий инкремент — единая post-install topology transaction с переключением one-/two-card, Ethernet/HiLink и `wg-ingress` профилей через WebUI safe apply/confirm/rollback; затем Hub logs/update/diagnostics. До exact release повторить privileged Gateway↔VPS systemd/browser/coexistence gates. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP-логи, privileged many-to-many Management Fabric и проверенный inner `wg-admin` relay с role backup round-trip. Единая post-install смена topology и полный Hub lifecycle ещё не завершены. Поэтому exact `a7a783b` нельзя объявлять окончательным `INSTALL_READY` для всего расширенного проекта, хотя schema-25 recovery gate закрыт. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.
...
9. Schema 29 Gateway / schema 4 VPS Agent, обязательные role backups и privileged Gateway/VPS runtimes существуют в current source, но опубликованные/exact signed artifacts относятся к прежнему schema-25 scope. Gateway/VPS multi-peer ACL/coexistence и inner admin-relay packet/anti-spoof/backup round-trip прошли privileged Docker/local gates; полный Hub lifecycle и новый exact signed install gate ещё не закрыты.
