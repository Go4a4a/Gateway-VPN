...
### Сессия 2026-09-05 — опубликованный production-signed testing candidate `v0.1.1-testing.373554a`

**Сделано и проверено:**

- из exact clean commit `373554a095be019bbb6739fafaf753172041026d` собран production-signed candidate `v0.1.1-testing.373554a` с signer fingerprint `fceb4a543d90aabbbe11a42d2c210c1565c235d017da7adc6c5b9d95c8eda60c`;
- Gateway tree (57 файлов), VPS tree (35 файлов), channel manifest и все пять role-artifacts прошли локальную Ed25519/SHA-256 верификацию;
- Gateway archive SHA-256 `f22a77d663f304f3b786fd5bbe6b8a1d7a65e8df1b256fa2168bc10c65cffd31`, bootstrap `cbf8caa44449578e037697b437075bb0567f21a4598143eac91f575f52180c5e`, VPS archive `55f8cbd42b5e7530098006c1432a8a1d0d9c525c84e7cde6c7d82609595fab24`, Linux deploy `dae8e542ecaaa00b5d0871ce48ff5d5fc47db9637746b5c519970c60f3ec25ff`, Windows deploy `2ce3670927a9549af6120dbbf7d1d5f4e80163c6a73420732a7bf448733fbd15`;
- channel manifest SHA-256 `1d5f510f21b24b73a79a6b172495e1467bdc8090385ee77d1a8f71f640b6ecd4`; exact annotated tag `v0.1.1-testing.373554a` отправлен в `origin` и разрешается в тот же commit;
- GitHub prerelease опубликован по [v0.1.1-testing.373554a](https://github.com/Go4a4a/Gateway-VPN/releases/tag/v0.1.1-testing.373554a) с 14 assets и `isPrerelease=true`, `isDraft=false`;
- независимое скачивание всех 14 опубликованных assets в project-local `.cache/release-builder/post-publication-0.1.1-testing.373554a-20260905` дало exact SHA-256 совпадение и повторную успешную `channel-verify`;
- GitHub `Latest` остался `v0.1.0-successor.5723940`; stable channel и предыдущие immutable testing releases не изменялись.

**Ограничения:** публикация signed candidate не закрывает hardware gates. Физические Ubuntu Gateway/VPS, минимум два HiLink-модема, Keenetic, USB/power-cut, carrier loss, реальный внешний VPS и 24/72-часовой endurance всё ещё требуют проверки на целевом железе. `ONE_ARM_WIREGUARD` не объявляется remote zero-to-ready без создания и безопасной передачи первого Keenetic peer/config.

**Следующий шаг:** использовать опубликованный candidate для физической установки/диагностики по выбранному профилю, фиксируя фактические модели, интерфейсы, firmware, route/MTU, modem failover и длительное поведение; после hardware evidence обновить DoD, не переименовывая этот release в production-ready автоматически.

### Сессия 2026-09-05 — доказанное подтверждение нового management-пути и сохранение topology при upgrade

**Сделано:**

- добавлены три явно разделённых режима подтверждения initial topology: `automatic`, `external-wireguard` и `local-console`;
- `automatic` подтверждает применение только если запрос управления остался на исходном интерфейсе и адресе; он не имитирует доступность нового пути;
- `external-wireguard` создаёт durable pending confirmation и принимает точную одноразовую фразу только из WebUI-запроса, пришедшего через настроенную пользователем подсеть `wg-ingress`;
- `local-console` принимает подтверждение только с `/dev/console`, Linux VT `tty1..tty63` или serial console; SSH, `/dev/pts`, pipe, `/dev/tty64` и USB TTY отклоняются;
- добавлена read-only команда `gateway-vpn topology-state`, которая возвращает только converged `ACTIVE` topology с profile, generation, LAN interface/address и состоянием DHCP/DNS;
- host upgrader передаёт внутренний `--preserve-applied-topology`, не удаляет активные `boot.nft`, `dnsmasq.conf` и принадлежащие topology networkd-файлы, не запускает initial topology apply повторно и проверяет candidate runtime по фактической активной topology;
- install report сохраняет canonical `initial_topology_token`, а fresh config явно фиксирует `lan_service_mode: dhcp_dns|disabled`;
- packaging assertions запрещают будущему изменению upgrader удалять активные topology assets.

**Проверено:** полный `go test ./... -count=1`, `go vet ./...`, `bash -n scripts/install-gateway.sh scripts/upgrade-gateway-host.sh test/netns/initial_topology_preflight.sh` и `git diff --check` — PASS. В сохранённом privileged Docker-контейнере `gateway-vpn-topology-contract-20260905` выполнен полный `test/netns/initial_topology_preflight.sh`: подтверждены valid direct/bridge tokens, отказ до mutation для mismatch/unknown/unsupported входов, console gate, durable external confirmation/rollback и topology backend apply/commit/rollback. Evidence сохранён в project-local `.cache/netns/initial-topology-preflight-20260905T070757Z-1`; контейнер, evidence и кэши автоматически не удалялись.

**Ограничения:** production-signed testing candidate, exact tag и clean Windows guest gate ещё не выполнены. Физические Ubuntu Gateway/VPS, HiLink/Keenetic, USB/power-cut и 24/72-часовые endurance проверки не заменены локальным fixture. Мастер пока не создаёт и не передаёт первый Keenetic peer/config до удалённого `ONE_ARM_WIREGUARD` apply, поэтому этот профиль не объявляется zero-to-ready.

**Следующий шаг:** с отдельным разрешением пользователя собрать production-signed testing candidate из проверенного source, затем повторить clean Windows guest gate и передать пользователю точную команду первой установки стандартного `ETHERNET_HILINK` профиля на физическое Ubuntu-железо.

### Сессия 2026-09-05 — повторный полный CI после исправления Linux fixture

**Проверено:** GitHub Actions run `33953634770` для exact commit `b9e3b8c7b9f43309e89b7d5db3eb5c669bd686ab` завершился `success`. `Repository secret history gate`, `Go, packaging and syntax gates` (race suite, vet, fuzz smoke, release builds и syntax), `Windows portable deploy contract` и `Linux nftables fail-closed gate` завершены успешно. Linux gate дополнительно прошёл сборку command test binary и `initial_topology_preflight.sh` с явным третьим аргументом, после чего успешно выполнил topology rollback, service-route, Management Resource, VPS boundary и pinned Ubuntu systemd steps.

**Решение:** CI wiring считается исправленным; старый failed run `33952475721` сохранён как диагностическое evidence и не переиспользуется. Следующий шаг — с отдельным разрешением пользователя собрать production-signed testing candidate из проверенного source и затем повторить clean Windows guest gate. Stable/latest и опубликованный `v0.1.1-testing.f300b25` не изменялись.
...
- auto-confirm first-install ограничен неизменившимся management origin: shared-only/one-arm не выдаёт локальный root-процесс за фактическое подключение через новый `wg-ingress`, отклоняется до `Stage` и требует отдельного external-confirm/local-console workflow;
...
**Следующий шаг:** добавить внешний confirmation/local-console contract для first-install `ONE_ARM_WIREGUARD`/shared-only `MIXED` и интеграционный Linux fixture; после source validation и отдельного разрешения — новый immutable testing candidate и повторный clean-Windows gate.
