...
**Общее состояние:** `MAIN_SYNCED / FULL_CI_RUN_33863273880_PASS / PRODUCTION_SIGNED_TESTING_3BB466B_PUBLISHED_IMMUTABLE_PRERELEASE / NATIVE_WINDOWS_ARTIFACT_SMOKE_PASS / FRESH_WINDOWS_GATE_TARGETS_READY / EXPANDED_SUCCESSOR_IMPLEMENTATION_LOCAL_PASS / CLEAN_WINDOWS_GUEST_INTERACTIVE_INPUT_PENDING / HARDWARE_AND_ENDURANCE_GATES_PENDING`
**Текущий этап:** exact current-source candidate `3bb466b6a418298c46d13073edeee5cff505ab1c` содержит тот же code tree, который прошёл GitHub Actions CI #86 (`33863273880`) полностью; последующие commits до candidate меняли только журнал с `[skip ci]`. Production signer подписал bundle `0.1.1-testing.3bb466b`, exact tag отправлен, а проверенный Draft опубликован как immutable testing prerelease: `https://github.com/Go4a4a/Gateway-VPN/releases/tag/v0.1.1-testing.3bb466b`. После публикации все `14/14` assets заново скачаны и совпали с локальным bundle по имени и SHA-256; подписанный channel и пять role artifacts криптографически проверены, публичный manifest доступен без GitHub credential. Stable/latest `v0.1.0-successor.5723940` не изменён, одноразовая GitHub-авторизация удалена. Текущий Windows deploy EXE прошёл native positive/negative trust smoke и PowerShell 5.1 gate. Для clean-Windows E2E подготовлены два fresh Ubuntu 24.04 systemd/SSH target на exact Default Switch address; оба доступны только по pinned disposable key, не содержат Gateway/VPS state и имеют пустой `systemctl --failed`. Gateway target получил отдельный empty `lan0`; validated `.wsb` запущен и отвечает, read-only mapped inputs и пошаговые значения мастера доступны внутри guest. Установка ещё не начата: обязательный ручной запуск опубликованной one-command в Sandbox PowerShell ожидает пользователя. После этого остаются физические Ubuntu Gateway/VPS, HiLink/Keenetic и 24/72-часовые endurance gates.
...
| Автоматические тесты | `FULL_LINUX_RACE_VET_SYNTAX_PASS / UBUNTU24_SYSTEMD_PASS / WINDOWS_NATIVE_TRUST_AND_FRESH_TARGET_PREP_PASS / GATEWAY_RESTORE_POINT_REBOOT_GATE_PASS / EXACT_VPS_UPDATE_GATE_PASS` | Финальный native Linux `go test -race ./... -count=1 -p 1`, `go vet ./...`, Node/shell syntax, diff и Ubuntu 24 systemd/GRUB gates проходят. Windows current EXE trust-smoke и fresh two-target preparation прошли; clean guest execution ещё pending. Privileged Gateway gate подтверждает historical rollback, `SIGKILL` safety recovery и новый PID 1; VPS gate сохраняет success/finalize/`SIGKILL` rollback evidence |
