...
**Общее состояние:** `MAIN_DIRTY_SOURCE_FIXES / FULL_CI_PRIOR_CANDIDATE_PASS / PRODUCTION_SIGNED_TESTING_3BB466B_PUBLISHED_IMMUTABLE_PRERELEASE / NATIVE_WINDOWS_ARTIFACT_SMOKE_PASS / FRESH_WINDOWS_GATE_TARGETS_READY / CLEAN_WINDOWS_GUEST_LOCAL_VALIDATION_FAILED_SAFE / NETNS_HARNESS_RELIABILITY_FIX_PENDING_CI / NEW_SIGNED_CANDIDATE_REQUIRED / HARDWARE_AND_ENDURANCE_GATES_PENDING`
**Текущий этап:** exact published candidate `3bb466b6a418298c46d13073edeee5cff505ab1c` был реально запущен пользователем в clean Windows Sandbox. Он безопасно остановился на `LOCAL_VALIDATION` с `DEPLOY_INPUT_INVALID`, не изменив ни Gateway, ни VPS. Введённый VPS endpoint был без имени пользователя (`172.18.224.1` вместо `USER@HOST`), а старый validator ошибочно запрещал один host даже при разных SSH-портах `42022/42023`, которые используются двумя изолированными target-контейнерами. В source исправлены проверка полного `host+port`, понятные русские подсказки мастера с примерами для каждого поля и project-local staging directory для Windows one-command вместо системного `%TEMP%`; оба target-а остаются чистыми. Дополнительно release-gate harness теперь ждёт готовности `tcpdump` перед отправкой MSS SYN и использует bounded 10-секундное окно: GitHub CI подтвердил прежний timeout `124` как единственный сбой этого теста. Focused и полный serial `go test ./... -count=1 -p 1` после изменений проходят. Для повторного clean-Windows gate нужен новый production-signed testing candidate и отдельное явное разрешение на key/tag/release; stable/latest не менять. После успешного гостевого gate остаются физические Ubuntu Gateway/VPS, HiLink/Keenetic и 24/72-часовые endurance gates.
...
| Автоматические тесты | `FULL_LINUX_RACE_VET_SYNTAX_PASS / SUCCESSOR_FOCUSED_AND_SERIAL_TESTS_PASS / UBUNTU24_SYSTEMD_PASS / WINDOWS_NATIVE_TRUST_AND_FRESH_TARGET_PREP_PASS / GATEWAY_RESTORE_POINT_REBOOT_GATE_PASS / EXACT_VPS_UPDATE_GATE_PASS` | Финальный native Linux `go test -race ./... -count=1 -p 1`, `go vet ./...`, Node/shell syntax, diff и Ubuntu 24 systemd/GRUB gates проходят. После source fixes focused и полный serial `go test ./... -count=1 -p 1` проходят; старый published EXE trust-smoke и fresh two-target preparation прошли, clean guest фактически остановился безопасно на local validation и должен быть повторён новым signed candidate. Privileged Gateway gate подтверждает historical rollback, `SIGKILL` safety recovery и новый PID 1; VPS gate сохраняет success/finalize/`SIGKILL` rollback evidence |
