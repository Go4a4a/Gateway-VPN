...
**Текущий этап:** exact candidate `0.1.0-traffic.3c13b09` из clean commit `3c13b093718e9e0ac2b38da9617baf05039ba732` завершил локальный release gate. Две независимые signed сборки побайтно совпали; fresh Ubuntu 24.04 install/idempotency, новый PID 1, format-2 schema `1 → 2` update, автоматический rollback `2 → 1`, terminal timer no-op, successful finalization с переводом `recovery` на новый release и повторный finalized PID 1 прошли без failed units. Кандидат готов к первой установке на физическое железо, но ещё не опубликован в GitHub и подписан только disposable acceptance key.
...
| Packaging | `3C13B09_LOCAL_INSTALL_READY / PUBLISH_PENDING` | `0.1.0-traffic.3c13b09` имеет reproducible 76-file bundle, verified Ed25519 tree, SBOM/provenance/channel consistency, fresh install/idempotency и два новых PID 1. Permanent key, фактический LAN interface, push/tag/assets и публичная GitHub installation остаются отдельными рубежами |
...
Следующий инкремент — внешний release/hardware gate. После явного разрешения пользователя: push локальных commits, создание отдельной production signing identity и immutable GitHub tag/release с exact artifacts `0.1.0-traffic.3c13b09`. Затем — установка одной командой на физический Ubuntu 24.04 Gateway и реальный VPS, фиксация фактического LAN interface, HiLink/Keenetic/WireGuard packet captures и исправление только реально найденных отклонений. Push/tag/release и работа с production key без отдельного разрешения не выполняются.
...
6. Публичный remote и GitHub CI работают; GitHub release immutability включена. Exact disposable signed `0.1.0-traffic.3c13b09` прошёл local release gate, но отдельный backed-up production key и реальный tag/release ещё не подготовлены; CI не получает release secrets, а 39 локальных commits не отправлены без разрешения пользователя.
