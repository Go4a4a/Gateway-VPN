...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / REMOTE_GATEWAY_UPDATE_CONTRACT_ADDED / SCHEMA_V30_SOURCE / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_GATEWAY_PRIVILEGED_NETNS_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V4_SOURCE / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / VPS_OPERATIONS_DIAGNOSTICS_LOCAL_PASS / VPS_OPERATIONS_ROOT_BOUNDARY_PRIVILEGED_PASS / GATEWAY_VPS_AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_PRIVILEGED_PACKET_3X_PASS / E2E_RELAY_ROLE_BACKUP_ROUNDTRIP_PASS / WG_ADMIN_ROTATION_ROLLBACK_PASS / LOCAL_RESOURCE_GATEWAY_PACKET_PASS / TOPOLOGY_PROFILE_SAFE_APPLY_PRIVILEGED_PASS / INITIAL_LAN_ROLE_IMPORT_LOCAL_PASS / GROUPED_RESPONSIVE_WEBUI_BROWSER_PASS / FIREWALL_SCHEMA_V6 / WG_INGRESS_LOCAL_PASS / WATCHDOG_19_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. Текущий source реализует signed VPS pointer update: strict staging/WebUI, offline DB migration, atomic release+DB switch, redundant journals, independent recovery, 24h finalization и lifecycle packaging. Полный локальный Go/vet/syntax/systemd suite проходит; перед закрытием блока требуется exact disposable-signed privileged systemd apply/finalize/SIGKILL rollback gate. Затем обязательный scope включает remote Gateway update/complete restore-point retention, Windows deploy delivery и exact release audit. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`; относительно текущего расширенного обязательного scope программная часть ориентировочно `93%`. Готовность к первой установке на физический стенд — примерно `89%`, полная production-готовность — примерно `77%`. Signed VPS update теперь реализован и локально проверен, но до privileged gate не считается закрытым. В программном scope остаются его exact host evidence, remote Gateway update sources/automation/full restore-point UI и lifecycle, Windows deploy delivery и финальная exact release/security сборка. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Management Fabric / VPS Hub | `BOTH_ROLES_PRIVILEGED_APPLY_AND_COEXISTENCE_PASS / E2E_RELAY_PACKET_3X_PASS / OPERATIONS_DIAGNOSTICS_LOCAL_PASS / VPS_UPDATE_SOURCE_PASS` | Обе роли имеют parameter-free root apply/recovery. VPS updater реализует strict upload, pointer-compatible host contract, DB copy migration, atomic switch, independent recovery и path-free WebUI status; exact privileged update lifecycle gate ещё pending |
...
| Signed update | `GATEWAY_POINTER_BASELINE_PASS / VPS_TRANSACTION_LOCAL_PASS_PRIVILEGED_PENDING / REMOTE_GATEWAY_DELIVERY_RETENTION_PENDING / HARDWARE_POWER_CUT_PENDING` | Gateway pointer/host-contract baseline сохраняется. VPS staging/DB+release transaction/redundant recovery/finalize/API/UI/packaging проходят local tests; exact signed systemd apply/finalize/SIGKILL rollback gate подготовлен, но ещё не выполнен. Полный Gateway GitHub/WebUI remote update и complete restore-point retention pending |
...
| Автоматические тесты | `FULL_GO_VET_SYNTAX_PASS / VPS_UPDATE_JOURNAL_RACE_FIXED / UBUNTU24_SYSTEMD_PASS / EXACT_VPS_UPDATE_GATE_PENDING` | Полный `go test ./... -count=1 -p 1`, `go vet ./...`, все Node/shell syntax, diff и Ubuntu 24 systemd/GRUB gates проходят. Новые tests покрывают restrictive umask, runtime ordering, public-status boundary, потерю active marker и повреждение обеих journal copies |
