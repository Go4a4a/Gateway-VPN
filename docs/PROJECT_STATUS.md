...
| SQLite | `V25_SYSTEM_UNINSTALL_MIGRATION_AND_EXACT_16_TO_25_PASS` | Migration 25 добавляет partial unique invariant для одной активной `SYSTEM_UNINSTALL`; migrations 23–24 сохраняют ingress/watchdog contour без private keys в SQLite. Exact `16 → 25`, integrity/foreign-key/migration history, SIGKILL/new-PID1 preserve и reinstall проходят; physical power cut остаётся внешним gate |
...
| Автоматические тесты | `SCHEMA_V25_FULL_LINUX_NETNS_SYSTEMD_BROWSER_HOST_UPGRADE_UNINSTALL_PASS` | Полные serial `go test ./... -count=1` и `go vet ./...`, Linux cross-build, JavaScript/shell syntax, diff check, Ubuntu WireGuard JSON contour, SSH multi-LAN/uplink/opt-out, firewall recovery, реальный `wg-ingress` kernel handshake/lifecycle, offline systemd verify, browser preview, exact host-upgrade и new-PID1 uninstall/reinstall проходят. Hardware gates ещё нужны |
