...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V26 / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_DATA_PAIRING_RENDERER_LOCAL_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V3_MANAGED_ADMIN_CONFIG_LOCAL_PASS / VPS_HOST_PLAN_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / AMNEZIA_COEXISTENCE_REQUIRED / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_SPEC_ADDED / LOCAL_RESOURCE_ACCESS_FOUNDATION / GROUPED_WEBUI_SPEC_ADDED / FIREWALL_SCHEMA_V4 / WG_INGRESS_LOCAL_PASS / WATCHDOG_17_IMPLEMENTED_19_SPECIFIED / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. В текущем successor source Gateway schema 26 реализует durable Management Fabric foundation, а VPS Agent schema 3 — pairing/Gateway/resources/ACL control plane и полный managed administrator key/config lifecycle: one-use `.conf`, deletion before response, make-before-break replacement и backup/restore filtering. Детерминированный VPS host-plan уже выводит только fixed `wg-mgmt`/51821/protocol-186 peers/routes/ACL без private data или commands, но остаётся read-only. Restricted VPS Hub имеет функциональные предметные страницы и полный `.gvpn-vps` backup/restore; обязательный Gateway `.gvpn` остаётся отдельной ролью. Для расширения всё ещё отсутствуют parameter-free root fabric trigger, privileged WireGuard/route/double-ACL apply/rollback, реальный host watchdog/recovery, logs/update/diagnostics Hub, Amnezia coexistence lifecycle и grouped Gateway WebUI. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`, а относительно расширенного обязательного scope от 2026-08-30 — ориентировочно `88%`; общая production-готовность расширенного проекта — ориентировочно `71%`. Schema 26 foundation, VPS Hub schema-3 control plane, managed administrator delivery, read-only host projection и обе обязательные WebUI backup-роли закрыты локальным кодом и тестами, но privileged many-to-many host runtime, full Hub watchdog/lifecycle и coexistence ещё впереди. Далее обязательны multi-site/multi-VPS/Amnezia netns/systemd/security tests и физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
| Management Fabric / VPS Hub | `SCHEMA3_CONTROL_MANAGED_ADMIN_HOST_PLAN_LOCAL_PASS / PRIVILEGED_FABRIC_RUNTIME_PENDING` | VPS Agent управляет invitations, Gateway, external/managed administrators, typed resources/ACL и generations. Managed `.conf` выдаётся один раз, rotation сохраняет source peer, а deterministic host-plan содержит fixed peers/addresses/alias routes/ACL без commands/private data. Root apply/rollback, operational logs/update/diagnostics и coexistence отсутствуют |
...
| API / Web UI | `VPS_HUB_GROUPED_AND_MANAGED_ADMIN_API_LOCAL_PASS / BROWSER_RETEST_PENDING / GATEWAY_GROUPED_NAV_PENDING` | VPS Hub pages/API теперь создают external либо re-authenticated managed peer, one-use config, replacement rotation, revoke, resources и ACL. Предыдущий grouped browser smoke прошёл, но новый managed flow после этой правки ещё требует browser retest. Hub Logs/Diagnostics и grouped Gateway navigation отсутствуют |
...
| Backup / restore | `GATEWAY_EXACT_SIGNED_POWER_CUT_PASS / VPS_ROLE_MANAGED_KEY_FILTER_SIGKILL_RECOVERY_PASS / HOST_NOT_RUN` | `.gvpn-vps` включает только AVAILABLE managed admin secrets и исключает consumed/revoked/orphan. Import-as-new удаляет source administrator tree; same-VPS, cross-role, SIGKILL/fresh-boot rollback проходят. Реальный bare-metal VPS power cut ещё не выполнялся |
...
| Автоматические тесты | `VPS_MANAGED_ADMIN_HOST_PLAN_FULL_GO_BROWSER_PASS / PRIVILEGED_RUNTIME_PENDING / SCHEMA25_EXACT_HOST_EVIDENCE` | Полные Windows и offline Linux test/vet/build, Node syntax, format/diff и VPS Hub browser smoke проходят для schema 3, one-use/concurrent semantics, rotation, backup filtering, import-as-new purge и typed host-plan. Privileged many-to-many runtime не выполнялся |
...
Следующий инкремент — parameter-free privileged fabric trigger, journalled WireGuard/route/double-ACL apply/rollback и legacy `wg-mgmt` adoption; до первой root mutation добавить negative Amnezia/Docker/UFW coexistence fixtures. Затем выполнить privileged multi-Gateway/netns/systemd/browser gates. Production `.gvkey` не открывать, tag/Release не создавать.
