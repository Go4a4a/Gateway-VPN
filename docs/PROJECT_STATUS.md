...
| Packaging | `KEY_CUSTODY_HARDENED / PUBLIC_RELEASE_PENDING` | `0.1.0-validation.47297a7` имеет reproducible validation bundle; production keygen теперь Linux-only, absolute/same-secure-directory/outside-Git, exclusive/fsynced и self-verifying. Public tags/releases пока отсутствуют; permanent key, новая RC version, LAN interface, draft assets и GitHub installation остаются отдельными рубежами |
