...
**Текущий этап:** Шестой unique exact signed candidate `d471c12` полностью прошёл Ubuntu 24.04 DHCP installer acceptance: archive dry-run, fresh apply, exact idempotent reinstall и новый PID 1 с pre-network test `lan0`; control/firewall guard/broker/dnsmasq/finalize timer active, Mihomo ожидаемо condition-inactive без LKG, HTTPS 200, DNS/DHCP sockets, state/lease identity, `PATH_BLOCKED`, отсутствие active transaction artifacts и `NRestarts=0` проверены. Installer regression закрыт. Следующий инкремент — реализация PLAN §9.8 self-health supervisor, typed recovery boundary, API/OpenAPI/WebUI и crash/hang/reboot-loop matrix до 24-часового endurance
