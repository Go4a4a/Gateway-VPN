...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / REMOTE_GATEWAY_UPDATE_CONTRACT_ADDED / VPS_UPDATE_SYSTEMD_SIGKILL_ROLLBACK_PASS / SCHEMA_V30_SOURCE / EXACT_UNINSTALL_NEW_PID1_RECOVERY_PASS / MANAGEMENT_FABRIC_GATEWAY_PRIVILEGED_NETNS_PASS / VPS_AGENT_HUB_CONTROL_PLANE_LOCAL_PASS / VPS_AGENT_SCHEMA_V4_SOURCE / VPS_PRIVILEGED_FABRIC_RUNTIME_LOCAL_PASS / VPS_AGENT_HUB_BACKUP_LOCAL_PASS / VPS_OPERATIONS_DIAGNOSTICS_LOCAL_PASS / VPS_OPERATIONS_ROOT_BOUNDARY_PRIVILEGED_PASS / GATEWAY_VPS_AMNEZIA_UFW_DOCKER_COEXISTENCE_NETNS_PASS / ROLE_SEPARATED_BACKUP_LOCAL_PASS / WINDOWS_DEPLOY_LATE_STAGE_SPEC_ADDED / E2E_ADMIN_RELAY_PRIVILEGED_PACKET_3X_PASS / E2E_RELAY_ROLE_BACKUP_ROUNDTRIP_PASS / WG_ADMIN_ROTATION_ROLLBACK_PASS / LOCAL_RESOURCE_GATEWAY_PACKET_PASS / TOPOLOGY_PROFILE_SAFE_APPLY_PRIVILEGED_PASS / INITIAL_LAN_ROLE_IMPORT_LOCAL_PASS / GROUPED_RESPONSIVE_WEBUI_BROWSER_PASS / FIREWALL_SCHEMA_V6 / WG_INGRESS_LOCAL_PASS / WATCHDOG_19_LOCAL_PASS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact schema-25 candidate `0.1.0-successor.ga7a783b` сохраняет доказанный uninstall/new-PID1 baseline. Signed VPS update закрыт privileged Ubuntu 24/systemd gate: проверены strict staging/WebUI, offline DB migration, atomic release+DB switch, redundant journals, independent recovery, 24h finalization contract и автоматический rollback после `SIGKILL`. Следующий обязательный блок — remote Gateway update и полный lifecycle complete restore points; затем Windows deploy delivery и exact release/security audit. Новый tag/Release запрещён до отдельного разрешения пользователя.

**Оценка прогресса:** относительно прежнего schema-25 scope программная реализация остаётся примерно `97%`; относительно текущего расширенного обязательного scope программная часть ориентировочно `94%`. Готовность к первой установке на физический стенд — примерно `90%`, полная production-готовность — примерно `78%`. Signed VPS update реализован и прошёл privileged apply/finalize/`SIGKILL` rollback gate. В программном scope остаются remote Gateway update sources/automation, полный restore-point retention/WebUI и lifecycle, Windows deploy delivery и финальная exact release/security сборка. Privileged Docker не заменяет физические Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic captures, hardware-validated firmware/USB recovery и RTC S5, а также несокращаемые 24/72-часовые endurance.
...
|---|---|---|
| Архитектурный план | `DONE / REMOTE_GATEWAY_UPDATE_AMENDED_2026-08-31` | Закреплены generic uplinks/topology/Management Fabric и отдельный remote Gateway update contract: signed sources, preflight/baseline, independent recovery, two-stage acceptance и complete restore-point retention |
...
| Firewall / routing | `SCHEMA_V6_DYNAMIC_MANAGEMENT_AND_TOPOLOGY_NETNS_PASS / HARDWARE_PENDING` | Schema `6` сохраняет counters, TUN/direct и Management Fabric gates, проецирует exact set локальных management-интерфейсов и peer-scoped one-arm rules. Privileged netns подтвердил startup/flush recovery, multi-LAN SSH, `wg-ingress`, topology rollback, единственный direct mark для `wg-ingress`, allowlisted peer и блокировку spoofed source; физический capture остаётся обязательным |
...
| Management Fabric / VPS Hub | `BOTH_ROLES_PRIVILEGED_APPLY_AND_COEXISTENCE_PASS / E2E_RELAY_PACKET_3X_PASS / OPERATIONS_DIAGNOSTICS_LOCAL_PASS / VPS_UPDATE_SYSTEMD_GATE_PASS` | Обе роли имеют parameter-free root apply/recovery. VPS updater реализует strict upload, pointer-compatible host contract, DB copy migration, atomic switch, independent recovery и path-free WebUI status; privileged gate подтвердил success/finalize и автоматический rollback согласованной release+DB пары после `SIGKILL` |
...
| SQLite | `GATEWAY_V30_AND_VPS_AGENT_V4_LOCAL_PASS / EXACT_RELEASE_16_TO_25_PASS` | Gateway schema 30 добавляет durable topology profile/generation/state и расширяет safe-apply operation kind; schema 29 inner contour/relay/tunnel/trust mode сохраняется. Contiguous VPS Agent schema 4 хранит public relay allocation и external administrator trust mode без inner private key. v1→current migration, integrity/FK/lifecycle и оба encrypted backup/restore round-trip проходят. Exact signed host evidence пока относится к Gateway schema 25 |
...
| API / Web UI | `GROUPED_GATEWAY_AND_VPS_LOCAL_PASS / MAINTENANCE_IA_SPEC_PENDING_IMPLEMENTATION` | Обе роли прошли предметные вкладки при 320×720. План теперь отдельно делит Gateway «Обслуживание» на обновление, версии/восстановление, backup и диагностику; новый update/retention UI ещё не реализован и не проходил browser gate |
| Logging / audit | `GATEWAY_AND_VPS_THEMATIC_LOCAL_PASS / VPS_ROOT_SNAPSHOT_BOUNDARY_PASS / BARE_METAL_PENDING` | Gateway logging contract сохранён. VPS Hub объединяет bounded journald records и security audit по тематическим категориям, применяет второй redaction pass к URL credentials/query, authorization, WireGuard keys/PSK и structured secrets. Root snapshot ownership доказан в privileged Ubuntu; реальные VPS retention/journald и Gateway OpenSSH/SFTP ещё не проверены на железе |
...
| Signed update | `GATEWAY_POINTER_BASELINE_PASS / VPS_TRANSACTION_SYSTEMD_SIGKILL_ROLLBACK_PASS / REMOTE_GATEWAY_DELIVERY_RETENTION_PENDING / HARDWARE_POWER_CUT_PENDING` | Gateway pointer/host-contract baseline сохраняется. VPS staging/DB+release transaction/redundant recovery/finalize/API/UI/packaging и exact disposable-signed systemd apply/finalize/SIGKILL rollback прошли. Полный Gateway GitHub/WebUI remote update и complete restore-point retention pending |
| Packaging | `PRODUCTION_GVKEY_READY / VPS_AGENT_SOURCE_GATES_PASS / VPS_OPERATIONS_SYSTEMD_PASS / REPRODUCIBLE_A7A783B_BASELINE / PUBLIC_5723940_IMMUTABLE` | VPS signed-tree contract требует Agent/config/restore/fabric/operations units; installer, recovery и uninstall синхронизированы. Ubuntu 24 systemd graph и root snapshot gate прошли. VPS dependency allowlist исправлен с `python3-minimal` на полный `python3`, потому что строгие JSON gates требуют стандартную библиотеку. Новый exact signed artifact ещё не собирался; production key не открывался, public Release неизменён |
...
| Автоматические тесты | `FULL_GO_VET_SYNTAX_PASS / VPS_UPDATE_JOURNAL_RACE_FIXED / UBUNTU24_SYSTEMD_PASS / EXACT_VPS_UPDATE_GATE_PASS` | Полный `go test ./... -count=1 -p 1`, `go vet ./...`, все Node/shell syntax, diff и Ubuntu 24 systemd/GRUB gates проходят. Новые tests покрывают restrictive umask, runtime ordering, public-status boundary, потерю active marker и повреждение обеих journal copies; privileged VPS update gate подтверждает success/finalize/`SIGKILL` rollback |
...
| 10 | `IN_PROGRESS_LOCAL` | Прежние README/NETWORKING/OPERATIONS/H1/H2 входили в проверенный signed tree; Management Fabric/topology/local-resource contract и фактический safe-apply lifecycle реализованы и описаны в текущем source, но ещё не собраны и не проверены как новый exact production-signed tree |
...
| 43 | `IN_PROGRESS_LOCAL` | Lightweight Go Agent/Hub, Management Fabric, backup, logs/diagnostics и signed pointer-update transaction реализованы. VPS updater проходит local transaction/API/packaging/systemd checks; exact privileged update lifecycle и portable Windows delivery ещё не завершены |
...
| 47 | `IN_PROGRESS_LOCAL` | Базовые Gateway signed staging, pointer/DB rollback и host-contract upgrade существуют; GitHub channel polling/download, maintenance scheduling, two-stage baseline evaluation, complete restore-point retention/UI и multi-VPS observation ещё требуют реализации и interruption gates |

**Итог аудита:** `28 PASS_LOCAL`, `5 IN_PROGRESS_LOCAL`, `12 PARTIAL_EXTERNAL`, `2 NOT_RUN_EXTERNAL`. Полный Definition of Done не достигнут: обе backup-роли, information architecture, privileged Gateway/VPS Management Fabric, end-to-end administrator relay, единая topology transaction и Hub operations/diagnostics закрыты локально; signed VPS update, remote Gateway update/retention и Windows delivery требуют реализации/evidence. Затем остаются отдельно разрешаемая production publication, H1/H2/VPS/RTC и реальные 24/72h evidence.

## Ближайший следующий инкремент

Следующий инкремент — зафиксировать exact local source baseline и выполнить подготовленный disposable-signed Ubuntu 24 VPS systemd gate: успешный apply/finalize, SIGKILL root updater, automatic rollback release+DB и сохранение foreign state. Следом реализовать remote Gateway update sources/automation, two-stage acceptance и complete restore-point retention/WebUI; после этого — Windows deploy delivery. Production `.gvkey` не открывать, tag/Release не создавать.

## Критический путь до release

Расширенный universal-installer successor локально реализует data-plane, refresh, generic uplinks, direct/VPN read model, node preferences, startup policy, SSH/OpenSSH, входящий WireGuard, тематические SFTP/Hub-логи, bounded diagnostics, privileged many-to-many Management Fabric, проверенный inner `wg-admin` relay с role backup round-trip и единую post-install смену topology. Signed VPS update, полный remote Gateway update/restore-point lifecycle и Windows delivery ещё не завершены. Поэтому exact `a7a783b` нельзя объявлять окончательным `INSTALL_READY` для всего расширенного проекта, хотя schema-25 recovery gate закрыт. Production key существует отдельно и не открывается; immutable version tag/GitHub Release не создаются без отдельного разрешения пользователя.
...
9. Schema 30 Gateway / schema 4 VPS Agent, обязательные role backups, topology safe apply и privileged Gateway/VPS runtimes существуют в current source, но опубликованные/exact signed artifacts относятся к прежнему schema-25 scope. Gateway/VPS multi-peer ACL/coexistence, inner admin-relay packet/anti-spoof/backup round-trip и topology/one-arm gate прошли privileged Docker/local gates; полный Hub lifecycle и новый exact signed install gate ещё не закрыты.
