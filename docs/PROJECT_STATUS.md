...
**Текущий этап:** portable encrypted release-key workflow опубликован в `main`: exact clean commit `08b7c5c` создал disposable `.gvkey`, byte-identical backup и полный подписанный Gateway/VPS/channel bundle при `noexec /dev/shm`; exact successor `cbd8bf7` зафиксировал согласованный минимум 10 Unicode-символов и прошёл remote CI. Постоянный production key ещё не создан: две попытки открыть интерактивный prompt через Codex UI/отдельный PowerShell завершились до ввода, а целевые каталоги доказанно пусты. Предыдущий docs-complete validation successor `0.1.0-validation.47297a7` и его hardware handoff evidence остаются неизменными; будущий публичный hardware release получает новую immutable version и permanent signer.
...
| Репозиторий | `PUBLIC_MAIN_CBD8BF7 / REMOTE_CI_PASS` | Пользователь явно разрешил push; encrypted key workflow и 10-character policy опубликованы в `Go4a4a/Gateway-VPN`. Exact GitHub Actions runs `33052162474` (`08b7c5c`) и `33053389413` (`cbd8bf7`) завершились `completed/success`; tag/release/production key не создавались |
...
| Packaging | `PORTABLE_GVKEY_BUNDLE_PASS / PUBLIC_RELEASE_PENDING` | Exact `08b7c5c` при `--network none` создал disposable Argon2id/AES-GCM `.gvkey`, byte-identical verified backup и полный Gateway/VPS/bootstrap/deploy/channel bundle; private PEM раскрывался только в `noexec /dev/shm`, удалился через trap и отсутствовал в `dist`. Public tags/releases отсутствуют; permanent key, новая RC version, LAN interface, draft assets и GitHub installation остаются отдельными рубежами |
...
| Автоматические тесты | `REMOTE_CI_CBD8BF7_PASS` | Exact GitHub Actions run `33053389413`: formatting, race-enabled suite, vet, Linux release entrypoints, JS/shell syntax, native nft/netns fail-closed и Gateway/VPS systemd verification на Ubuntu 24.04 — PASS |
