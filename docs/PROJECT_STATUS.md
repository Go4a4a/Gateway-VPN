...
**Текущий этап:** Exact signed candidate `0.1.0-endurance.43a4bd0` собран и verified, dry-run Ubuntu 24.04 PASS, но apply корректно откатился: dnsmasq после privilege drop не мог пройти через state root mode `0700`. Исправление `0710` traverse-only + supplementary service group покрыто packaging test; полный Windows и offline Linux test/vet PASS, повторный unique signed install ещё впереди. Новое обязательное требование круглосуточного self-health supervisor внесено в PLAN §9.8; до 24-часового endurance нужно реализовать и проверить его bounded settings/API/WebUI/recovery policy

**Оценка прогресса:** около `96%` программной реализации и около `87%` полной production-готовности. Оценка снижена после принятия нового supervisor scope и найденного signed-install defect. Production-оценка дополнительно включает ещё не выполненные permanent signing/public GitHub release, реальный Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture и обязательный 24/72-часовой endurance, которые нельзя заменить disposable signing, privileged Docker/systemd, netns или cross-build.
...
| Self-health / watchdog | `FOUNDATION_PRESENT / POLICY_NOT_IMPLEMENTED` | systemd restart, always-on firewall guard, modem/reconcile/periodic health и transaction recovery уже есть; единый supervisor, component health read model, flexible bounded settings/API/WebUI, hang detection и durable restart/reboot-loop protection приняты в PLAN §9.8, но ещё не реализованы |
...
| Автоматические тесты | `LOCAL_WINDOWS_AND_LINUX_PASS / REMOTE_PENDING` | Полный Windows и network-disabled Linux/amd64 `go test ./...`/`go vet ./...` проходят для state-root permission/PLAN §9.8 инкремента; systemd graph ранее прошёл `systemd-analyze verify`. Exact signed `.13` подтвердила orchestration/handshake, `.21/.22` — update recovery, `.27` — restore и Gateway uninstall; byte-identical current VPS uninstaller прошёл preserve/reinstall/purge. Удалённый CI для local-ahead commits ещё не выполнен |

## Ближайший следующий инкремент

Следующий локальный инкремент: завершить полный Windows/Linux suite для state-root permission fix, commit, собрать новую unique exact signed version и повторить Ubuntu 24.04 dry-run/apply/reboot/readiness. Затем реализовать PLAN §9.8: durable bounded watchdog policy, component supervisor/read model, typed recovery boundary, API/OpenAPI и карточку WebUI; только после его crash/hang/reboot-loop tests собирать финальный VCS-stamped candidate, выполнять harness smoke и запускать несокращаемый 24-часовой developer run. Параллельный внешний путь — bare-metal Ubuntu 24.04 Gateway и VPS: exact public-GitHub install dry-run/apply, reboot, внешний WireGuard handshake и HiLink/Keenetic packet-capture matrix минимум с одним модемом. Multi-modem gate требует отдельно минимум два модема с разными management-подсетями. Положительный Ubuntu 20.04 acceptance остаётся внешним gate на Pro-attached VPS; до production tag/assets нужен backed-up long-lived Ed25519 key и подтверждённое место его хранения.

## Критический путь до release

Основной оставшийся путь — self-health supervisor, повтор signed install acceptance, developer endurance, release/hardware integration acceptance и найденные при ней исправления. Signed update и restore reboot/power-cut matrices уже пройдены в privileged Docker/systemd; отдельно ещё требуются watchdog crash/hang/reboot-loop matrix, 24-часовой developer run, production GitHub/Ubuntu deploy, VPS reboot, Keenetic/HiLink packet-capture matrix и несокращаемый 72-часовой release endurance.

Если целевые Linux Gateway, VPS и модемы доступны без пауз, обновлённый оптимистичный календарный ориентир до проверенного release — `5–10 дней`: примерно `2–5` интенсивных дней на supervisor, повторную signed integration и исправления, затем минимум `72 часа` release endurance. Это не обещанная дата: отсутствие стенда, найденные routing/hardware defects либо новое расширение scope сдвигают срок. Без фактического доступа к Linux/VPS/оборудованию можно завершить код и synthetic tests, но нельзя честно поставить production status `DONE`.
...
| DEV-126 | 2026-08-26 | Shared state root имеет mode `0710 gateway-vpn:gateway-vpn`; изолированные `gateway-vpn-mihomo` и `gateway-vpn-dns` входят в supplementary group только для traverse, а secrets и остальные private children сохраняют `0700` | Exact signed DHCP install обнаружил, что прежний parent `0700` делает правильно принадлежащий dnsmasq lease directory недостижимым после privilege drop; Mihomo имел тот же latent defect. `0710` не разрешает group list/read/write root |
...
**Найдено:** namespaced journal доказал `dnsmasq: cannot open or create lease file ... Permission denied`. `/var/lib/gateway-vpn` имел `0700 gateway-vpn:gateway-vpn`, поэтому изолированный dnsmasq UID не мог traverse к собственному child; Mihomo имел тот же latent defect. Исправление меняет только parent на `0710`, добавляет dns UID в supplementary group и сохраняет private children `0700`. Полный Windows и network-disabled Linux/amd64 `go test ./... -count=1` + `go vet ./...` — PASS. Повторный signed install пока не выполнен.
...
**Следующий шаг:** полный test/vet для permission fix, commit и новая unique signed Ubuntu install; затем реализация supervisor до harness smoke и 24-часового developer endurance.
