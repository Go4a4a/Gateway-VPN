...
**Общее состояние:** `IMMUTABLE_PUBLIC_RELEASE / EXPANDED_SUCCESSOR_IMPLEMENTATION / SCHEMA_V21 / ETHERNET_RUNTIME_LOCAL_PASS / WATCHDOG_EXPANSION_IN_PROGRESS / HARDWARE_GATES_PENDING`
**Текущий этап:** опубликованный successor `0.1.0-successor.5723940` остаётся неизменяемым контрольным release с проверенной GitHub attestation. Exact source `1b90ffcb99b25f79954cbc1b4bde7bcc0140175d` и disposable-signed `0.1.0-successor.1b90ffc` сохраняются как доказательство прежнего universal-installer scope, но больше не являются кандидатом текущего расширенного контракта. Canonical generic `uplink × access method`, direct-only whitelist classifier, bounded physical modem recovery, safe Ethernet lifecycle/runtime reconciliation и ручное capability-gated power management реализованы локально. Текущий блок расширяет self-health/watchdog на WireGuard, policy routing, фоновые workers и convergence. Затем остаются `wg-ingress`, тематические логи/SFTP и полный contextual help. Firmware/USB recovery и RTC wake-from-S5 остаются аппаратными gates конкретного Gateway. Новый tag/Release запрещён до реализации и проверки всего расширения.

**Оценка прогресса:** прежний scope остаётся воспроизводимо завершённым примерно на `99%`, но это больше не процент текущего проекта. После расширения знаменателя текущая программная реализация оценивается примерно в `86%`, общая production-готовность — примерно в `69%`. Это не потеря уже сделанного: installer, update/rollback, fail-closed, подписки, multi-uplink policy, API/auth/logging/watchdog и release pipeline переиспользуются. Незавершённый объём теперь включает Ethernet runtime observation/routing convergence, hardware-validated firmware/USB recovery, полный `wg-ingress`, оставшиеся generic WebUI/read models, exports/help и повторный сквозной audit. После этого всё равно остаются physical Ubuntu Gateway/VPS, Mihomo/WireGuard/HiLink/Keenetic packet capture, RTC S5 и обязательные 24/72-часовые endurance.
...
| Репозиторий | `EXPANDED_SUCCESSOR_WORKTREE / PUBLIC_BASE_IMMUTABLE` | Public `v0.1.0-successor.5723940` и Release `378316577` неизменны. Новый schema-v21/uplink/watchdog increment проходит локальный audit, не подписан, не тегирован и не опубликован |
...
| SQLite | `V21_ETHERNET_RUNTIME_MIGRATION_LOCAL_PASS / UPDATE_AUDIT_PENDING` | Migration 21 разделяет configured и observed Ethernet addressing, добавляет readiness reason и HiLink compatibility projection. Полный migration suite, backup/recovery/update fixtures и foreign-key checks проходят. Signed 13→21 update/recovery gate ещё не запускался |
...
| API / Web UI | `ETHERNET_LIFECYCLE_AND_MANUAL_POWER_LOCAL_PASS / WATCHDOG_WG_INGRESS_UX_PENDING` | Вкладка физических выходов создаёт, включает, выключает и безопасно удаляет Ethernet, переназначает карту, меняет DHCP/static, показывает observed readiness/impact и не теряет configured DNS. В «Система и безопасность» добавлено питание. Watchdog settings, WG-ingress clients, thematic log tabs и полный contextual help ещё не завершены |
...
| Автоматические тесты | `SCHEMA_V20_WINDOWS_LINUX_GO_TEST_VET_SYSTEMD_PASS / NETNS_SUCCESSOR_AUDIT_PENDING` | Полные Windows и Linux/amd64 `go test ./...`/`go vet ./...`, JavaScript syntax, diff checks, Ubuntu 24.04 `bash -n` и `systemd-analyze verify` текущего worktree проходят. Прежний exact CI остаётся evidence только прежнего scope; новый networkd/netns и signed update gate ещё нужен |
