...
`gateway-vpn-deploy` запускается с отдельного административного Linux/amd64 компьютера. Нужны `/usr/bin/ssh`, HTTPS downloader, `sha256sum`, два заранее проверенных SSH host keys в одном absolute `known_hosts`, passwordless `sudo -n` на обеих machines и отдельные SSH destinations. Launcher запускает OpenSSH с `-F /dev/null`, `BatchMode=yes`, `StrictHostKeyChecking=yes`, запрещёнными password/keyboard-interactive/TTY и bounded output; произвольные SSH options, ProxyCommand и shell fragments из пользовательских полей не принимаются.
