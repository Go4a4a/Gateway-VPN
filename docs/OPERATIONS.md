...
### Первая установка без Windows

Windows не является обязательным компонентом. Если к будущему Gateway доступны монитор и клавиатура, на самой Ubuntu можно выполнить локальную команду из нужного GitHub Release. После проверки подписи запускается тот же интерактивный мастер: он показывает интерфейсы, подсети, SSH/SFTP, DHCP и WireGuard, а изменения выполняются только после точного подтверждения `INSTALL`. Вводить topology token вручную не нужно — мастер создаёт его из выбранных портов, а установщик проверяет его перед изменениями.

Windows `gateway-vpn-deploy.exe` — только дополнительный способ, когда администратор хочет одной сессией настроить локально доступный Gateway и отдельный VPS. Белый IP у Gateway для этого не нужен: Windows должен видеть Gateway по локальной сети, а VPS — по своему публичному адресу. Для полностью изолированного объекта остаётся офлайн-вариант: заранее скачанный подписанный release переносится на Gateway и проверяется тем же bootstrap; приватные ключи подписи на Gateway не копируются.
...
Signed release дополнительно содержит `install-deploy-windows-<version>.command.txt`. Его содержимое копируется в уже открытый PowerShell — файл `.ps1` запускать и менять ExecutionPolicy не требуется. Перед вставкой создайте или откройте project-local рабочую папку и выполните команду из неё; launcher создаёт скрытую случайную подпапку `.gateway-vpn-deploy-<id>` внутри текущего каталога, а не использует `%TEMP%` или профиль Windows. Через HTTPS загружаются exact Windows EXE, raw manifest, detached signature и public signing key; SHA-256 EXE и manifest сверяются **до запуска**, затем открывается русскоязычный `--interactive` wizard и временная подпапка гарантированно удаляется. После успеха или ошибки исходное окно не закрывается: команда показывает результат, сохраняет точный launcher code в `$LASTEXITCODE`, восстанавливает изменённую на время загрузки TLS-настройку и возвращает обычный prompt для диагностики. Перед использованием администратор независимо сверяет signer fingerprint и SSH host-key fingerprints с доверенным источником.
