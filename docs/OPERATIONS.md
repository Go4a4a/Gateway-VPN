...
Gateway command может включать `--install-dependencies`. До mutation installer проверяет signed release, Ubuntu 24.04/x86_64, NTP/DNS/RAM/disk и целостность APT/dpkg, затем симулирует только отсутствующие `iproute2`, `nftables`, `wireguard-tools`, `kmod`, `procps`, `dnsmasq-base` с `--no-install-recommends --no-remove --no-upgrade`. Используется именно binary-only `dnsmasq-base`, чтобы installer не запускал конкурирующий общий DNS daemon; при `--enable-dhcp` существующий wildcard либо LAN listener TCP/UDP 53 блокирует apply до mutation. После `apt-get update` simulation повторяется; OS packages при application rollback/uninstall не удаляются.
