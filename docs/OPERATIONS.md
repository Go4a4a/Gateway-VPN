...
Successor добавляет один лёгкий `gateway-vpn-vps` Go Agent с embedded WebUI/CLI/SQLite. Он слушает только localhost и admin WireGuard, а SSH port forwarding остаётся аварийным способом доступа; публичный bind запрещён по умолчанию. WebUI показывает Gateway/links/admin/resources/ACL, ownership-scoped watchdog, тематические logs и diagnostics. Signed update/recovery является отдельным следующим lifecycle-блоком и до его реализации не показывается как готовая функция. Agent не предоставляет shell, generic systemd manager, APT upgrade либо power control VPS.
