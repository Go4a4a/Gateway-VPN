...
Retention применяется к полным restore points, а не к отдельным binaries. Каждый point содержит retained identity проверенного подписанного release, соответствующий SQLite snapshot, application config, secrets, subscriptions, TLS, Mihomo generations/state, owned host-contract metadata, hashes и verification result. Default: `current + обязательный recovery + две более старые точки`. Пользователь может ограничить количество, общий размер и возраст и вручную удалить eligible point; current, recovery и версия активной transaction не удаляются.

Ручной rollback к исторической точке разрешён только при совместимом текущему узлу `host_contract_sha256` и требует активной authenticated session, повторного ввода текущего пароля, точной typed confirmation и повторной server-side проверки point/protected roles непосредственно перед root trigger. WebUI заранее предупреждает, что release, БД, настройки, secrets, подписки, TLS и Mihomo state будут заменены выбранным историческим состоянием, а все более новые активные данные перестанут действовать. До переключения data path переводится в `PATH_BLOCKED` и создаётся отдельная complete safety-точка текущего состояния. Восстановленная пара проходит новый initial health gate и stability window; при failure/SIGKILL/reboot fixed recovery возвращает safety-точку, а не пытается выполнить down-migration.
