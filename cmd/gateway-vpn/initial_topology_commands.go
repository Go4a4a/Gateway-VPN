...
// handoff before any host mutation. Applying non-default profiles is a separate
// rollback-protected stage and is never implied by successful decoding here.
func runInitialTopologyCheck(args []string) int {
	...
	if *lanInterface != "" {
		if err := installtopology.ValidateCurrentLAN(plan, *lanInterface, members); err != nil {
