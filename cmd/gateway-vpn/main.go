...
func run(args []string) int {
	...
	case flags.NArg() == 1 && flags.Arg(0) == "preflight":
		...
		return 1
	default:
		fmt.Fprintln(os.Stderr, "usage: gateway-vpn [--version|--check-defaults|--check-config PATH|preflight|firewall-boot|firewall-guard|network-broker|wireguard-ingress-bootstrap|initial-topology-check|watchdog|network-rollback|network-recover|database-restore|update-offline-check|update-lifecycle-check|update-apply|update-recover|update-rollback|update-finalize|serve]")
