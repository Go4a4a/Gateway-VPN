...
[[ $(wc -l <"$MARKER") == 5 ]] || { echo "Gateway VPN VPS recovery marker field count is invalid" >&2; exit 1; }
[[ $(grep -Ec '^(version|old_ipv4_forward|old_ipv6_all_forwarding|old_ipv6_default_forwarding|preserve_wg_config)=' "$MARKER") == 5 ]] || { echo "Gateway VPN VPS recovery marker schema is invalid" >&2; exit 1; }
for marker_key in version old_ipv4_forward old_ipv6_all_forwarding old_ipv6_default_forwarding preserve_wg_config; do
...
[[ "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z][0-9A-Za-z.-]*)?(\+[0-9A-Za-z][0-9A-Za-z.-]*)?$ && "$OLD_FORWARD" =~ ^[01]$ && "$OLD_IPV6_ALL" =~ ^[01]$ && "$OLD_IPV6_DEFAULT" =~ ^[01]$ && "$PRESERVE_WG_CONFIG" =~ ^[01]$ ]] || {
...
systemctl disable --now wg-quick@wg-mgmt.service gateway-vpn-vps-firewall.service >/dev/null 2>&1 || true
