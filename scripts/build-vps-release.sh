...
printf '{\n  "spdxVersion": "SPDX-2.3",\n  "dataLicense": "CC0-1.0",\n  "SPDXID": "SPDXRef-DOCUMENT",\n  "name": "Gateway-VPN-VPS-%s-linux-amd64",\n  "documentNamespace": "https://gateway-vpn.invalid/spdx/vps/%s/%s",\n  "creationInfo": {"created": "%s", "creators": ["Tool: gateway-vpn-build-vps-release"]},\n  "packages": [{"name": "Gateway VPN VPS role", "SPDXID": "SPDXRef-GatewayVPNVPS", "versionInfo": "%s", "downloadLocation": "NOASSERTION", "filesAnalyzed": false, "checksums": [{"algorithm": "SHA256", "checksumValue": "%s"}, {"algorithm": "SHA256", "checksumValue": "%s"}]}]\n}\n' "$VERSION" "$VERSION" "$COMMIT" "$BUILD_DATE" "$VERSION" "$CONTROL_SHA256" "$AGENT_SHA256" >"$DEST/share/supply-chain/sbom.spdx.json"
printf '{\n  "_type": "https://in-toto.io/Statement/v1",\n  "subject": [{"name": "bin/gateway-vpnctl", "digest": {"sha256": "%s"}}, {"name": "bin/gateway-vpn-vps-agent", "digest": {"sha256": "%s"}}],\n  "predicateType": "https://slsa.dev/provenance/v1",\n  "predicate": {"buildDefinition": {"buildType": "https://gateway-vpn.invalid/build/vps-linux-amd64/v1", "externalParameters": {"version": "%s", "profiles": ["debian-12", "ubuntu-20.04", "ubuntu-22.04", "ubuntu-24.04", "ubuntu-26.04"]}, "internalParameters": {"commit": "%s"}, "resolvedDependencies": []}, "runDetails": {"builder": {"id": "gateway-vpn-build-vps-release"}, "metadata": {"invocationId": "%s-%s"}}}\n}\n' "$CONTROL_SHA256" "$AGENT_SHA256" "$VERSION" "$COMMIT" "$VERSION" "$COMMIT" >"$DEST/share/supply-chain/provenance.intoto.json"
(
  cd -- "$DEST"
