...
Fail-closed IPv4-шлюз для одного или нескольких HiLink/Ethernet uplink, HAPP-совместимых подписок и одного Mihomo TUN.
...
Автоматизированные Linux/netns и privileged Ubuntu 24.04 Docker/systemd gates пройдены, включая firewall recovery, настоящий локальный WireGuard ingress handshake/lifecycle и текущий exact disposable-signed install/reinstall/uninstall/update/rollback rehearsal обеих ролей. Физический Gateway, HiLink/Keenetic, реальный VPS/provider UDP, clean-Windows two-target deploy и 24/72-часовой endurance ещё не пройдены, поэтому проект нельзя считать production-ready для домашнего трафика. Точный актуальный status, номера schema/candidate и ограничения находятся только в `docs/PROJECT_STATUS.md`; порядок первой проверки на железе — в разделе **Первый аппаратный acceptance** документа `docs/OPERATIONS.md`.
